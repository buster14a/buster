...
int main(void)
{
    ...
    int exchanged = atomic_compare_exchange_strong(&value, &expected, 15);
    int was_set = atomic_flag_test_and_set(&flag);
    atomic_flag_clear(&flag);
    atomic_thread_fence(memory_order_acquire);
    atomic_signal_fence(memory_order_seq_cst);
    return !(previous == 10 && exchanged && expected == 12 && atomic_load(&value) == 15 && !was_set && atomic_is_lock_free(&value) &&
