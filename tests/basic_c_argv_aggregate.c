...
static int local_text_array(void)
{
    ...
    return arguments[0].pointer[0] == '-' &&
        arguments[0].length == 13 &&
        arguments[1].pointer[0] == 'i' &&
        arguments[1].length == 7;
}

static int indexed_aggregate_copy(void)
{
    struct ParameterLike *parameters =
        indexed_parameters;
    ...
    parameters[written_parameter_count++] =
        parameters[parameter_count - 1];
    parameter_count -= 1;
    return parameter_count == 2 &&
        written_parameter_count == 2 &&
        parameters[1].
            name.pointer[0] == 'r' &&
        parameters[1].
            name.length == 5 &&
        parameters[1].location.offset == 34 &&
        parameters[1].location.line == 1 &&
        parameters[1].location.column == 35 &&
        parameters[1].type == 7 &&
...
static int large_tail_local(void)
{
    ...
    invocation.action = 3;
    return invocation.input_count == 12 &&
        invocation.action == 3 &&
        invocation.dialect == 17;
}

static unsigned long stack_argument_sum(
    unsigned long a01,
    unsigned long a02,
    unsigned long a03,
    unsigned long a04,
...
static _Bool text_equal(
    struct Text left,
    char const *right,
    unsigned long right_length)
{
...
static struct LargeInvocation parse_large_arguments(
    struct TextSlice arguments)
{
    struct LargeInvocation invocation = {
    ...
    for (unsigned long argument_index = 0;
        argument_index < arguments.length;
        argument_index += 1)
    {
        struct Text argument =
...
static struct ParsedArguments parse_arguments(
    struct ArgumentSlice arguments)
{
...
{
    unsigned long expected_invocation_size =
        sizeof(unsigned long) == 4 ? 168 : 176;
    if (sizeof(struct LargeInvocation) !=
    ...
    struct LargeInvocation large =
        parse_large_arguments(
            (struct TextSlice){
    ...
    struct ParsedArguments parsed =
        parse_arguments(command_arguments);
    if (argument_state.arguments.length != 2 ||
        argument_state.arguments.pointer[0][0] !=
            'a' ||
        argument_state.arguments.pointer[1][0] !=
            'b' ||
        parsed.input_count != 3 ||
        parsed.input_paths[2][0] != 'b')
    {
        return 10;
    }
    if (large.action != 3)
