...
long long native_aggregate_mix(struct NativeAggregate3, double, struct NativeAggregate24, long long, struct NativeAggregate5,
                               double, struct NativeAggregate7, struct NativeAggregate12, struct NativeAggregate32);
struct NativeAggregate24 native_aggregate_result(struct NativeAggregate16, double, struct NativeAggregate24, int, struct NativeAggregateAligned);
long long native_aggregate_variadic(struct NativeAggregate16, int, ...);
struct NativeAggregate24 native_aggregate_dispatch(
    struct NativeAggregate24 (*)(struct NativeAggregate16, double, struct NativeAggregate24, int, struct NativeAggregateAligned),
    struct NativeAggregate16, struct NativeAggregate24, struct NativeAggregateAligned);
int native_aggregate_call_host(void);
long long native_aggregate_host_mix(volatile struct NativeAggregate3 a, double b, volatile struct NativeAggregate24 c, long long d,
...
{
    long long result = (long long)(f * 3 + b * 2) + d;
...
struct NativeAggregate24 native_aggregate_host_result(struct NativeAggregate16 a, double b, struct NativeAggregate24 c, int d,
...
long long native_aggregate_host_variadic(struct NativeAggregate16 first, int count, ...)
{
    va_list ap;
...
long long native_aggregate_host_variadic_large(int bias, ...)
{
    va_list ap;
    __builtin_va_start(ap, bias);
    double lead = __builtin_va_arg(ap, double);
    struct NativeAggregate32 first = __builtin_va_arg(ap, struct NativeAggregate32);
    struct NativeAggregate24 second = __builtin_va_arg(ap, struct NativeAggregate24);
    double tail = __builtin_va_arg(ap, double);
    struct NativeAggregate16 last = __builtin_va_arg(ap, struct NativeAggregate16);
    __builtin_va_end(ap);
    return bias + (long long)(2 * lead + 3 * tail) + first.words[0] + 2 * first.words[3] + 3 * second.words[2] + 5 * last.words[1];
}
...
int main(void)
{
    ...
    {
        bad |= native_aggregate_mix(a, 2.5, c, 31ll, e, 3.5, g, h, i) != 5114;
        bad |= a.bytes[0] != 1 || c.words[2] != 23 || e.bytes[4] != 8 || g.bytes[6] != 15 || h.words[2] != 18 || i.words[3] != 27;
        struct NativeAggregate24 result = native_aggregate_result(p, 7.0, c, 32, aligned);
        bad |= result.words[0] != 39 || result.words[1] != 62 || result.words[2] != 57;
        bad |= native_aggregate_variadic(p, 5, a, 11.0, p, g, 33ll) != 273;
    }
    bad |= native_aggregate_call_host();
    printf("native-aggregate=%d\n", bad);
    return bad;
}
