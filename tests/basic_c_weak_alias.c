...
// musl's own spelling. A function declared through a type name rather than a
// parameter-list declarator is an object declaration to this frontend and
// cannot be called by name (issue #641), so these are proven through their
// addresses -- which is what the alias is, and what a linker resolves.
