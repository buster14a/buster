#!/usr/bin/env python3
"""Existing CI tool tests plus metamorphic evidence retention regressions."""
...
import ci_tools_test_core as _core

sys.path.insert(0, str(_core.ROOT / "tools"))
...
class MetamorphicEvidenceTests(unittest.TestCase):
    ...
    def outcome(*, phase=1, launched=1, result=0, platform_status=0, timed_out=0,
                command='"build/build-checks-ci_on-cc_clang-sanitize_on-fuzz_available_on-configs_Debug/Debug/ide.exe" cc',
                stderr=""):
    ...
    def marker(self, bundle="build/failure-0"):
    ...
    def test_base_timeout_launch_failure_and_missing_output_are_distinct(self):
        cases = (
            (self.outcome(timed_out=1, result=1), "timeout"),
            (self.outcome(launched=0, result=1), "launch-failure"),
    ...
    def test_desktop_summary_records_missing_advertised_evidence_as_required_failure(self):
        ...
        status = _core.ci_summary.write_report(environment)
...
_core.MetamorphicEvidenceTests = MetamorphicEvidenceTests

if __name__ == "__main__":
    _core.unittest.main(module=_core)
