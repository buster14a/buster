...
with `-fregister-allocator=mir-stack`. That stage is followed by a MIR
compatibility compile-only gate: the stage-2 compiler builds `ide-stage2-none`
with `-fregister-allocator=none`, which is remapped to MIR_STACK. It does not
run that output because a second MIR_STACK benchmark adds no distinct coverage.
QUALITY exercises the same machine pipeline and is covered by focused/all-mode
tests, so another full unity compile would add CI cost without distinct
self-host coverage.
