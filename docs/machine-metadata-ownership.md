...
| Binary writers | `ByteWriter` owns bounded append/patch operations for CodeView and PDB; those formats own allocation, capacity estimates, layout and final publication. `ObjectBuffer` remains separate. | The [bounded writer contract](byte-writer.md) preserves format-specific failures and output lifetimes (#314). Object-format policy is not an ISA encoding authority. |
