...
""")
# slot caller: record fixed flags and census slots
rep("""            machine_fast_close_live_ranges(arena, function, prepass, reads, writes, words, slot_starts, slot_ends);
        }""", """            u32* probe_touched = arena_allocate(arena, u32, function->stack_slot_count + 1);
            memcpy(probe_touched, slot_starts, sizeof(u32) * function->stack_slot_count);
            fast_probe.fixed = slot_fixed; fast_probe.touched_starts = probe_touched; fast_probe.slot_calls += 1; fast_probe.slots += function->stack_slot_count;
            if (function->stack_slot_count > fast_probe.max_slots) fast_probe.max_slots = function->stack_slot_count;
            for (u32 probe_slot = 0; probe_slot < function->stack_slot_count; probe_slot += 1)
            {
                if (slot_starts[probe_slot] == UINT32_MAX) fast_probe.slots_untouched += 1; else if (slot_fixed[probe_slot]) fast_probe.slots_fixed += 1; else fast_probe.slots_colorable += 1;
            }
            machine_fast_close_live_ranges(arena, function, prepass, reads, writes, words, slot_starts, slot_ends);
            fast_probe.fixed = 0; fast_probe.touched_starts = 0;
        }""")
