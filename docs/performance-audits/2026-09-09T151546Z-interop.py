...
def convert(source, destination):
    ...
    # COFF's displacement uses P+4, whereas ELF uses P; subtract four once
        ...
        if section[1] == 4:
            ...
            for offset in range(section[4], section[4] + section[5], 24):
                ...
                if info & 0xffffffff == 2:
                    struct.pack_into('<q', data, offset + 16, addend - 4)
                    adjusted += 1
    destination.write_bytes(data)
    records.append({'converted': str(destination), 'pc32_adjustments': adjusted})
host = (repo / 'tests/differential/native_variadic_host.c').read_text()
first_definition = host.index('long long native_va_host_ints(')
main = host.index('int main(void)')
callees = host[host.index('struct NativeVaResult'):main]
callees = ('typedef __builtin_va_list va_list;\n#define va_start __builtin_va_start\n#define va_arg __builtin_va_arg\n'
           '#define va_end __builtin_va_end\n#define va_copy __builtin_va_copy\n') + callees
(out / 'callees.c').write_text(callees)
observer = host[:first_definition]
for spelling in ['long long native_va_', 'double native_va_', 'struct NativeVaResult native_va_', 'int native_va_']:
...
for name, compiler in subjects:
    run([*compiler, '-c', 'tests/differential/native_variadic.c', '-o', out / (name + '.obj')])
