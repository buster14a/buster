...
No compaction or compaction without inserted parameters means four zero counters;
retained pre-existing parameters still receive the mandatory dense remap. For
one compacted function with B blocks, S sweeps and R removed parameters,
`block_visits == B*S`. With inserted parameters, `1 <= S <= R+1`; without
inserted parameters, `S == 0`.
