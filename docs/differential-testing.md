...
audit for source, workflow, runner, timing, sampled process-tree RSS and retained
evidence identities. Requested four reduced the sum of per-platform median
corpus wall times by 60.8% from requested one and by 27.8% from requested two;
this is a corpus-invocation result, not a measured complete-job or workflow
speedup.
...
These controls are not a full-corpus one/two/four-worker timing cohort. The
separate #408 qualification supplied that cohort before hosted policy changed;
complete native-job and workflow latency remain separate measurements.
