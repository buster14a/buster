...
queues a bounded fresh run from the new default-branch revision; no previous
snapshot or quartet is reused. Publication is one leased ref update, so a
failure or cancellation cannot leave a partially published generated state.
