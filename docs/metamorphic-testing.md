...
skip. Node is invoked with `--experimental-wasm-memory64`, including on engines
where Memory64 is not enabled by default. A version that rejects this option
fails explicitly.
...
repeat reduction. Each `failure-N` contains the original pair, artifacts, logs
with exact argument vectors, `reproducer.txt`, and a `minimized` directory.
...
A five-minute campaign budget (one minute for smoke), checked between work
...
issue #220 fix keeps placemarkers through all `##` operations and removes them
...
compile-only pair fatal. All these names have the `BUSTER_METAMORPHIC_` prefix.
