...
def finalize(args: argparse.Namespace) -> int:
    ...
    record_phase(
        root,
        {
            ...
            "observer_overhead_ns": 0,
            "message": "validated upload-artifact input",
        },
    )
    ...
    unsuccessful = sorted(phase["phase"] for phase in phases if phase["phase"] in required and phase["status"] != "success")
    if expect_complete and (missing or unsuccessful):
        raise ObservationError(
            "successful native payload has incomplete timing evidence: "
            + "; ".join(filter(None, [f"missing={','.join(missing)}" if missing else "", f"unsuccessful={','.join(unsuccessful)}" if unsuccessful else ""]))
        )
    ...
    output = artifact_directory / "native-observation.json"
    atomic_write(output, canonical_bytes(observation))
    print(
        f"NATIVE_OBSERVATION complete={str(complete).lower()} outcome={observation['correctness_outcome']} "
        f"phases={len(phases)} overhead_us={observer_overhead_ns / 1000:.3f} output={output}"
    )
    summary = os.environ.get("GITHUB_STEP_SUMMARY")
    if summary:
        with open(summary, "a", encoding="utf-8") as destination:
            destination.write("\n## Native phase observation\n\n")
            destination.write(f"Complete timing evidence: **{str(complete).lower()}**. Observer overhead: **{observer_overhead_ns / 1_000_000:.3f} ms**.\n\n")
            destination.write("| Phase | Status | Duration |\n|---|---:|---:|\n")
            for phase in phases:
                destination.write(f"| `{phase['phase']}` | {phase['status']} | {phase['elapsed_ns'] / 1_000_000:.3f} ms |\n")
    return 0
...
def phase_map(enriched: Mapping[str, Any]) -> dict[str, int]:
    ...
    for phase in observation.get("phases", []):
        if phase.get("status") == "success":
            result[str(phase["phase"])] = int(phase["elapsed_ns"])
    return result


def classify_observations(target: Mapping[str, Any], comparisons: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    if len(comparisons) < 2:
        raise ObservationError("classification requires at least two comparator observations")
    ...
    for index, comparison in enumerate(comparisons):
        ...
        if changed:
            raise ObservationError(f"comparison {index} identity mismatch: {', '.join(changed)}")
    ...
    if missing_required:
        raise ObservationError("matched phase set is incomplete: " + ", ".join(missing_required))
    rows = []
    slow: set[str] = set()
    for phase in sorted(common):
        baseline = statistics.median(phases[phase] for phases in comparison_phases)
        if baseline <= 0:
            raise ObservationError(f"non-positive comparator duration for {phase}")
    ...
    broad = len(slow) >= MINIMUM_BROAD_PHASES and bool(slow & SETUP_PHASES) and CORPUS_PHASE in slow
    return {
        "schema": CLASSIFICATION_SCHEMA,
        "classification": "hosted-runner-wide" if broad else "not-broad",
        "matched_observation_count": len(comparisons) + 1,
        "matched_identity_sha256": sha256_bytes(canonical_bytes(identity)),
        "threshold_ratio": SLOW_RATIO,
        "minimum_broad_phases": MINIMUM_BROAD_PHASES,
        "slow_phases": sorted(slow),
        "phase_comparisons": rows,
    }
...
def classify_command(args: argparse.Namespace) -> int:
    ...
    result = classify_observations(target, comparisons)
