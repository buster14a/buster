...
class MaterializerTests(unittest.TestCase):
    ...
    def test_self_test_and_receipt_are_stable(self):
        ...
        archived = materializer.materialize_file(repository / "docs/native-retirement-dependencies-v1.json",
                                                  repository, self.root / "archived")
        self.assertEqual(archived["archived_replay"]["projection"], {
        ...
        self.assertEqual(archived["archived_replay"]["row_identity_sha256"],
                         "9604102b75a14631aeb1d6a3652d36506a05928a0046c52cc50a00b942826ce6")
        self.assertEqual(len(archived["archived_replay"]["rows"]), 4032)
        descriptor = json.loads((repository / "docs/native-retirement-dependencies-v1.json").read_text(encoding="utf-8"))
        descriptor["archived_replay"]["projection"]["row_identity_sha256"] = "0" * 64
        with self.assertRaisesRegex(materializer.MaterializationError, "row identity digest"):
            materializer.materialize(descriptor, repository, self.root / "tampered-replay")
