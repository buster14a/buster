...
FULL_APPLICABILITY_LEDGER_COUNT = 500
FULL_APPLICABILITY_LEDGER_SHA256 = "20865fd2316c1df8c323bfa705a8ed01a52ccb473d2e6729ee79c780a81d575c"
...
def classify_applicability(row, result, outcome, baseline_unresolved=False, declared_supported_gap=False,
    ...
    elif authenticated_class in {"platform-inapplicable", "unavailable"}:
...
def validate(directory):
    ...
    for key, result in by_result.items():
        ...
        if authenticated_class in {"platform-inapplicable", "unavailable"}:
            # Keep the structured evidence and residual diagnostics intact,
            # but do not turn an authenticated non-applicable target into a
            # candidate/reference acceptance failure.  This branch is keyed
            # only by the checked-in fixture/target ledger above; disposition
            # text and producer prose never reach it.
            outcome["candidate_failure"] = False
            outcome["reference_failure"] = False
            outcome["acceptance_failure"] = False
            outcome["unexpected_failure"] = False
