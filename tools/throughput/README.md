...
| `simplify_passes`, `simplify_block_visits`, `simplify_empty_block_visits`, `simplify_parameter_visits`, `simplify_incoming_visits` | Executed sweeps and visited blocks/parameters/incoming rows, including re-evaluations and the initial active-block census. After the first sweep only notified parameters are evaluated; a sweep counts once it evaluates any. Empty-block visits are a subset of block visits. |
| `simplify_trivial_removals` | Parameters removed because their incoming roots collapsed to one value, each changing that root; forwarded and memory-form parameters unlinked without evaluation are excluded. |
| `simplify_survivors`, `simplify_classified`, `simplify_watch_links`, `simplify_notifications`, `simplify_agenda_words` | The trivial-parameter agenda's own cost: parameters kept by the first sweep (witnesses recorded), survivors classified after a first sweep that removed a trivial parameter, witness watches linked, watchers made due by a removal, and dirty-set words scanned. |
