...
class ContractTests(unittest.TestCase):
    ...
    def test_applicability_projection_retains_rows_and_is_not_disposition_owned(self):
        before = contract.validate(self.shards[0])
        ...
        self.assertEqual(report["rows_validated"], int(before["manifest"]["rows"]))
        self.assertEqual(report["rows_identity_sha256"], before["rows_identity_sha256"])
        self.assertIn(1, report["inapplicable_rows"])
        self.assertNotIn(1, report["candidate_failure_rows"])
        _fields, applicability_rows = read_table(self.root / "applicability.tsv")
        row_one = next(row for row in applicability_rows if row["row"] == "1")
        self.assertEqual(row_one["applicability"], "platform-inapplicable")
        self.assertTrue(self.validate()["clean_candidate"])

        # A producer-controlled disposition cannot turn the authenticated
        # projection back into an admitted cell.
        fields, results = read_table(self.shards[0] / "results.tsv")
        candidate = next(row for row in results if row["row"] == "1")
        candidate["disposition"] = "strict-success"
        write_table(self.shards[0] / "results.tsv", fields, results)
        forged = self.validate(require_clean=False)
        _fields, applicability_rows = read_table(self.root / "applicability.tsv")
        row_one = next(row for row in applicability_rows if row["row"] == "1")
        self.assertEqual(row_one["applicability"], "platform-inapplicable")
        self.assertNotIn(1, forged["candidate_failure_rows"])
