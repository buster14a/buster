...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        for (unsigned profile = 0; profile < 6; profile += 1)
        {
            for (unsigned test = 0; test < 16; test += 1)
            {
                for (unsigned storage = 0; storage < 2; storage += 1)
                {
                    ...
                    args[n++] = profile < 2 ? "clang" : profile < 4 ? "gcc" : argv[1];
                    if (profile >= 4) { args[n++] = "cc"; }
                    args[n++] = "-std=c17";
                    args[n++] = "-fwrapv"; args[n++] = "-fno-strict-aliasing";
                    args[n++] = "-funsigned-char"; args[n++] = "-g0";
                    args[n++] = profile < 4 && (profile & 1) ? "-O2" : "-O0";
                    if (profile < 2) { args[n++] = "--target=x86_64-linux-gnu"; }
                    else if (profile < 4) { args[n++] = "-m64"; }
                    else
                    {
                        args[n++] = "-target"; args[n++] = "x86_64-linux";
                        args[n++] = "-fregister-allocator=fast";
                        args[n++] = "-fverify-codegen"; args[n++] = "-fno-machine-fallback";
                        args[n++] = profile == 4 ? "-ffrontend-ssa" : "-fno-frontend-ssa";
                    }
