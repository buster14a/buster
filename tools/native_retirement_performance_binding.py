...
SUPPORT_DECLARATION_SHA256 = "24ae23cff2ab5bd6ff46304a878ed4dc3e5d639422751de93ea74dabde7e437e"
...
def _check_execution_evidence(root, binding):
    ...
    admission = _keys(admission, (
        ...
        "lease_protocol", "observations", "observation_count", "policy_sha256"),
    ...
    _artifact(admission["observations"], "aa_admission_receipt.observations")
    _check_evidence(root, admission["observations"], "aa_admission_receipt.observations")
    _positive_int(admission["observation_count"], "aa_admission_receipt.observation_count")
    _sha(admission["policy_sha256"], "aa_admission_receipt.policy_sha256")
    expected_policy = {name: float(AGGREGATE_THRESHOLDS[name]) for name in METRICS}
    if admission["policy_sha256"] != _canonical_json_digest(expected_policy):
        _fail("A/A admission policy differs from the approved aggregate guards")
    performance_rows = _support_file(binding["support"], "performance_rows")
    parsed, _axes, _family = _performance_rows(
        _evidence_bytes(root, performance_rows, "support.files.performance_rows"))
    row_by_id = {item["row"]: item for item in parsed}
    rounds = binding["rules"]["sampling"]["rounds"]
    pairs = binding["rules"]["sampling"]["pairs_per_round"]
    expected_count = len(parsed) * rounds * pairs
    if admission["observation_count"] != expected_count:
        _fail("A/A observations do not cover the complete frozen population")
    observations_path = Path(root).resolve() / PurePosixPath(admission["observations"]["path"])
    seen = 0
    try:
        observations_stream = observations_path.open(encoding="utf-8")
    except OSError as error:
        _fail(f"A/A observations are unreadable: {error}")
    with observations_stream:
        for line in observations_stream:
            try:
                item = json.loads(line, object_pairs_hook=_json_object)
            except (UnicodeDecodeError, json.JSONDecodeError) as error:
                _fail(f"A/A observation is invalid JSON: {error}")
            item = _keys(item, ("record_id", "row", "round", "pair", "measurements"),
                         f"aa_observations[{seen}]")
            row_number = seen // (rounds * pairs)
            remainder = seen % (rounds * pairs)
            round_number = remainder // pairs
            pair_number = remainder % pairs
            if (item["row"], item["round"], item["pair"]) != (row_number, round_number, pair_number) \
                    or item["record_id"] != f"aa-row-{row_number}/round-{round_number}/pair-{pair_number}":
                _fail("A/A observations are not in canonical row/round/pair order")
            if row_number not in row_by_id:
                _fail("A/A observation names an unknown performance row")
            expected_metrics = {name for name in METRICS if row_by_id[row_number]["metrics"].get(name, False)}
            measurements = _keys(item["measurements"], expected_metrics,
                                  f"aa_observations[{seen}].measurements")
            for metric in expected_metrics:
                pair_value = _keys(measurements[metric], ("baseline", "candidate"),
                                   f"aa_observations[{seen}].measurements.{metric}")
                baseline_value = pair_value["baseline"]
                candidate_value = pair_value["candidate"]
                if type(baseline_value) not in (int, float) or isinstance(baseline_value, bool) \
                        or type(candidate_value) not in (int, float) or isinstance(candidate_value, bool) \
                        or not math.isfinite(float(baseline_value)) or not math.isfinite(float(candidate_value)) \
                        or baseline_value <= 0 or candidate_value <= 0:
                    _fail("A/A observations must be finite positive numbers")
                ratio = float(candidate_value) / float(baseline_value)
                limit = float(AGGREGATE_THRESHOLDS[metric])
                if not 1.0 / limit <= ratio <= limit:
                    _fail("A/A observation exceeds the admitted symmetric guard")
            seen += 1
    if seen != expected_count:
        _fail("A/A observations ended before the frozen population")
