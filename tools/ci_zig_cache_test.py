...
class ZigCacheWorkflowTests(unittest.TestCase):
    def test_workflow_exposes_only_bounded_exact_cache_controls(self):
        text = (ROOT / ".github/workflows/ci.yml").read_text(encoding="utf-8")
        self.assertIn("zig_cache_mode:", text)
        self.assertIn("type: choice", text)
        self.assertIn("- ordinary", text)
        self.assertIn("- prime", text)
        self.assertIn("- read", text)
        self.assertIn("zig_cache_namespace:", text)
        self.assertIn("tools/ci_zig_cache.py resolve", text)
        ...
        cache_paths = [
            line.strip() for line in cache_section.splitlines()
    ...
    def test_no_second_workflow_owns_the_cohort_interface(self):
        ...
        for path in (ROOT / ".github/workflows").glob("*.yml"):
            if "zig_cache_mode:" in path.read_text(encoding="utf-8"):
