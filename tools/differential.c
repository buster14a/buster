...
typedef enum DKind { D_EXIT, D_SIGNAL, D_TIMEOUT, D_SPAWN } DKind;
...
BUSTER_GLOBAL_LOCAL DObservation d_observe(DSettings* settings, SliceString8 command, String8 prefix)
{
    ...
    if (spawn.handle)
    {
        ...
        observation.raw_status = wait.platform_status;
        ...
        else
        {
            ...
            observation.kind = WIFEXITED(status) ? D_EXIT : D_SIGNAL;
            observation.status = (u32)(WIFEXITED(status) ? WEXITSTATUS(status) : WTERMSIG(status));
#endif
        }
...
BUSTER_GLOBAL_LOCAL bool d_verification(DSettings* settings, DObservation* observation, DConfig config)
{
    ...
    while (offset < text.length)
    {
        ...
        {
            matches += 1;
            String8 terminated = string_duplicate_arena(settings->arena, line, true);
            unsigned version = 0, ir = 0, mir = 0, scheduled = 0;
            char mode[32] = {0};
            int consumed = 0;
            int fields = sscanf((char*)terminated.pointer, "CODEGEN_VERIFY version=%u ir=%u mir=%u scheduled=%u allocator=%31s%n",
                                &version, &ir, &mir, &scheduled, mode, &consumed);
            valid = fields == 5 && consumed == (int)line.length && version == 1 && ir > 0 &&
                    string_equal(string_from_pointer((char8*)mode), expected);
            // Fallback-only functions may legitimately produce zero selected MIR.
            (void)mir;
            (void)scheduled;
            u64 after = end < text.length ? end + 1 : end;
            char8* normalized = arena_allocate(settings->arena, char8, text.length);
            if (offset) { memcpy(normalized, text.pointer, (size_t)offset); }
            if (text.length > after) { memcpy(normalized + offset, text.pointer + after, (size_t)(text.length - after)); }
            observation->output = (String8){.pointer = normalized, .length = offset + text.length - after};
        }
...
BUSTER_GLOBAL_LOCAL u32 d_matrix(DConfig* configs, u32 capacity)
{
    ...
    for (u32 allocator = 0; allocator < BUSTER_ARRAY_LENGTH(d_allocators); allocator += 1)
    {
        ...
        {
            for (u32 promotion = 0; promotion < 4; promotion += 1)
            {
...
BUSTER_GLOBAL_LOCAL void d_reduce(DSettings* settings, DCase test, DConfig config, u32 signature, String8 directory)
{
    ...
    while (!exhausted && trials < settings->reduce_limit && best.length)
    {
        ...
        for (u64 line = 0; !changed && line < lines && trials < settings->reduce_limit; line += chunk)
        {
            ...
            if (o0.ran && o2.ran && d_normal(o0.run) && d_normal(o2.run) && d_difference(o0.run, o2.run) == 0)
            {
    ...
    bool confirmed = o0.ran && o2.ran && d_normal(o0.run) && d_normal(o2.run) &&
                     !d_difference(o0.run, o2.run) && d_classify(actual, o0, false) == signature;
...
BUSTER_GLOBAL_LOCAL u32 d_self_test(Arena* arena)
{
    ...
    errors += count != BUSTER_ARRAY_LENGTH(d_allocators) * BUSTER_ARRAY_LENGTH(d_optimizations) * 4;
    ...
    else
    {
        ...
        if (!list)
        {
            ...
            else
            {
                ...
                {
                    fprintf(settings.report, "prefix\tkind_0exit_1signal_2timeout_3spawn\tstatus\traw_status\tsanitizer\telapsed_us\n");
                    String8 manifest = string_format(arena, S8("version=1\nide={S8}\ncc={S8}\nconfigurations={u32}\nseed={u32}\ngenerated={u32}\nverify={u32}\nsanitize_oracle={u32}\n"),
                        settings.ide, settings.cc, count, seed, generated, (u32)settings.verify, (u32)settings.sanitize_oracle);
