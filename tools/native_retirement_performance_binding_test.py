...
class BindingTests(unittest.TestCase):
    ...
    def test_operator_dispatch_is_protected_and_checkout_free(self):
        workflow = (ROOT / ".github" / "workflows" /
                    "native-retirement-performance.yml").read_text(encoding="utf-8")
        self.assertIn("workflow_dispatch:", workflow)
        self.assertNotIn("pull_request", workflow)
        self.assertNotIn("push:", workflow)
        self.assertIn("github.repository == 'buster14a/buster'", workflow)
        self.assertIn("github.ref == 'refs/heads/main'", workflow)
        self.assertIn("github.ref_protected == true", workflow)
        self.assertIn("environment: native-retirement-performance", workflow)
        self.assertIn(
            "runs-on: [self-hosted, Linux, X64, buster-zen5, ryzen-9700x]",
            workflow)
        self.assertNotIn("actions/checkout", workflow)
        self.assertNotIn("github.workspace", workflow.lower())
        self.assertIn("exec /usr/local/libexec/buster-bench-service", workflow)
        self.assertIn("submit-remote", workflow)
        self.assertIn("validate-buster-v1", workflow)
        self.assertIn("BASELINE_COMMIT: ${{ inputs.baseline_commit }}", workflow)
        self.assertIn("CANDIDATE_COMMIT: ${{ inputs.candidate_commit }}", workflow)
    ...
    def _build_record(cls, full=False):
        ...
        aa_observation = {
            "record_id": "aa-row-0/round-0/pair-0", "row": 0, "round": 0, "pair": 0,
            "measurements": {
        ...
        aa_observation_data = (json.dumps(aa_observation, sort_keys=True, separators=(",", ":")) + "\n").encode("utf-8")
        aa_observations = artifact("execution/aa-observations.jsonl", aa_observation_data)
        aa_admission = json_artifact("execution/aa-admission.json", {
            ...
            "observations": aa_observations, "observation_count": 1,
            "policy_sha256": binding._canonical_json_digest(
                {name: float(binding.AGGREGATE_THRESHOLDS[name]) for name in binding.METRICS}),
        })
    ...
    def _build_small_evidence_fixture(self):
        ...
        for round_number in range(2):
            for pair in range(60):
                aa_lines.append(json_data({
                    "record_id": f"aa-row-0/round-{round_number}/pair-{pair}",
        ...
        aa_observation_descriptor = put("execution/aa-observations.jsonl", b"".join(aa_lines))
        aa_descriptor = record["execution"]["host"]["aa_admission_receipt"]
        aa_value = json.loads(contents[aa_descriptor["path"]].decode())
        aa_value.update({"observations": aa_observation_descriptor, "observation_count": 120})
