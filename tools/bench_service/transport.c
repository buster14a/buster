...
{
    bool valid = request && response && bq_transport_response_valid(request, response, response->size);
    u32 operation = valid ? bq_u32(request->bytes + 8) : 0;
    u32 body_size = valid ? response->size - BQ_CONTROL_HEADER : 0;
    u8 const* body = valid ? response->bytes + BQ_CONTROL_HEADER : NULL;
    BqError error = valid ? (BqError)bq_u32(body) : BQ_BAD_REQUEST;
    if (valid && error != BQ_OK)
    {
        valid = body_size == 124;
    }
    else if (valid && operation == BQ_OP_CAPABILITIES)
    {
        u32 capabilities_size = (u32)sizeof(bq_capabilities_v2) - 1;
        valid = body_size == 4 + capabilities_size && !memcmp(body + 4, bq_capabilities_v2, capabilities_size);
    }
    else if (valid && operation == BQ_OP_LOGS)
    {
        valid = body_size >= 20;
        u32 count = valid ? bq_u32(body + 4) : 0;
        valid = valid && count <= BQ_LOG_PAGE && body_size == 20 + count * 32;
        u64 requested_job = valid ? bq_u64(request->bytes + BQ_CONTROL_HEADER) : 0;
        for (u32 index = 0; valid && index < count; index += 1)
            valid = bq_u64(body + 20 + index * 32 + 8) == requested_job;
    }
    else if (valid)
    {
        valid = operation == BQ_OP_SUBMIT || operation == BQ_OP_STATUS ||
                operation == BQ_OP_RESULT || operation == BQ_OP_CANCEL;
        bool result_body = valid && body_size == BQ_CONTROL_BODY;
        valid = valid && !(((operation == BQ_OP_SUBMIT || operation == BQ_OP_CANCEL) && body_size != 124) ||
                           (operation == BQ_OP_STATUS && body_size != 124 && !result_body) ||
                           (operation == BQ_OP_RESULT && !result_body));
        if (valid && (operation == BQ_OP_STATUS || operation == BQ_OP_RESULT || operation == BQ_OP_CANCEL))
            valid = bq_u64(body + 4) == bq_u64(request->bytes + BQ_CONTROL_HEADER);
        if (valid && operation == BQ_OP_SUBMIT)
        {
            ...
            valid = bq_u64(body + 4) && !memcmp(body + 56, request_digest, SHA256_HEX_CAPACITY - 1);
        }
        if (valid && result_body)
        {
            u32 path_length = bq_u32(body + 124);
            valid = bq_result_path_valid(body + 128, path_length) &&
