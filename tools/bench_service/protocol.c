...
BUSTER_GLOBAL_LOCAL char const bq_capabilities_v2[] =
    ...
    "profile=retirement-v1 validity=sealed materialization=installed-read-only workspace=per-attempt\n"
    "worker=fixed-systemd recipe=fixed-retirement-v1 validation=operator-required "
#ifdef __linux__
        ...
        else if (schema == BQ_CONTROL_SCHEMA && operation == BQ_OP_WORKER_RUN && length >= 16)
        {
                ...
                installed_length + workspace_length + lease_length == length - 16)
            {
                BqWorkerConfig config = {
                    ...
                    .limits = {cpu, 8ull * 1024 * 1024 * 1024, 0, 256, 5ull * 24 * 60 * 60 * 1000000},
