...
bounded public control frames. The fixed installed helper executes the reviewed
native-retirement experiment only when its operator-installed dependencies are
available. This repository change does not install those dependencies, change
server configuration, measure performance or qualify a 9700X.
...
`/usr/local/libexec/buster-native-retirement-performance` harness. The installed
file must be byte-identical to
`tools/bench_service/native_retirement_performance.py`; the recipe checks its
compiled SHA-256 before and after execution. The harness opens the fixed
`/usr/local/share/buster-bench/native-retirement-performance-v1/definition.json`
bundle, whose top-level SHA-256 is also compiled into the deployed build driver.
That authenticated definition owns the frozen #508 population, per-row argv,
source/build/toolchain identities, host qualification and lease receipts, and
publication identity. The harness owns the `native-retirement-performance-v1`
A/A admission, paired A/B, sealed-result and independent-replay implementation.
No request field supplies a command, flag, executable, sampling choice,
definition identity or additional path.
...
measurement starts. Baseline generation/build runs explicitly as the trusted
`buster-bench` identity. Candidate generation/build runs explicitly as the
separate `buster-bench-candidate` identity in `candidate/staging`; it cannot
write the final candidate build, queue, lease, installed tree, or final result
surface. The operator-installed measurement harness runs as the trusted service
identity, receives only fixed policy and immutable source/binary identities,
and writes only the staging evidence directory. After a
...
before measurement starts. After the harness exits, the trusted driver copies that
...
foundation linkage used by the throughput tool. The dedicated performance
harness remains an operator-installed dependency and is never run by these
repository tests.
...
operator-installed compiler or native-retirement harness. Tests use private,
...
locks built baseline/candidate trees before measurement. The recipe accepts only
...
after their builds with the digests after measurement. Any mismatch fails closed
...
The recipe has no smoke or `--no-guard` path. It invokes only
`native-retirement-performance-v1` with two rounds, the admitted even 60--254
pair range, two warmups, mandatory A/A and paired A/B, the immutable 16,777,216
record partition cap, at most three partitions and the 39,518,208-record / 16
GiB experiment ceilings. The harness freezes the partition plan before A/A,
runs same-binary A/A before any candidate sample, emits A/B observations in
canonical row/round/pair order, invokes the reviewed C statistics adapter,
seals a distinct replay archive, and runs the authenticated binding validator.
Only then does it write the verdict. A zero exit without the validator receipt,
independent-replay binding, complete binding record, and verdict is rejected by
the build driver before publication. The service receipt remains only an
execution record; the bound validator and independent replay decide acceptance.

The manual `native-retirement-performance.yml` dispatcher is an enqueue-only
operator boundary. It runs only for a protected `main` ref in the canonical
repository, requires the `native-retirement-performance` environment and the
five dedicated 9700X runner labels, performs no checkout, and invokes only the
operator-installed typed `submit-remote` client. The baseline and candidate
inputs remain data: both the client and server require complete immutable
lowercase commit identities. The environment approval policy, installed client,
private socket and service deployment are operator prerequisites; this workflow
does not provision or weaken any of them.
...
Still outside this slice: live installation and qualification of the fixed
performance entrypoint; live-systemd/polkit/deployment qualification; transport
credentials beyond the local peer UID/GID boundary; retention migration; and a
real physical-9700X A/A plus A/B result. The repository contains the fixed
recipe contract, durable whole-result bundle and failure/cancellation evidence
with injected replay tests; these are not claims about a deployed host or a
measured result.
