...
BUSTER_LOCAL File file_from_path(Arena* arena, CompilerFileReadOptions options)
{
    ...
    let absolute_path = arena_join_string(arena, BUSTER_STRING_ARRAY_TO_SLICE(parts), true);
...
BUSTER_LOCAL bool compile(Arena* arena, String relative_path, CompilerOptions options)
{
...
BUSTER_IMPL ProcessResult thread_entry_point()
{
    ...
    cc_program_state.cwd = path_absolute(arena, ".");
    BUSTER_UNUSED(thread);
    printf("Hello world from the compiler\n");
    let basic = S("test/cc/basic.c");
