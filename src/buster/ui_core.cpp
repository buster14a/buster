...
BUSTER_F_IMPL void ui_autopop(UI_State* state)
{
...
BUSTER_F_IMPL void ui_state_select(UI_State* state)
{
    ui_state = state;
}

BUSTER_F_IMPL UI_State* ui_state_get(void)
{
    return ui_state;
}

BUSTER_F_IMPL Arena* ui_build_arena(void)
{
    let arena = ui_state->build_arenas[ui_state->build_count % BUSTER_ARRAY_LENGTH(ui_state->build_arenas)];
    return arena;
}

BUSTER_F_IMPL UI_Key ui_key_null(void)
{
    UI_Key key = {};
    return key;
}

BUSTER_F_IMPL UI_State* ui_state_allocate(RenderingHandle* rendering, RenderingWindowHandle* window)
{
...
BUSTER_F_IMPL void ui_state_deinitialize(UI_State* state)
{
    if (state)
    {
        for (u64 i = 0; i < BUSTER_ARRAY_LENGTH(state->build_arenas); i += 1)
...
BUSTER_F_IMPL u64 ui_widget_index_from_key(UI_Key key)
{
...
BUSTER_F_IMPL String8 ui_text_from_key_string(String8 string)
{
...
BUSTER_F_IMPL String8 ui_hash_from_key_string(String8 string)
{
    String8 result = string;
...
BUSTER_F_IMPL UI_Key ui_key_from_string(UI_Key seed, String8 string)
{
...
BUSTER_F_IMPL UI_Key ui_key_from_string_format(UI_Key seed, char* format, ...)
{
    let scratch = scratch_begin(0, 0);
    va_list args;
    va_start(args, format);
    let string = string8_format_va(scratch.arena, string8_from_pointer((char8*)format), args);
    va_end(args);
    let result = ui_key_from_string(seed, string);
    return result;
}

BUSTER_F_IMPL u8 ui_key_equal(UI_Key a, UI_Key b)
{
    return a.value == b.value;
}

BUSTER_F_IMPL UI_Widget* ui_widget_from_key(UI_Key key)
{
    UI_Widget* result = 0;
...
BUSTER_F_IMPL UI_Widget* ui_widget_make_from_key(UI_WidgetFlags flags, UI_Key key)
{
    ...
    widget->pref_size[(u64)Axis2::X] = ui_top(pref_width);
    widget->pref_size[(u64)Axis2::Y] = ui_top(pref_height);
...
BUSTER_F_IMPL UI_Widget* ui_widget_make(UI_WidgetFlags flags, String8 string)
{
...
BUSTER_F_IMPL UI_Widget* ui_widget_make_format(UI_WidgetFlags flags, String8 format, ...)
{
    let arena = ui_build_arena();
    va_list args;
    va_start(args, format);
    let string = string8_format_va(arena, format, args);
    va_end(args);
...
BUSTER_F_IMPL UI_Signal ui_signal_from_widget(UI_Widget* widget)
{
    let rect = widget->rect;
    let mouse_position = ui_state->mouse_position;
    UI_Signal signal = {
        .clicked_left =
            (u32)((widget->flags.mouse_clickable & (ui_state->mouse_button_events[(u64)WindowingEventMouseButtonKind::WINDOWING_EVENT_MOUSE_LEFT].action == WindowingEventMouseButtonAction::WINDOWING_EVENT_MOUSE_RELEASE)) != 0) &
...
BUSTER_F_IMPL void ui_stack_reset(UI_State* state)
{
...
BUSTER_F_IMPL UI_Size ui_pixels(u32 width, f32 strictness)
{
    return (UI_Size) {
        .kind = UI_SizeKind::UI_SIZE_PIXEL_COUNT,
        .strictness = strictness,
        .value = (f32)width,
    };
}

BUSTER_F_IMPL UI_Size ui_percentage(f32 percentage, f32 strictness)
{
    return (UI_Size) {
        .kind = UI_SizeKind::UI_SIZE_PERCENTAGE,
        .strictness = strictness,
        .value = percentage,
    };
}

BUSTER_F_IMPL UI_Size ui_em(f32 value, f32 strictness)
{
...
BUSTER_F_IMPL u8 ui_build_begin(OsWindowingHandle* windowing, OsWindowHandle* window, f64 frame_time, OsWindowingEventList* event_queue)
{
    ...
    if (open)
    {
        ...
        ui_push_next_only(child_layout_axis, Axis2::Y);
...
BUSTER_F_IMPL void ui_compute_independent_sizes(UI_Widget* widget)
{
    for (EACH_ENUM_INT(Axis2,axis))
    {
        let pref_size = widget->pref_size[axis];
        switch (pref_size.kind)
        {
            default: break;
            case UI_SizeKind::Count: BUSTER_UNREACHABLE(); break;
            case UI_SizeKind::UI_SIZE_PIXEL_COUNT: widget->computed_size[axis] = floor_f32(widget->pref_size[axis].value); break;
        }
    }
...
BUSTER_F_IMPL void ui_compute_upward_dependent_sizes(UI_Widget* widget)
{
    for (EACH_ENUM_INT(Axis2,axis))
    {
        let pref_size = widget->pref_size[axis];
        switch (pref_size.kind)
        {
            default: break;
            case UI_SizeKind::Count: BUSTER_UNREACHABLE(); break;
            case UI_SizeKind::UI_SIZE_PERCENTAGE:
            {
                for (UI_Widget* ancestor = widget->parent; ancestor; ancestor = ancestor->parent)
                {
                    if (ancestor->pref_size[axis].kind != UI_SizeKind::UI_SIZE_BY_CHILDREN)
                    {
                        widget->computed_size[axis] = floor_f32(ancestor->computed_size[axis] * widget->pref_size[axis].value);
                        break;
                    }
                }
            } break;
...
BUSTER_F_IMPL void ui_compute_downward_dependent_sizes(UI_Widget* widget)
{
    for (UI_Widget* child_widget = widget->first; child_widget; child_widget = child_widget->next)
    {
    ...
    for (EACH_ENUM_INT(Axis2,axis))
    {
        ...
        switch (pref_size.kind)
        {
            default: break;
            case UI_SizeKind::Count: BUSTER_UNREACHABLE(); break;
            case UI_SizeKind::UI_SIZE_BY_CHILDREN:
            {
                // TODO: implement
                BUSTER_TRAP();
            } break;
...
BUSTER_F_IMPL void ui_resolve_conflicts(UI_Widget* widget)
{
    for (EACH_ENUM_INT(Axis2,axis))
    {
        ...
        if (!(widget->flags.v & ((u64)UI_WidgetFlagEnum::UI_WIDGET_FLAG_OVERFLOW_X << axis)))
        {
            for (UI_Widget* child_widget = widget->first; child_widget; child_widget = child_widget->next)
            {
                if (!(child_widget->flags.v & ((u64)UI_WidgetFlagEnum::UI_WIDGET_FLAG_FLOATING_X << axis)))
                {
            ...
            if (conflict > 0 && total_fixup_budget > 0)
            {
                for (UI_Widget* child_widget = widget->first; child_widget; child_widget = child_widget->next)
                {
                    if (!(child_widget->flags.v & ((u64)UI_WidgetFlagEnum::UI_WIDGET_FLAG_FLOATING_X << axis)))
                    {
                        ...
                        child_widget->computed_size[axis] = floor_f32(child_widget->computed_size[axis] - fixup_size_this_child);
                    }
                }
            }
        }
        ...
        for (UI_Widget* child_widget = widget->first; child_widget; child_widget = child_widget->next)
        {
            ...
            child_widget->relative_corner_delta[(u64)Corner::CORNER_00][axis] = child_widget->relative_rect.p0[axis] - last_relative_rect.p0[axis];
            child_widget->relative_corner_delta[(u64)Corner::CORNER_01][axis] = this_corner_01[axis] - last_corner_01[axis];
            child_widget->relative_corner_delta[(u64)Corner::CORNER_10][axis] = this_corner_10[axis] - last_corner_10[axis];
            child_widget->relative_corner_delta[(u64)Corner::CORNER_11][axis] = child_widget->relative_rect.p1[axis] - last_relative_rect.p1[axis];
            ...
            if (!(child_widget->flags.v & ((u64)UI_WidgetFlagEnum::UI_WIDGET_FLAG_FLOATING_X << axis)))
            {
                child_widget->rect.p0[axis] = floor_f32(child_widget->rect.p0[axis]);
                child_widget->rect.p1[axis] = floor_f32(child_widget->rect.p1[axis]);
            }
        }
...
BUSTER_F_IMPL void ui_build_end(void)
{
    ...
    for (u32 i = 0; i < BUSTER_ARRAY_LENGTH(ui_state->mouse_button_events); i += 1)
    {
        ...
        if (event->action == WindowingEventMouseButtonAction::WINDOWING_EVENT_MOUSE_RELEASE)
        {
            event->action = WindowingEventMouseButtonAction::WINDOWING_EVENT_MOUSE_RELAX;
        }
    }
...
BUSTER_F_IMPL WidgetIterator ui_widget_recurse_depth_first(UI_Widget* widget, u64 sibling_offset, u64 child_offset)
{
    WidgetIterator it = {};
...
BUSTER_F_IMPL void ui_draw(void)
{
    ...
    while (widget)
    {
        ...
        {
            rendering_window_render_text(rendering, window, widget->text, widget->text_color, RenderFontType::RENDER_FONT_TYPE_MONOSPACE, widget->rect.x0, widget->rect.y0);
        }
