...
BUSTER_F_IMPL StringOs font_file_get_path(Arena* arena, FontIndex index)
{
    BUSTER_GLOBAL_LOCAL StringOs table[(u64)FontIndex::Count] = {};
        ...
        if (FcInit())
        {
            ...
            if (pat)
            {
                ...
                if (match) {
                    ...
                    {
                        table[(u64)FontIndex::FONT_INDEX_MONO] = string8_duplicate_arena(arena, string8_from_pointer((char*)file), true);
                    }
    ...
    static_assert(BUSTER_ARRAY_LENGTH(table) == (u64)FontIndex::Count);
//     StringOs mono_path =
// #if defined(_WIN32)
...
BUSTER_F_IMPL FontTextureAtlasDescription font_texture_atlas_create(Arena* arena, FontTextureAtlasCreate create)
{
