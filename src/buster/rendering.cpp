...
BUSTER_GLOBAL_LOCAL VkDescriptorType vulkan_descriptor_type(DescriptorType type)
{
    ...
    switch (type)
    {
        case DescriptorType::DESCRIPTOR_TYPE_IMAGE_PLUS_SAMPLER:
            result = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            break;
        case DescriptorType::Count: BUSTER_UNREACHABLE();
    }
...
BUSTER_GLOBAL_LOCAL VkShaderStageFlags vulkan_shader_stage(ShaderStage shader_stage)
{
    ...
    switch (shader_stage)
    {
        break; case ShaderStage::SHADER_STAGE_VERTEX: result = VK_SHADER_STAGE_VERTEX_BIT;
        break; case ShaderStage::SHADER_STAGE_FRAGMENT: result = VK_SHADER_STAGE_FRAGMENT_BIT;
        break; case ShaderStage::Count: BUSTER_UNREACHABLE();
    }
...
STRUCT(WindowFrame)
{
    ...
    FramePipelineInstantiation pipeline_instantiations[(u64)BusterPipeline::Count];
...
BUSTER_GLOBAL_LOCAL DescriptorType descriptor_type_from_vulkan(VkDescriptorType descriptor_type)
{
    ...
    {
        break; case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER: result = DescriptorType::DESCRIPTOR_TYPE_IMAGE_PLUS_SAMPLER;
...
STRUCT(RenderingHandle)
{
    ...
    u32 texture_count;
    FontTextureAtlas fonts[(u64)RenderFontType::Count];
    const VkAllocationCallbacks* allocator;
    VkPhysicalDeviceMemoryProperties device_memory_properties;
    VkSampler sampler;
    ImmediateContext immediate;
    Pipeline pipelines[(u64)BusterPipeline::Count];
...
STRUCT(RenderingWindowHandle)
{
    ...
    PipelineInstantiation pipeline_instantiations[(u64)BusterPipeline::Count];
...
{
    let pipeline_instantiation = &window->pipeline_instantiations[(u64)pipeline_index];
...
BUSTER_F_IMPL void rendering_window_texture_update_end(RenderingHandle* rendering, RenderingWindowHandle* window, BusterPipeline pipeline_index)
{
    let pipeline_instantiation = &window->pipeline_instantiations[(u64)pipeline_index];
...
BUSTER_F_IMPL void rendering_window_rect_texture_update_begin(RenderingWindowHandle* window)
{
    rendering_window_texture_update_begin(window, BusterPipeline::BUSTER_PIPELINE_RECT, (u32)RectTextureSlot::Count);
}

BUSTER_F_IMPL void window_rect_texture_update_end(RenderingHandle* rendering, RenderingWindowHandle* window)
{
    rendering_window_texture_update_end(rendering, window, BusterPipeline::BUSTER_PIPELINE_RECT);
}

BUSTER_F_IMPL __attribute__((noinline)) RenderingHandle* rendering_initialize(Arena* arena)
{
            ...
            if (vkEnumerateInstanceVersion)
            {
                ...
                if (result == VK_SUCCESS && api_version >= VK_API_VERSION_1_3)
                {
                                ...
                                if (result == VK_SUCCESS)
                                {
                                    for (u32 layer_i = 0; layer_i < layer_count && supported_instance_layer_count < BUSTER_ARRAY_LENGTH(instance_layer_names); layer_i += 1)
                                    {
                                        ...
                                        for (u64 requested_i = 0; requested_i < BUSTER_ARRAY_LENGTH(instance_layer_names); requested_i += 1)
                                        {
                                            ...
                                            if (string8_equal(layer_name, requested_name))
                                            {
                    ...
                    DescriptorSetLayoutBinding rect_descriptor_set_layout_bindings[] = {
                        {
                            .binding = 0,
                            .type = DescriptorType::DESCRIPTOR_TYPE_IMAGE_PLUS_SAMPLER,
                            .stage = ShaderStage::SHADER_STAGE_FRAGMENT,
                            .count = (u8)RectTextureSlot::Count,
                        },
                    };
                    ...
                    VkPipeline pipeline_handles[(u64)BusterPipeline::Count];
                    VkPipelineShaderStageCreateInfo shader_create_infos[MAX_SHADER_MODULE_COUNT_PER_PIPELINE];
                    VkGraphicsPipelineCreateInfo graphics_pipeline_create_infos[(u64)BusterPipeline::Count];
...
BUSTER_F_IMPL RenderingWindowHandle* rendering_window_initialize(Arena* arena, OsWindowingHandle* windowing, RenderingHandle* rendering, OsWindowHandle* window)
{
    ...
    for (u64 frame_index = 0; frame_index < result->frame_count; frame_index += 1)
    {
        ...
        for (EACH_ENUM_INT(BusterPipeline, pipeline_index))
        {
            ...
            pipeline->vertex_buffer.gpu.type = BufferType::BUFFER_TYPE_VERTEX;
            pipeline->index_buffer.gpu.type = BufferType::BUFFER_TYPE_INDEX;
            pipeline->transient_buffer.type = BufferType::BUFFER_TYPE_STAGING;
        }
    }
    ...
    for (EACH_ENUM_INT(BusterPipeline, pipeline_index))
    {
        ...
        u16 descriptor_type_counter[(u64)DescriptorType::Count] = {};
        ...
        for (u64 descriptor_index = 0; descriptor_index < pipeline_descriptor->descriptor_set_count; descriptor_index += 1)
        {
            ...
            for (u64 binding_index = 0; binding_index < descriptor_set_layout_bindings->count; binding_index += 1)
            {
                ...
                let counter_ptr = &descriptor_type_counter[(u64)descriptor_type];
                let old_counter = *counter_ptr;
                *counter_ptr = old_counter + (u16)binding_descriptor->descriptorCount;
            }
        }

        VkDescriptorPoolSize pool_sizes[(u64)DescriptorType::Count];
        u32 pool_size_count = 0;

        for (EACH_ENUM_INT(DescriptorType, i))
        {
    ...
    for (u32 frame_i = 0; frame_i < result->frame_count; frame_i += 1)
    {
        ...
        if (vkCreateCommandPool(rendering->device, &command_pool_create_info, rendering->allocator, &frame->command_pool) == VK_SUCCESS)
        {
            ...
            frame->bound_pipeline = BusterPipeline::Count;
        }
    }
...
BUSTER_F_IMPL void rendering_window_queue_pipeline_texture_update(RenderingHandle* rendering, RenderingWindowHandle* window, BusterPipeline pipeline_index, u32 resource_slot, TextureIndex texture_index)
{
    let pipeline_instantiation = &window->pipeline_instantiations[(u64)pipeline_index];
...
BUSTER_F_IMPL void rendering_window_queue_rect_texture_update(RenderingHandle* rendering, RenderingWindowHandle* window, RectTextureSlot slot, TextureIndex texture_index)
{
    rendering_window_queue_pipeline_texture_update(rendering, window, BusterPipeline::BUSTER_PIPELINE_RECT, (u32)slot, texture_index);
}

BUSTER_F_IMPL void rendering_queue_font_update(RenderingHandle* rendering, RenderingWindowHandle* window, RenderFontType type, FontTextureAtlas atlas)
{
    // static_assert(RECT_TEXTURE_SLOT_MONOSPACE_FONT < RECT_TEXTURE_SLOT_PROPORTIONAL_FONT);
    let slot = (RectTextureSlot)((u32)RectTextureSlot::RECT_TEXTURE_SLOT_MONOSPACE_FONT + (u32)type);
    rendering_window_queue_rect_texture_update(rendering, window, slot, atlas.texture);
    rendering->fonts[(u32)type] = atlas;
}

BUSTER_F_IMPL void rendering_window_rect_texture_update_end(RenderingHandle* rendering, RenderingWindowHandle* window)
{
    rendering_window_texture_update_end(rendering, window, BusterPipeline::BUSTER_PIPELINE_RECT);
}
...
BUSTER_GLOBAL_LOCAL VkFormat vk_texture_format(TextureFormat format)
{
    ...
    switch (format)
    {
        break; case TextureFormat::TEXTURE_FORMAT_R8_UNORM: result = VK_FORMAT_R8_UNORM;
        break; case TextureFormat::TEXTURE_FORMAT_R8G8B8A8_SRGB: result = VK_FORMAT_R8G8B8A8_SRGB;
        break; case TextureFormat::Count: BUSTER_UNREACHABLE();
    }
...
BUSTER_GLOBAL_LOCAL u32 format_channel_count(TextureFormat format)
{
    switch (format)
    {
        break; case TextureFormat::TEXTURE_FORMAT_R8_UNORM: return 1;
        break; case TextureFormat::TEXTURE_FORMAT_R8G8B8A8_SRGB: return 4;
        break; case TextureFormat::Count: BUSTER_UNREACHABLE();
    }
...
BUSTER_F_IMPL TextureIndex rendering_texture_create(RenderingHandle* rendering, TextureMemory texture_memory)
{
...
BUSTER_F_IMPL TextureIndex white_texture_create(Arena* arena, RenderingHandle* rendering)
{
    ...
    auto white_texture = rendering_texture_create(rendering, (TextureMemory) {
        ...
        .depth = 1,
        .format = TextureFormat::TEXTURE_FORMAT_R8G8B8A8_SRGB,
    });

    return white_texture;
}

BUSTER_F_IMPL FontTextureAtlas rendering_font_create(Arena* arena, RenderingHandle* rendering, FontTextureAtlasCreate create)
{
...
BUSTER_F_IMPL void rendering_window_frame_begin(RenderingHandle* rendering, RenderingWindowHandle* window)
{
...
BUSTER_GLOBAL_LOCAL void buffer_destroy(RenderingHandle* rendering, VulkanBuffer buffer)
{
    ...
    if (buffer.memory.handle)
    {
        if (buffer.type == BufferType::BUFFER_TYPE_STAGING)
        {
...
{
    u8 is_dst = (type == BufferType::BUFFER_TYPE_VERTEX) | (type == BufferType::BUFFER_TYPE_INDEX);
    u8 is_src = type == BufferType::BUFFER_TYPE_STAGING;

    VkBufferUsageFlags usage = 
        (VK_BUFFER_USAGE_TRANSFER_DST_BIT * is_dst) |
        (VK_BUFFER_USAGE_TRANSFER_SRC_BIT * is_src) |
        ((VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) * (type == BufferType::BUFFER_TYPE_VERTEX)) |
        (VK_BUFFER_USAGE_INDEX_BUFFER_BIT * (type == BufferType::BUFFER_TYPE_INDEX));
...
BUSTER_GLOBAL_LOCAL void buffer_copy_to_host(VulkanBuffer buffer, Slice<HostBufferCopy> regions)
{
    BUSTER_CHECK(buffer.type == BufferType::BUFFER_TYPE_STAGING);
...
BUSTER_GLOBAL_LOCAL void buffer_copy_to_local_command(VkCommandBuffer command_buffer, Slice<LocalBufferCopy> copies)
{
...
BUSTER_F_IMPL void rendering_window_frame_end(RenderingHandle* rendering, RenderingWindowHandle* window)
{
    let frame = window_frame(window);
    ...
    {
        for (EACH_ENUM_INT(BusterPipeline, i))
        {
            ...
            if (!arena_buffer_is_empty(frame_pipeline_instantiation->vertex_buffer.cpu))
            {
                ...
                buffer_copy_to_host(frame_pipeline_instantiation->transient_buffer, (Slice<HostBufferCopy>) BUSTER_ARRAY_TO_SLICE(((HostBufferCopy[]) {
                                (HostBufferCopy) {
                                .source = (ByteSlice) {
                ...
                buffer_copy_to_local_command(frame->command_buffer, (Slice<LocalBufferCopy>) BUSTER_ARRAY_TO_SLICE(((LocalBufferCopy[]) {
                                {
        ...
        for (EACH_ENUM_INT(BusterPipeline, i))
        {
            ...
            if (!arena_buffer_is_empty(frame_pipeline_instantiation->vertex_buffer.cpu))
            {
                // Bind pipeline and descriptor sets
                {
                    ...
                    frame->bound_pipeline = (BusterPipeline)i;
                }
...
{
    let frame = window_frame(window);
    let vertex_buffer = &frame->pipeline_instantiations[(u32)pipeline_index].vertex_buffer;
...
BUSTER_GLOBAL_LOCAL void rendering_window_pipeline_add_indices(RenderingWindowHandle* window, BusterPipeline pipeline_index, Slice<u32> indices)
{
    let frame = window_frame(window);
    let index_buffer = &frame->pipeline_instantiations[(u32)pipeline_index].index_buffer;
...
BUSTER_F_IMPL void rendering_window_render_rect(RenderingWindowHandle* window, RectDraw draw)
{
    ...
    let vertex_offset = rendering_window_pipeline_add_vertices(window, BusterPipeline::BUSTER_PIPELINE_RECT, BUSTER_ARRAY_TO_BYTE_SLICE(vertices), BUSTER_ARRAY_LENGTH(vertices));
    ...
    rendering_window_pipeline_add_indices(window, BusterPipeline::BUSTER_PIPELINE_RECT, (Slice<u32>)BUSTER_ARRAY_TO_SLICE(indices));
}

// TODO: support gradient
BUSTER_F_IMPL void rendering_window_render_text(RenderingHandle* rendering, RenderingWindowHandle* window, String8 string, float4 color, RenderFontType font_type, f32 x_offset, f32 y_offset)
{
    let texture_atlas = &rendering->fonts[(u32)font_type];
...
BUSTER_F_IMPL void rendering_window_deinitialize(RenderingHandle* rendering, RenderingWindowHandle* window)
{
    if (vkDeviceWaitIdle(rendering->device) == VK_SUCCESS)
    {
...
BUSTER_F_IMPL void rendering_deinitialize(RenderingHandle* rendering)
{
    if (vkDeviceWaitIdle(rendering->device) == VK_SUCCESS)
    {
        for (EACH_ENUM_INT(BusterPipeline, i))
