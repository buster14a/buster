...
BUSTER_GLOBAL_LOCAL bool analysis_path_has_suffix(String8 path, String8 suffix)
{
    return path.length >= suffix.length &&
        string_equal(
            string_slice(path, path.length - suffix.length, path.length),
            suffix);
}

BUSTER_GLOBAL_LOCAL String8 analysis_module_path(
    Arena* arena,
    String8 module_root,
    String8 module_name)
{
    bool root_has_separator = module_root.length &&
        (module_root.pointer[module_root.length - 1] == '/' ||
         module_root.pointer[module_root.length - 1] == '\\');
    bool has_extension = analysis_path_has_suffix(module_name, S8(".bbb"));
    if (!module_root.length)
    {
        return has_extension ?
            string_duplicate_arena(arena, module_name, true) :
            string_duplicate_arena(
                arena,
                string_format(arena, S8("{S8}.bbb"), module_name),
                true);
    }
    String8 formatted = string_format(
        arena,
        root_has_separator ?
            (has_extension ? S8("{S8}{S8}") : S8("{S8}{S8}.bbb")) :
            (has_extension ? S8("{S8}/{S8}") : S8("{S8}/{S8}.bbb")),
        module_root,
        module_name);
...
BUSTER_GLOBAL_LOCAL AnalysisProgramDiscovery* analysis_program_discovery_find(
    AnalysisProgramDiscovery* first,
    String8 name)
{
...
BUSTER_GLOBAL_LOCAL bool analysis_program_module_after(
    AnalysisProgramModule left,
    AnalysisProgramModule right)
{
...
BUSTER_GLOBAL_LOCAL void analysis_import_diagnostic_push(
    Arena* arena,
    AnalysisResult* result,
    AnalysisImport* import,
    AnalysisDiagnosticKind kind,
    String8 message,
    AnalysisImport* previous)
{
    AnalysisDiagnostic* diagnostic = arena_allocate(arena, AnalysisDiagnostic, 1);
    *diagnostic = (AnalysisDiagnostic){
        .message = message,
        ...
        .kind = kind,
        .subject = kind == ANALYSIS_DIAGNOSTIC_DUPLICATE_IMPORT_NAMESPACE ?
    ...
    u32 input_count)
{
    ...
    for (u32 source_index = 0; source_index < input_count; source_index += 1)
    {
        ...
        for (AstImport* import = source->parser->first_import; import; import = import->next)
        {
            result.module.imports[import_index] = (AnalysisImport){
                .name_space = string_duplicate_arena(
                        result_arena,
                        import->name_space.text,
                        false),
        ...
        for (AstDataDeclaration* data = source->parser->first_data_declaration;
            data;
            data = data->next)
        {
            result.module.entities[entity_index] = (AnalysisEntity){
    ...
    for (u32 index = 0; index < import_count; index += 1)
    {
        ...
        for (u32 previous_index = 0; previous_index < index; previous_index += 1)
        {
            ...
            {
                analysis_import_diagnostic_push(
                        result_arena,
                        &result,
                        import,
                        ANALYSIS_DIAGNOSTIC_DUPLICATE_IMPORT_NAMESPACE,
                        S8("duplicate import namespace"),
                        previous);
                break;
            }
        }
...
void analysis_resolve_imports(
    Arena* result_arena,
    AnalysisResult** modules,
    u32 module_count)
{
    AnalysisResult** program_modules =
            arena_allocate(result_arena, AnalysisResult*, module_count);
    ...
    for (u32 module_index = 0; module_index < module_count; module_index += 1)
    {
        ...
        for (u32 import_index = 0;
             import_index < module->module.import_count;
             import_index += 1)
        {
            ...
            if (match_count == 0)
            {
                ...
                analysis_import_diagnostic_push(
                        ...
                        S8("imported module was not found"),
                        0);
            }
            else if (match_count > 1)
            {
                ...
                analysis_import_diagnostic_push(
                        ...
                        S8("imported module name is ambiguous"),
                        0);
            }
            else
            {
    ...
    AnalysisImportTraversalFrame* frames =
            arena_allocate(scratch.arena, AnalysisImportTraversalFrame, module_count);
    for (u32 module_index = 0; module_index < module_count; module_index += 1)
    {
...
void analysis_resolve_program_interfaces(
    Arena* result_arena,
    AnalysisResult** modules,
    u32 module_count)
{
...
AnalysisEntity* analysis_find_qualified_entity(
    ...
    AnalysisNamespace name_space)
{
         ...
         import_index += 1)
    {
        AnalysisImport* import = module->module.imports + import_index;
        if (import->state == ANALYSIS_IMPORT_RESOLVED &&
            string_equal(import->name_space, import_name_space))
        {
            if (target)
            {
                return 0;
    ...
    for (u32 entity_index = 0;
         entity_index < target->module.entity_count;
         entity_index += 1)
    {
        AnalysisEntity* entity = target->module.entities + entity_index;
        if (entity->name_space == name_space &&
            string_equal(entity->name, entity_name))
        {
            return entity;
        }
    }
...
String8 analysis_serialize_module_interface(Arena* arena, AnalysisResult* result)
{
    ...
    for (u32 import_index = 0;
         import_index < result->module.import_count;
         import_index += 1)
    {
         ...
         entity_index += 1)
    {
        ...
        AnalysisConstant constant = semantic->constant ?
            *semantic->constant : (AnalysisConstant){0};
        parts[part_count++] = string_format(
                arena,
                ...
                entity->id.index.value,
                entity->source.value,
                (u32)entity->kind,
                (u32)entity->name_space,
                entity->name,
                semantic->type.value,
                (u32)constant.kind,
                constant.kind == ANALYSIS_CONSTANT_INTEGER ||
                    constant.kind == ANALYSIS_CONSTANT_BOOLEAN ||
                    constant.kind == ANALYSIS_CONSTANT_ENUM ?
                    constant.integer : 0,
                (u32)constant.is_negative);
    }
    for (u32 type_index = 0; type_index < result->types.count; type_index += 1)
    {
            ...
            argument_index += 1)
        {
            parts[part_count++] = string_format(
                arena,
                S8("type_argument type={u32} index={u32} value={u32}\n"),
                type->id.value,
                argument_index,
...
AnalysisInterfaceCacheEntry* analysis_interface_cache_find(
    AnalysisInterfaceCache* cache,
    String8 module_name)
{
    for (AnalysisInterfaceCacheEntry* entry = cache->first;
        entry;
        entry = entry->next)
    {
...
bool analysis_interface_cache_store(
    Arena* arena,
    AnalysisInterfaceCache* cache,
    String8 module_name,
    AnalysisInterfaceSummary summary)
{
    AnalysisInterfaceCacheEntry* entry = analysis_interface_cache_find(
        cache,
        module_name);
    bool changed = !entry ||
        entry->summary.hash != summary.hash ||
...
BUSTER_GLOBAL_LOCAL String8 analysis_root_module_name(
    Arena* arena,
    AnalysisProgramOptions options)
{
    ...
    {
        if (options.root_path.pointer[index] == '/' ||
            options.root_path.pointer[index] == '\\')
        {
            start = index + 1;
        }
    }
    ...
    if (end >= start + 4 &&
        string_equal(string_slice(options.root_path, end - 4, end), S8(".bbb")))
    {
        end -= 4;
    }
    return string_duplicate_arena(
        arena,
        string_slice(options.root_path, start, end),
        false);
}

AnalysisProgram analysis_program_load(
    Arena* result_arena,
    ...
    AnalysisProgramOptions options)
{
    ...
    AnalysisProgramDiscovery* first = arena_allocate(
        result_arena,
        AnalysisProgramDiscovery,
        1);
    *first = (AnalysisProgramDiscovery){
    ...
    for (AnalysisProgramDiscovery* discovery = first;
        discovery;
        discovery = discovery->next)
    {
        ...
        TokenizerResult tokenizer = tokenize(
            result_arena,
            discovery->source.pointer,
            discovery->source.length);
        discovery->parser = parser_parse(
            result_arena,
            expression_arena,
            discovery->source,
            tokenizer);
        program.parser_diagnostic_count +=
            tokenizer.error_count + discovery->parser.diagnostic_count;

        for (AstImport* import = discovery->parser.first_import;
            import;
            import = import->next)
        {
            ...
            AnalysisProgramDiscovery* appended = arena_allocate(
                result_arena,
                AnalysisProgramDiscovery,
                1);
    ...
    for (u32 index = 1; index < discovery_count; index += 1)
    {
        ...
        u32 insertion = index;
        while (insertion &&
            analysis_program_module_after(program.modules[insertion - 1], value))
        {
    ...
    program.module_results = arena_allocate(
        result_arena,
        AnalysisResult*,
        discovery_count);
    for (u32 index = 0; index < discovery_count; index += 1)
    {
        ...
        *module->analysis = analysis_index_module(
            result_arena,
            (AnalysisModuleId){ .value = index },
            module->name,
            &input,
            1);
...
BUSTER_GLOBAL_LOCAL u32 analysis_ast_type_count(Arena* scratch_arena, AnalysisResult* result)
{
    ...
    for (u32 index = 0; index < result->module.entity_count; index += 1)
    {
        ...
        {
            analysis_ast_type_link_push(
                scratch_arena,
                &top,
                entity->ast.data->type);
            continue;
        }
    ...
    while (top)
    {
        ...
        {
            case AST_TYPE_NAMED:
            case AST_TYPE_QUALIFIED_NAMED: break;
...
BUSTER_GLOBAL_LOCAL bool analysis_type_matches(AnalysisType* existing, AnalysisType* candidate)
{
    ...
    switch (candidate->kind)
    {
        ...
        {
            return
                existing->as.declaration.module.value ==
                    candidate->as.declaration.module.value &&
                existing->as.declaration.index.value ==
                    candidate->as.declaration.index.value;
        }
        case ANALYSIS_TYPE_POISON:
        case ANALYSIS_TYPE_VOID:
        case ANALYSIS_TYPE_BOOL:
...
BUSTER_GLOBAL_LOCAL AnalysisTypeId analysis_builtin_type_find(
    AnalysisTypeTable* table,
    String8 name);

BUSTER_GLOBAL_LOCAL AnalysisResult* analysis_program_module_from_id(
    AnalysisResult* result,
    AnalysisModuleId id)
{
    if (result->module.id.value == id.value)
...
String8 analysis_format_diagnostic(
    Arena* arena,
    AnalysisResult* result,
    AnalysisDiagnostic* diagnostic)
{
    u32 note_count = 0;
    for (AnalysisDiagnosticNote* note = diagnostic->first_note;
        note;
        note = note->next)
    {
        note_count += 1;
    ...
    String8 path = analysis_source_path(
        result,
        diagnostic_module,
        diagnostic->source);
    parts[0] = string_format(
        arena,
        diagnostic->subject.length ?
        ...
        note = note->next)
    {
        ...
        parts[part_index] = string_format(
            arena,
            S8("{S8}:{u32}:{u32}: note: {S8}\n"),
    ...
    return string_join_arena(
        arena,
        (SliceString8){ .pointer = parts, .length = part_index },
        false);
}

BUSTER_GLOBAL_LOCAL AnalysisEntitySemantic* analysis_declaration_semantic(
    AnalysisResult* result,
    AnalysisEntityId declaration,
    AnalysisResult** declaration_module)
{
    AnalysisResult* module =
            analysis_program_module_from_id(result, declaration.module);
...
BUSTER_GLOBAL_LOCAL void analysis_import_type_task_push(
    Arena* scratch_arena,
    AnalysisImportTypeTask** top,
    AnalysisImportTypeTask task)
{
    AnalysisImportTypeTask* pushed =
...
BUSTER_GLOBAL_LOCAL AnalysisTypeId analysis_type_import(
    Arena* result_arena,
    Arena* scratch_arena,
    ...
    AnalysisTypeId source_id)
{
    ...
    analysis_import_type_task_push(
        scratch_arena,
        &top,
        ...
        if (task->kind == ANALYSIS_IMPORT_TYPE_VISIT)
        {
            ...
            switch (type->kind)
            {
                ...
                case ANALYSIS_TYPE_FLOAT:
                {
                    ...
                    *task->destination =
                            builtin.value == ANALYSIS_ID_UNDERLYING_INVALID ?
                            destination->types.builtin.poison : builtin;
                } break;
                case ANALYSIS_TYPE_COMPILE_TIME_PARAMETER:
                {
...
BUSTER_GLOBAL_LOCAL AnalysisImport* analysis_import_find(
    AnalysisResult* result,
    String8 name_space)
{
    ...
    AnalysisTypeTask* task)
{
    ...
    switch (ast->id)
    {
        ...
        {
            AnalysisImport* import = analysis_import_find(
                    context->result,
                    ast->qualified.name_space.text);
            if (import && import->state != ANALYSIS_IMPORT_RESOLVED)
            {
                *task->destination = poison;
                return;
            }
            AnalysisEntity* entity = import ?
                    analysis_find_qualified_entity(
                        context->result,
                        ast->qualified.name_space.text,
                        ast->qualified.name.text,
                        ANALYSIS_NAMESPACE_TYPE) : 0;
            ...
            AnalysisEntitySemantic* semantic =
                    import->target->module.semantics + entity->id.index.value;
            ...
            *task->destination = analysis_type_import(
                    context->result_arena,
    ...
    AnalysisTypeTask* task)
{
    ...
    if (entity->kind == ANALYSIS_ENTITY_DATA)
    {
        AnalysisTypeTask* finish = analysis_type_task_push(
            context,
            (AnalysisTypeTask){
                .owner = entity,
                .destination = task->destination,
...
void analysis_resolve_module_interfaces(Arena* result_arena, AnalysisResult* result)
{
         ...
         import_index += 1)
    {
            ...
            import->target->types.types)
        {
            BUSTER_CHECK(
                    result->types.capacity <=
                    UINT32_MAX - import->target->types.count);
...
BUSTER_GLOBAL_LOCAL String8 analysis_diagnostic_message(AnalysisDiagnosticKind kind)
{
    switch (kind)
    {
        ...
        case ANALYSIS_DIAGNOSTIC_DUPLICATE_DECLARATION: return S8("duplicate declaration");
        ...
        if (node->id == AST_NODE_AGGREGATE_LITERAL)
        {
                ...
                literal_field = literal_field->next, child += 1)
            {
                ...
                else
                {
                    ...
                    AnalysisTypeId field_type = analysis_type_import(
                            context->result_arena,
                            context->scratch_arena,
                            context->result,
                            declaration_module,
    ...
    for (u32 index = 0; index < ast.count; index += 1)
    {
        ...
        switch (node->id)
        {
            ...
            case AST_NODE_MEMBER_ACCESS:
            {
                ...
                {
                    AnalysisResult* target = analysis_program_module_from_id(
                            context->result,
                            base->namespace_module);
                    AnalysisEntity* entity = target ?
                            analysis_value_entity_find(
                                    target,
                                    node->member_access.member.text) : 0;
                    if (entity)
                    {
                        AnalysisEntitySemantic* semantic =
                                target->module.semantics + entity->id.index.value;
                        if (semantic->state == ANALYSIS_RESOLUTION_RESOLVED)
                        {
    ...
    AnalysisTypeId return_type)
{
    ...
    switch (statement->id)
    {
        ...
        case AST_STATEMENT_DATA:
        {
            ...
            {
                local->constant =
                    initializer->nodes[data->initializer.count - 1].constant;
        ...
        case AST_STATEMENT_SWITCH:
        {
            ...
            {
                AnalysisEntitySemantic* enum_semantic =
                        analysis_declaration_semantic(
                                context->result,
                                switched_type->as.declaration,
                                0);
...
BUSTER_GLOBAL_LOCAL void analysis_analyze_module_constants(
    Arena* result_arena,
    Arena* scratch_arena,
    AnalysisResult* result)
{
        ...
        entity_index += 1)
    {
        ...
        AnalysisEntitySemantic* semantic =
            result->module.semantics + entity_index;
        ...
        AnalysisTypedExpression* expression = analysis_expression(
            &context,
            0,
            entity->ast.data->initializer,
            semantic->type);
        semantic->constant = arena_allocate(
            result_arena,
            AnalysisConstant,
            1);
        ...
        {
            *semantic->constant =
                expression->nodes[expression->ast.count - 1].constant;
        }
        ...
        for (u32 index = 0; index < result->types.count; index += 1)
        {
            ...
            switch (type->kind)
            {
                ...
                case ANALYSIS_TYPE_UNION:
                {
                    ...
                    {
                        AnalysisTypeLayout imported_layout =
                                analysis_type_from_id(
                                        declaration_module,
                                        semantic->type)->layout;
                        if (imported_layout.state == ANALYSIS_LAYOUT_RESOLVED ||
                            imported_layout.state == ANALYSIS_LAYOUT_ERROR)
                        {
...
BUSTER_GLOBAL_LOCAL void analysis_program_schedule_worker(void* raw_worker)
{
    AnalysisProgramScheduleWorker* worker =
        (AnalysisProgramScheduleWorker*)raw_worker;
    for (u32 index = worker->worker_index;
        index < worker->ready_count;
        index += worker->worker_count)
    {
        AnalysisProgramJob ready = worker->ready[index];
        AnalysisResult* module = 0;
        for (u32 module_index = 0;
            module_index < worker->program->module_count;
            module_index += 1)
        {
            AnalysisResult* candidate =
                worker->program->module_results[module_index];
            if (candidate &&
                candidate->module.id.value == ready.module.value)
            {
                module = candidate;
                break;
            }
        ...
        worker->callback(
            module,
            module->jobs + ready.job.value,
            worker->worker_index,
            worker->user_data);
    }
}

BUSTER_GLOBAL_LOCAL u32 analysis_program_module_index(
    AnalysisProgram* program,
    AnalysisModuleId id)
{
...
AnalysisProgramScheduleResult analysis_execute_program_jobs(
    Arena* result_arena,
    AnalysisProgram* program,
    u32 worker_count,
    AnalysisProgramJobCallback* callback,
    void* user_data)
{
        ...
        module_index += 1)
    {
        ...
        offsets[module_index + 1] = offsets[module_index] +
            (module ? module->job_count : 0);
    }
    u32 total = offsets[program->module_count];
    schedule.execution_order = arena_allocate(
        result_arena,
        AnalysisProgramJob,
        total);
    bool* complete = arena_allocate(result_arena, bool, total);
    memset(complete, 0, sizeof(*complete) * total);
    AnalysisProgramJob* ready = arena_allocate(
        result_arena,
        AnalysisProgramJob,
        total);
    worker_count = BUSTER_MAX(worker_count, 1);
    ...
    while (schedule.execution_count < total)
    {
            ...
            module_index += 1)
        {
            ...
            for (u32 job_index = 0; job_index < module->job_count; job_index += 1)
            {
                ...
                for (u32 dependency_index = 0;
                    dependency_index < job->dependency_count;
                    dependency_index += 1)
                {
                    AnalysisJobId dependency = job->dependencies[dependency_index];
                    dependencies_complete &=
                        dependency.value < module->job_count &&
                        complete[offsets[module_index] + dependency.value];
                }
                ...
                if (job->kind == ANALYSIS_JOB_INTERFACE)
                {
                    ...
                    {
                        AnalysisImport* import =
                            module->module.imports + import_index;
                        if (import->state != ANALYSIS_IMPORT_RESOLVED ||
                            !import->target)
                        {
                            continue;
                        }
                        u32 target_index = analysis_program_module_index(
                            program,
                            import->target_id);
                        BUSTER_CHECK(target_index != UINT32_MAX);
                        for (u32 target_job = 0;
                            target_job < import->target->job_count;
                            target_job += 1)
                        {
                            if (import->target->jobs[target_job].kind ==
                                ANALYSIS_JOB_INTERFACE)
                            {
                                dependencies_complete &=
                                    complete[offsets[target_index] + target_job];
                            }
                        }
                    }
                }
                    ...
                    job->entity.index.value < module->module.entity_count)
                {
                    ...
                    for (u32 dependency_index = 0;
                        dependencies_complete &&
                        dependency_index < body->dependency_count;
                        dependency_index += 1)
                    {
                        AnalysisEntityId dependency =
                            body->dependencies[dependency_index];
                        ...
                        AnalysisResult* target =
                            program->module_results[target_index];
                        dependencies_complete &=
                            dependency.index.value < target->module.entity_count &&
                            complete[
        ...
        if (callback && active_workers > 1)
        {
            AnalysisProgramScheduleWorker* workers = arena_allocate(
                result_arena,
                AnalysisProgramScheduleWorker,
                active_workers);
            OsThreadHandle** handles = arena_allocate(
                result_arena,
                OsThreadHandle*,
                active_workers);
            for (u32 worker_index = 0;
                ...
                worker_index += 1)
            {
                ...
                handles[worker_index] = os_thread_create(
                    (ThreadCreateOptions){
        ...
        for (u32 ready_index = 0; ready_index < ready_count; ready_index += 1)
        {
            ...
            u32 module_index = analysis_program_module_index(
                program,
                item.module);
...
UnitTestResult analysis_tests(UnitTestArguments* arguments)
{
    ...
    String8 indexed_summary =
            analysis_serialize_module_interface(arguments->arena, &indexed);
    String8 forward_summary =
            analysis_serialize_module_interface(arguments->arena, &forward);
    ...
    AnalysisResult app_module = analysis_index_module(
        arguments->arena, (AnalysisModuleId){ .value = 20 }, S8("app"), &app_input, 1);
    AnalysisResult math_module = analysis_index_module(
        arguments->arena, (AnalysisModuleId){ .value = 21 }, S8("core/math"), &math_input, 1);
    AnalysisResult a_module = analysis_index_module(
        arguments->arena, (AnalysisModuleId){ .value = 22 }, S8("cycle/a"), &a_input, 1);
    AnalysisResult b_module = analysis_index_module(
        arguments->arena, (AnalysisModuleId){ .value = 23 }, S8("cycle/b"), &b_input, 1);
    ...
    analysis_resolve_imports(
        arguments->arena,
        import_modules,
        BUSTER_ARRAY_LENGTH(import_modules));
    ...
    BUSTER_TEST(arguments, app_module.first_diagnostic != 0 &&
            app_module.first_diagnostic->kind ==
                    ANALYSIS_DIAGNOSTIC_MISSING_IMPORTED_MODULE);
    AnalysisEntity* qualified_sum = analysis_find_qualified_entity(
        &app_module,
        S8("math"),
        S8("sum"),
    ...
    BUSTER_TEST(arguments, b_module.first_diagnostic != 0 &&
            b_module.first_diagnostic->kind == ANALYSIS_DIAGNOSTIC_IMPORT_CYCLE);
    ...
    AnalysisResult duplicate_import_module = analysis_index_module(
        arguments->arena,
        (AnalysisModuleId){ .value = 24 },
        S8("duplicate-import"),
        &duplicate_import_input,
        1);
    ...
    BUSTER_TEST(arguments, duplicate_import_module.first_diagnostic != 0 &&
            duplicate_import_module.first_diagnostic->kind ==
                    ANALYSIS_DIAGNOSTIC_DUPLICATE_IMPORT_NAMESPACE);
    ...
    String8 namespace_math_source = S8(
        "type Vector = struct { x: s32, y: s32, }\n"
        "code add : fn (a: s32, b: s32) s32\n"
        "{\n"
        "    return a + b;\n"
        "}\n"
    ...
    AnalysisProgramScheduleResult program_schedule =
        analysis_execute_program_jobs(
            arguments->arena,
            &scheduled_program,
            4,
            0,
    ...
    if (namespace_use)
    {
        AnalysisType* namespace_function = analysis_type_from_id(
                &namespace_app,
                namespace_app.module.semantics[
                        namespace_use->id.index.value].type);
        BUSTER_TEST(arguments,
                namespace_function->kind ==
                        ANALYSIS_TYPE_FUNCTION);
        BUSTER_TEST(arguments,
                namespace_function->kind != ANALYSIS_TYPE_FUNCTION ||
                namespace_function->as.function.argument_count == 1);
        if (namespace_function->kind == ANALYSIS_TYPE_FUNCTION &&
            namespace_function->as.function.argument_count == 1)
        {
            AnalysisType* namespace_vector = analysis_type_from_id(
                    &namespace_app,
                    namespace_function->as.function.argument_types[0]);
            BUSTER_TEST(arguments,
                    namespace_vector->kind == ANALYSIS_TYPE_STRUCT);
            BUSTER_TEST(arguments,
                    namespace_vector->as.declaration.module.value == 40);
            BUSTER_TEST(arguments,
                    namespace_vector->layout.state ==
    ...
    {
        String8 formatted = analysis_format_diagnostic(
            arguments->arena,
            &rule_result,
            rule_result.first_diagnostic);
        BUSTER_TEST(arguments,
            string_first_sequence(formatted, S8("semantic-rules.bbb")) !=
                BUSTER_STRING_NO_MATCH);
        BUSTER_TEST(arguments,
            string_first_sequence(formatted, S8(": error: ")) !=
                BUSTER_STRING_NO_MATCH);
    }
        ...
        fixture_index += 1)
    {
        ...
        {
            AnalysisEntity* constant = analysis_value_entity_find(
                &fixture_result,
                S8("this_is_a_constant"));
            BUSTER_TEST(arguments,
                constant && constant->kind == ANALYSIS_ENTITY_DATA);
            if (constant)
            {
                AnalysisEntitySemantic* semantic =
                    fixture_result.module.semantics +
                    constant->id.index.value;
                BUSTER_TEST(arguments,
                    semantic->constant &&
                    semantic->constant->kind ==
                        ANALYSIS_CONSTANT_INTEGER);
                BUSTER_TEST(arguments,
                    semantic->constant &&
                    semantic->constant->integer == 5);
            }
    ...
    AnalysisProgram loaded = analysis_program_load(
        arguments->arena,
        fixture_expression_arena,
    ...
    if (loaded.root)
    {
        ...
        String8 first_summary = analysis_serialize_module_interface(
            arguments->arena,
            loaded.root->analysis);
        String8 second_summary = analysis_serialize_module_interface(
            arguments->arena,
            loaded.root->analysis);
        BUSTER_STRING_TEST(arguments, first_summary, second_summary);
        AnalysisInterfaceSummary hashed =
            analysis_module_interface_summary(
                arguments->arena,
                loaded.root->analysis);
        ...
        AnalysisProgramScheduleResult loaded_schedule =
            analysis_execute_program_jobs(
                arguments->arena,
                &loaded,
                4,
