...
enum
{
    CODEGEN_CANONICAL_ABI_MAX_PARTS = 4,
};

typedef struct CodegenCanonicalAbiPart CodegenCanonicalAbiPart;
struct CodegenCanonicalAbiPart
{
...
typedef struct CodegenCanonicalAbiValue CodegenCanonicalAbiValue;
struct CodegenCanonicalAbiValue
{
    CodegenCanonicalAbiPart parts[CODEGEN_CANONICAL_ABI_MAX_PARTS];
    u32 part_count;
    bool indirect;
    bool memory;
    u8 reserved[2];
...
typedef struct CodegenCanonicalAbiTask CodegenCanonicalAbiTask;
struct CodegenCanonicalAbiTask
{
    IrTypeId type;
    u64 offset;
};

BUSTER_GLOBAL_LOCAL bool codegen_canonical_system_v_classes(IrProgram* program, IrTypeId root_type, IrAbiClass classes[2])
{
    ...
    CodegenCanonicalAbiTask* tasks = arena_allocate(temporary.arena, CodegenCanonicalAbiTask, capacity);
    u32 count = 1;
    tasks[0] = (CodegenCanonicalAbiTask){
        .type = root_type,
    };
    bool valid = true;
    while (count && valid)
    {
        CodegenCanonicalAbiTask task = tasks[--count];
        ...
        if (type->kind == IR_TYPE_STRUCT || type->kind == IR_TYPE_UNION)
        {
            ...
            for (u32 index = 0; index < type->field_count; index += 1)
            {
                ...
                tasks[count++] = (CodegenCanonicalAbiTask){
                    .type = field->type,
...
BUSTER_GLOBAL_LOCAL bool codegen_canonical_homogeneous_float(IrProgram* program, IrTypeId root_type, IrTypeId* element_out, u32* count_out)
{
    TemporalArena temporary = scratch_begin(0, 0);
    u32 capacity = BUSTER_MAX(program->types.count * 16, 16);
    ...
    while (task_count && valid)
    {
        ...
        else if (type->kind == IR_TYPE_FLOAT || (type->kind == IR_TYPE_VECTOR && (type->layout.size == 8 || type->layout.size == 16)))
        {
            ...
            element = type_id;
            count += 1;
            if (count > CODEGEN_CANONICAL_ABI_MAX_PARTS)
            {
                valid = false;
            }
        }
...
{
    CodegenCanonicalAbiValue value = {0};
    IrType* type = ir_type_from_id(&program->types, type_id);
    ...
    if (type->kind == IR_TYPE_VECTOR)
    {
        ...
        if (abi == CODEGEN_ABI_X86_64_WINDOWS && !is_result)
        {
            value.part_count = 1;
            value.indirect = true;
    ...
    if (abi == CODEGEN_ABI_AARCH64_AAPCS64 || abi == CODEGEN_ABI_AARCH64_DARWIN || abi == CODEGEN_ABI_AARCH64_WINDOWS)
    {
        IrTypeId element = IR_TYPE_ID_INVALID;
        u32 count = 0;
        if (!(abi == CODEGEN_ABI_AARCH64_WINDOWS && variadic_argument) && codegen_canonical_homogeneous_float(program, type_id, &element, &count))
        {
            IrType* element_type = ir_type_from_id(&program->types, element);
            value.part_count = count;
            for (u32 part = 0; part < count; part += 1)
            {
                value.parts[part] = (CodegenCanonicalAbiPart){
                    .abi_class = IR_ABI_CLASS_FLOAT,
    ...
    if (!codegen_canonical_system_v_classes(program, type_id, classes))
    {
        value.part_count = 1;
        value.indirect = is_result;
        value.memory = !is_result;
