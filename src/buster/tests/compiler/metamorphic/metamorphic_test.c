                                               ...
                                               String8 side, u32 mask, bool reference)
{
    ...
    if (context->directory.length && context->compiler.length && file_write(source, BUSTER_SLICE_TO_BYTE_SLICE(text)))
    {
        String8 argv[16];
        u32 count = 0;
...
BUSTER_GLOBAL_LOCAL MetaSummary meta_run(MetaContext* context, u32 first_seed, u32 cases, u32 transform_mask)
{
    ...
    u64 budget_ns = context->full ? UINT64_C(300000000000) : UINT64_C(60000000000);
...
#if BUSTER_LINK_LIBC && !BUSTER_ANDROID && !BUSTER_IOS
    u32 seed = 1, cases = 4, mask = BUSTER_META_ALL_TRANSFORMS, require_execution = 0;
    ...
    if (valid)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                string_print(S8("METAMORPHIC_SUMMARY seed={u32} cases={u32} pairs={u32} executed={u32} unexecuted={u32} failures={u32} unique={u32} reducer_replays={u32} output={S8}\n"),
                             seed, cases, summary.pairs, summary.executed, summary.unexecuted, summary.failures, summary.unique_failures, summary.reductions, directory);
