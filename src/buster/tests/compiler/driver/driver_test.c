                                                                                                      ...
                                                                                                      CPreprocessResult expected, String8 text)
{
    ...
    u64 actual_index = 0;
    for (u64 expected_index = 0; expected_index < expected.token_count; expected_index += 1)
    {
        CToken expected_token = expected.tokens[expected_index];
        if (expected_token.kind == C_TOKEN_END_OF_FILE)
        {
            continue;
        }
        ...
        if (BUSTER_REQUIRE(arguments, actual_index < actual.token_count && actual.tokens[actual_index].kind != C_TOKEN_END_OF_FILE))
        {
            CToken actual_token = actual.tokens[actual_index++];
            BUSTER_TEST(arguments, actual_token.kind == expected_token.kind);
            BUSTER_STRING_TEST(arguments, c_token_spelling(actual.spelling_base, actual_token),
                               c_token_spelling(expected.spelling_base, expected_token));
        }
    }
    while (actual_index < actual.token_count &&
           (actual.tokens[actual_index].kind == C_TOKEN_NEWLINE || actual.tokens[actual_index].kind == C_TOKEN_END_OF_FILE))
    {
        actual_index += 1;
    }
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_preprocess_boundaries(UnitTestArguments* arguments)
{
    ...
    {
        CPreprocessResult expected = c_preprocess(arguments->arena, BYTE_SLICE_TO_STRING(8, source_bytes),
                                                  (CPreprocessOptions){
        ...
        ProcessSpawnResult stdout_child = os_process_spawn((SliceString8)BUSTER_ARRAY_TO_SLICE(stdout_arguments), (SliceString8){0},
                                                           (SliceString8){0},
                                                           (ProcessSpawnOptions){
        ...
        {
            ProcessWaitResult stdout_waited = os_process_wait_deadline(arguments->arena, stdout_child, 30000000);
            ...
            String8 required[] = {
                ...
                S8("long macro_name;"),
                S8("x shorter_name;"),
                S8("signed nested_name;"),
                S8("L \"text\";"),
                S8("1e +2;"),
                S8("1e ++ exponent_plus;"),
                S8("123 number_name;"),
                S8("123 'a';"),
                S8("123 ... ellipsis_after_number;"),
                S8(". . . ellipsis_tokens;"),
            ...
            String8 output_path = buster_test_temporary_path(arguments->arena, S8("buster-preprocess-boundaries"), S8(".i"));
            String8 output_arguments[] = {
                program_state->input.arguments.pointer[0], S8("cc"), S8("-E"), S8("-std=c23"), S8("-x"), S8("c"), S8("-o"), output_path,
                source_path,
            };
            ProcessSpawnResult output_child = os_process_spawn((SliceString8)BUSTER_ARRAY_TO_SLICE(output_arguments), (SliceString8){0},
                                                               (SliceString8){0},
                                                               (ProcessSpawnOptions){
                                                                   .capture = ((u64)1 << STANDARD_STREAM_OUTPUT) |
                                                                              ((u64)1 << STANDARD_STREAM_ERROR),
            ...
            {
                ProcessWaitResult output_waited = os_process_wait_deadline(arguments->arena, output_child, 30000000);
                BUSTER_TEST(arguments, !output_waited.timed_out && output_waited.result == PROCESS_RESULT_SUCCESS);
                BUSTER_TEST(arguments, output_waited.streams[STANDARD_STREAM_OUTPUT].length == 0);
                BUSTER_TEST(arguments, output_waited.streams[STANDARD_STREAM_ERROR].length == 0);
                ByteSlice written = file_read(arguments->arena, output_path, (FileReadOptions){0});
                BUSTER_TEST(arguments, written.pointer != 0);
                if (written.pointer)
                {
                    String8 output_text = BYTE_SLICE_TO_STRING(8, written);
                    BUSTER_STRING_TEST(arguments, output_text, stdout_text);
                    identity = compiler_driver_test_preprocess_token_identity(arguments, expected, output_text);
                    result.test_count += identity.test_count;
                    result.succeeded_test_count += identity.succeeded_test_count;
                }
        ...
        if (BUSTER_REQUIRE(arguments, backslash_bytes.pointer != 0))
        {
            ...
            CPreprocessResult backslash_expected = c_preprocess(arguments->arena, BYTE_SLICE_TO_STRING(8, backslash_bytes),
                                                                (CPreprocessOptions){
                                                                    .definitions = &definition,
                                                                    .source_path = backslash_path,
                                                                    .dialect = C_PREPROCESS_DIALECT_C23,
                                                                    .definition_count = 1,
                                                                });
            BUSTER_TEST(arguments, backslash_expected.diagnostic_count == 0);
            String8 backslash_arguments[] = {
                program_state->input.arguments.pointer[0], S8("cc"), S8("-E"), S8("-std=c23"), S8("-x"), S8("c"), S8("-DB=\\"),
                backslash_path,
            };
            ProcessSpawnResult backslash_child = os_process_spawn((SliceString8)BUSTER_ARRAY_TO_SLICE(backslash_arguments), (SliceString8){0},
                                                                  (SliceString8){0},
                                                                  (ProcessSpawnOptions){
                                                                      .capture = ((u64)1 << STANDARD_STREAM_OUTPUT) |
                                                                                 ((u64)1 << STANDARD_STREAM_ERROR),
            ...
            {
                ProcessWaitResult backslash_waited = os_process_wait_deadline(arguments->arena, backslash_child, 30000000);
                BUSTER_TEST(arguments, !backslash_waited.timed_out && backslash_waited.result == PROCESS_RESULT_SUCCESS);
                BUSTER_TEST(arguments, backslash_waited.streams[STANDARD_STREAM_ERROR].length == 0);
                String8 backslash_text = BYTE_SLICE_TO_STRING(8, backslash_waited.streams[STANDARD_STREAM_OUTPUT]);
                BUSTER_TEST(arguments, string_first_sequence(backslash_text, S8("\\ \nint after_backslash;")) != BUSTER_STRING_NO_MATCH);
                UnitTestResult identity = compiler_driver_test_preprocess_token_identity(arguments, backslash_expected, backslash_text);
                result.test_count += identity.test_count;
                result.succeeded_test_count += identity.succeeded_test_count;
            }
        }
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_wide_vector_boundaries(UnitTestArguments* arguments)
{
    ...
    // convention differs, so its positive exchange uses the AVX contract.
    {
        ...
        {
            bool available = hosts[host].length != 0;
            if (available && host == 1)
            {
                ...
                String8 version = {.pointer = (char8*)waited.streams[STANDARD_STREAM_OUTPUT].pointer,
                                   .length = waited.streams[STANDARD_STREAM_OUTPUT].length};
                available = spawn.handle && waited.result == PROCESS_RESULT_SUCCESS &&
                    string_first_sequence(version, S8("clang")) == BUSTER_STRING_NO_MATCH;
            }
#if BUSTER_LINUX
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                for (u32 pic = 0; pic < 2; pic += 1)
                {
                    for (u32 cpu = 0; cpu < (target < 6 ? BUSTER_ARRAY_LENGTH(cpus) : 1u); cpu += 1)
                    {
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                ...
                {
                    TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                    String8 output = buster_test_temporary_path(temporary.arena, S8("buster-sysv-aligned-call"), S8(".o"));
                    String8 command[] = {S8("-c"), S8("-target"), targets[target], S8("-march=baseline"), modes[mode], frontends[frontend],
                        S8("-DOVERALIGNED_STACK_SCALAR_ONLY=1"), S8("-fno-machine-fallback"), S8("-fverify-codegen"), sources[source], S8("-o"), output};
                    CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena,
                        compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command)));
                    BUSTER_TEST_RAW(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object, compiled.diagnostic);
                    BUSTER_TEST(arguments, compiled.codegen_statistics.function_count == functions[source] &&
                                           compiled.codegen_statistics.fallback_function_count == 0);
                    scratch_end(temporary);
                }
            }
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            for (u32 size = 0; size < BUSTER_ARRAY_LENGTH(sizes); size += 1)
            {
                ...
                CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena,
                    compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command)));
                // Large x64 MIR frames used to exceed the one-byte prologue
                // limit; ARM64 PE previously excluded every MIR function.
                // NONE is the MIR_STACK compatibility spelling on both targets.
                BUSTER_TEST_RAW(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object, compiled.diagnostic);
...
#if BUSTER_WINDOWS
                    if ((BUSTER_CPU_ARCH_X86_64 && target == 0) || (BUSTER_CPU_ARCH_AARCH64 && target == 2))
                    {
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_windows_arm64_unwind(UnitTestArguments* arguments)
{
    ...
    {
        TemporalArena temporary = scratch_begin(&arguments->arena, 1);
        String8 object_path = buster_test_temporary_path(temporary.arena, S8("buster-winarm64-unwind"), S8(".obj"));
        String8 command[] = {S8("-c"), mode ? S8("-g") : S8("-g0"), S8("-target"), S8("aarch64-windows"), modes[mode],
                             S8("-fno-machine-fallback"), S8("-fverify-codegen"), S8("tests/windows_arm64_mir_unwind.c"), S8("-o"), object_path};
        CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena,
            compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command)));
        BUSTER_TEST_RAW(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object, compiled.diagnostic);
        ...
        if (compiled.error == COMPILER_DRIVER_ERROR_NONE)
        {
            String8 output = buster_test_temporary_path(temporary.arena, S8("buster-winarm64-unwind"), S8(".exe"));
            String8 link[] = {object_path, S8("-l"), S8("ntdll"), S8("-o"), output};
            CompilerDriverResult linked = compiler_driver_execute_invocation(temporary.arena,
                compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(link)));
            BUSTER_TEST_RAW(arguments, linked.error == COMPILER_DRIVER_ERROR_NONE, linked.diagnostic);
            if (linked.error == COMPILER_DRIVER_ERROR_NONE)
            {
                ...
                if (spawn.handle)
                {
                    ProcessWaitResult wait = os_process_wait_sync(temporary.arena, spawn);
                    u64 report[4] = {0};
                    bool report_valid = wait.streams[STANDARD_STREAM_OUTPUT].length == sizeof(report);
                    if (report_valid) { memcpy(report, wait.streams[STANDARD_STREAM_OUTPUT].pointer, sizeof(report)); }
                    arguments->show(arguments, S8("ARM64 MIR unwind {S8}: result={u32} status={u32:x} success={u64} boundaries={u64} saved={u64:x} failure={u64:x}\n"),
...
// Quad widening has no runtime-library or scalar-quad ABI dependency. Keep
// the previously rejected NaN-sign fixture and the independent byte oracle
// strict on every AArch64 target whose long double format is binary128.
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_aarch64_float_to_f128(UnitTestArguments* arguments)
{
    UnitTestResult result = {0};
    ...
    for (u32 direction = 0; direction < 2; direction += 1)
    {
        host_objects[direction] = buster_test_temporary_path(arguments->arena,
            direction ? S8("buster-f128-host-library") : S8("buster-f128-host-observer"), S8(".o"));
        String8 command[12];
        u32 count = 0;
        ...
        if (direction) { command[count++] = S8("-DBUSTER_F128_HOST_LIBRARY=1"); }
        command[count++] = S8("-c");
        command[count++] = S8("tests/host_aarch64_float_to_f128.c");
        command[count++] = S8("-o");
            ...
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                for (u32 position = 0; position < BUSTER_ARRAY_LENGTH(positions); position += 1)
                {
                    for (u32 fixture = 0; fixture < BUSTER_ARRAY_LENGTH(fixtures); fixture += 1)
                    {
                        ...
                        {
                            String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-a64-f128-run"), S8(".elf"));
                            String8 link[] = {S8("-target"), targets[target], S8("-o"), executable, output};
                            CompilerDriverResult linked = compiler_driver_execute_invocation(temporary.arena,
                                compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(link)));
                            BUSTER_TEST_RAW(arguments, linked.error == COMPILER_DRIVER_ERROR_NONE, linked.diagnostic);
                            if (linked.error == COMPILER_DRIVER_ERROR_NONE)
                        ...
                        if (target == 0 && fixture == 1)
                        {
                            for (u32 direction = 0; direction < 2; direction += 1)
                            {
                                ...
                                CompilerDriverResult mixed_result = compiler_driver_execute_invocation(temporary.arena,
                                    compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(mixed)));
                                ...
                                {
                                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-f128-mixed-run"), S8(".elf"));
                                    String8 link[10];
                                    u32 count = 0;
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_aarch64_float_to_i128(UnitTestArguments* arguments)
{
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-a64-float_to_i128"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
                ...
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-a64-float_to_i128-run"), S8(".exe"));
                    String8 native_command[] = {modes[mode], frontends[frontend], S8("-fverify-codegen"), S8("-o"), executable,
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_native_i128_divide(UnitTestArguments* arguments)
{
    ...
    struct
    {
        ...
        {S8("tests/basic_c_aarch64_i128_divide.c"), 8, true},
        {S8("tests/basic_c_aarch64_i128.c"), 38, false},
        {S8("tests/basic_c_i128_shift_edges.c"), 2, true},
        {S8("tests/basic_c_i128_divide_oracle_edges.c"), 1, true},
        {S8("tests/basic_c_asm_goto_division.c"), 2, true},
    };
    for (u32 fixture = 0; fixture < BUSTER_ARRAY_LENGTH(fixtures); fixture += 1)
    {
        for (u32 target = 0; target < (fixtures[fixture].x86_64 ? BUSTER_ARRAY_LENGTH(targets) : 3u); target += 1)
        {
            for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
            {
                for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
                {
                    TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                    String8 output = buster_test_temporary_path(temporary.arena, S8("buster-native-i128_divide"), S8(".o"));
                    String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_x64_dynamic_stack(UnitTestArguments* arguments)
{
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-x64-dynamic-stack"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
                ...
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-x64-dynamic-stack-run"), S8(".exe"));
                    String8 native_command[] = {modes[mode], frontends[frontend], S8("-fverify-codegen"), S8("-o"), executable,
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_vla_row_address(UnitTestArguments* arguments)
{
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-vla-row-address"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
                ...
                bool native_target = native_arch && ((target % 3 == 0 && BUSTER_LINUX) || (target % 3 == 1 && BUSTER_MACOS) ||
                                                      (target % 3 == 2 && BUSTER_WINDOWS));
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-vla-row-address-run"), S8(".exe"));
                    String8 native_command[] = {modes[mode], frontends[frontend], S8("-fverify-codegen"), S8("-o"), executable,
...
// The argument count comes from IR; register exhaustion only changes placement.
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_many_native_arguments(UnitTestArguments* arguments)
{
    UnitTestResult result = {0};
    String8 targets[] = {S8("aarch64-linux"), S8("aarch64-macos"), S8("aarch64-windows"),
                         S8("x86_64-linux"), S8("x86_64-macos"), S8("x86_64-windows")};
    ...
    String8 host_command[16];
    u32 host_count = 0;
    host_command[host_count++] = S8(BUSTER_HOST_C_COMPILER);
    if (S8(BUSTER_HOST_C_COMPILER_ARG1).length) host_command[host_count++] = S8(BUSTER_HOST_C_COMPILER_ARG1);
    host_command[host_count++] = S8("-O2");
    host_command[host_count++] = S8("-fno-tree-vectorize");
    host_command[host_count++] = S8("-fno-tree-slp-vectorize");
    host_command[host_count++] = S8("-ffreestanding");
    host_command[host_count++] = S8("-fno-stack-protector");
    host_command[host_count++] = S8("-fno-unwind-tables");
    host_command[host_count++] = S8("-fno-asynchronous-unwind-tables");
    host_command[host_count++] = S8("-c");
    host_command[host_count++] = S8("tests/differential/many_native_arguments_host.c");
    host_command[host_count++] = S8("-o");
    host_command[host_count++] = host_object;
    ProcessSpawnResult host_spawn = os_process_spawn((SliceString8){.pointer = host_command, .length = host_count},
        (SliceString8){0}, (SliceString8){0}, (ProcessSpawnOptions){.use_process_environment = true});
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 source = S8("tests/basic_c_many_native_arguments.c");
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-many-arguments"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
                    S8("-fno-machine-fallback"), S8("-fverify-codegen"), S8("-o"), output, source};
                CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena,
                    compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command)));
                String8 description = string_format(temporary.arena, S8("many arguments {S8} {S8} {S8}: {S8}"),
                    targets[target], modes[mode], frontends[frontend], compiled.diagnostic);
                BUSTER_TEST_RAW(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object, description);
                BUSTER_TEST_RAW(arguments, compiled.codegen_statistics.function_count == 7 &&
                    compiled.codegen_statistics.fallback_function_count == 0, description);
#if (BUSTER_CPU_ARCH_AARCH64 || BUSTER_CPU_ARCH_X86_64) && !BUSTER_ANDROID && !BUSTER_IOS
                ...
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    ...
                    if (host_compiled)
                    {
                        ...
                        CompilerDriverResult mixed_result = compiler_driver_execute_invocation(temporary.arena,
                            compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(mixed_command)));
                        BUSTER_TEST_RAW(arguments, mixed_result.error == COMPILER_DRIVER_ERROR_NONE, mixed_result.diagnostic);
                        if (mixed_result.error == COMPILER_DRIVER_ERROR_NONE)
                        {
                            BUSTER_TEST(arguments, compiler_driver_test_process_success(temporary.arena, mixed));
                        }
                    }
#endif
                }
            ...
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                for (u32 pic = 0; pic < BUSTER_ARRAY_LENGTH(pics); pic += 1)
                {
                    for (u32 fixture = 0; fixture < BUSTER_ARRAY_LENGTH(sources); fixture += 1)
                    {
                        ...
                        bool native_target = (target == 0 && BUSTER_LINUX) || (target == 1 && BUSTER_MACOS) || (target == 2 && BUSTER_WINDOWS);
                        // NONE is the MIR_STACK compatibility spelling. This
                        // regression gates the supported MIR allocators against
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_aarch64_platform_variadic(UnitTestArguments* arguments)
{
    ...
    host_command[host_count++] = S8("-fno-asynchronous-unwind-tables");
    host_command[host_count++] = S8("-c");
    host_command[host_count++] = S8("tests/differential/aarch64_platform_variadic_host.c");
    host_command[host_count++] = S8("-o");
    host_command[host_count++] = host_object;
    ProcessSpawnResult host_spawn = os_process_spawn((SliceString8){.pointer = host_command, .length = host_count},
        (SliceString8){0}, (SliceString8){0}, (ProcessSpawnOptions){.use_process_environment = true});
    ...
    BUSTER_TEST(arguments, host_compiled);
#if BUSTER_WINDOWS
    String8 boundary_object = buster_test_temporary_path(arguments->arena, S8("buster-windows-variadic-abi"), S8(".o"));
    host_command[host_count - 3] = S8("tests/differential/aarch64_windows_variadic_boundary.S");
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                for (u32 fixture = 0; fixture < (target ? 2u : 1u); fixture += 1)
                {
                    ...
                    CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena,
                        compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command)));
                    String8 description = string_format(temporary.arena, S8("platform variadic {S8} {S8} {S8} {S8}: {S8}"),
                        targets[target], modes[mode], frontends[frontend], source, compiled.diagnostic);
                    BUSTER_TEST_RAW(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object, description);
                    BUSTER_TEST_RAW(arguments, compiled.codegen_statistics.function_count == (fixture ? 4u : 10u) &&
                        compiled.codegen_statistics.fallback_function_count == 0, description);
#if BUSTER_CPU_ARCH_AARCH64 && (BUSTER_MACOS || BUSTER_WINDOWS) && !BUSTER_IOS
                    ...
                    if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                    {
                        ...
                        if (boundary_compiled && fixture)
                        {
                            ...
                            CompilerDriverResult boundary_result = compiler_driver_execute_invocation(temporary.arena,
                                compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(boundary_command)));
                            BUSTER_TEST_RAW(arguments, boundary_result.error == COMPILER_DRIVER_ERROR_NONE, boundary_result.diagnostic);
                            if (boundary_result.error == COMPILER_DRIVER_ERROR_NONE)
                            {
                                BUSTER_TEST(arguments, compiler_driver_test_process_success(temporary.arena, boundary));
                            }
                        }
#endif
#endif
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_machine_fallback(UnitTestArguments* arguments)
{
    ...
    String8 modes[] = {S8("-fregister-allocator=mir-stack"), S8("-fregister-allocator=fast"), S8("-fregister-allocator=quality")};
    String8 frontends[] = {S8("-fno-frontend-ssa"), S8("-ffrontend-ssa")};
    // NONE is a documented compatibility spelling for MIR_STACK. Keep one
    // positive, debug-free object comparison here so a future dispatch edit
    // cannot quietly restore an independent direct oracle or change side
    // tables while preserving only the success bit.
    {
        TemporalArena temporary = scratch_begin(&arguments->arena, 1);
        String8 aliases[] = {S8("-fregister-allocator=none"), S8("-fregister-allocator=mir-stack")};
        CompilerDriverResult alias_results[2] = {0};
        for (u32 alias = 0; alias < BUSTER_ARRAY_LENGTH(aliases); alias += 1)
        {
            String8 output = buster_test_temporary_path(temporary.arena, alias ? S8("buster-mir-alias-stack") : S8("buster-mir-alias-none"), S8(".o"));
            String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), S8("x86_64-unknown-linux"), aliases[alias], S8("-fverify-codegen"),
                                 S8("-o"), output, S8("tests/basic_c_operations.c")};
            CompilerDriverInvocation invocation = compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command));
            alias_results[alias] = compiler_driver_execute_invocation(temporary.arena, invocation);
            BUSTER_TEST_RAW(arguments, alias_results[alias].error == COMPILER_DRIVER_ERROR_NONE && alias_results[alias].has_object,
                            alias_results[alias].diagnostic);
            BUSTER_TEST(arguments, alias_results[alias].codegen_statistics.verified_mir_function_count != 0);
            BUSTER_TEST(arguments, alias_results[alias].codegen_statistics.fallback_function_count == 0);
        }
        if (alias_results[0].error == COMPILER_DRIVER_ERROR_NONE && alias_results[1].error == COMPILER_DRIVER_ERROR_NONE)
        {
            ObjectFile* none = &alias_results[0].object;
            ObjectFile* mir_stack = &alias_results[1].object;
            BUSTER_TEST(arguments, none->section_count == mir_stack->section_count && none->symbol_count == mir_stack->symbol_count &&
                                        none->relocation_count == mir_stack->relocation_count);
            if (none->section_count == mir_stack->section_count)
            {
                for (u32 section = 0; section < none->section_count; section += 1)
                {
                    BUSTER_TEST(arguments, none->sections[section].kind == mir_stack->sections[section].kind &&
                                                none->sections[section].alignment == mir_stack->sections[section].alignment &&
                                                none->sections[section].virtual_size == mir_stack->sections[section].virtual_size &&
                                                none->sections[section].data.length == mir_stack->sections[section].data.length &&
                                                (!none->sections[section].data.length ||
                                                 !memcmp(none->sections[section].data.pointer, mir_stack->sections[section].data.pointer,
                                                         none->sections[section].data.length)));
                }
            }
            if (none->symbol_count == mir_stack->symbol_count)
            {
                for (u32 symbol = 0; symbol < none->symbol_count; symbol += 1)
                {
                    ObjectSymbol* left = none->symbols + symbol;
                    ObjectSymbol* right = mir_stack->symbols + symbol;
                    BUSTER_TEST(arguments, string_equal(left->name, right->name) && left->value == right->value && left->size == right->size &&
                                                left->section == right->section && left->kind == right->kind && left->global == right->global &&
                                                left->weak == right->weak && left->hidden == right->hidden);
                }
            }
            if (none->relocation_count == mir_stack->relocation_count)
            {
                for (u32 relocation = 0; relocation < none->relocation_count; relocation += 1)
                {
                    ObjectRelocation* left = none->relocations + relocation;
                    ObjectRelocation* right = mir_stack->relocations + relocation;
                    BUSTER_TEST(arguments, left->addend == right->addend && left->offset == right->offset && left->section == right->section &&
                                                left->symbol == right->symbol && left->kind == right->kind);
                }
            }
            BUSTER_TEST(arguments, none->sections[OBJECT_SECTION_TEXT].data.length == mir_stack->sections[OBJECT_SECTION_TEXT].data.length);
        }
        scratch_end(temporary);
    }
    struct
    {
        ...
        {.path = S8("tests/basic_c_asm_goto_range.c")},
            ...
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                ...
                for (u32 fixture = 0; fixture < BUSTER_ARRAY_LENGTH(corpus); fixture += 1)
                {
                    ...
                    if (fallbacks)
                    {
                        ...
                        // A refused MIR row is a structural failure. There is
                        // no direct oracle path and no fallback census row.
    ...
    // Keep the unchanged load/store fixture strict, and promote both broader
    // corpora from their last expected CAS/RMW refusal to zero fallback.
        ...
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(frontends); frontend += 1)
            {
                ...
                String8 census_command[] = {S8("-c"), S8("-g0"), S8("-target"), atomic_pair_targets[target], modes[mode], frontends[frontend],
                                            S8("-fno-machine-fallback"), S8("-fverify-codegen"), S8("-o"), census_output, S8("tests/basic_c_atomic_aggregate.c")};
                ...
                BUSTER_TEST_RAW(arguments, census.codegen_statistics.function_count != 0 &&
                                               census.codegen_statistics.fallback_function_count == 0 &&
                                               census.codegen_statistics.fallback_reason_counts[CODEGEN_FALLBACK_OPCODE] == 0 &&
                                               census.codegen_statistics.fallback_opcode_counts[IR_OPCODE_ATOMIC_COMPARE_EXCHANGE] == 0 &&
                ...
                String8 int128_command[] = {S8("-c"), S8("-g0"), S8("-target"), atomic_pair_targets[target], modes[mode], frontends[frontend],
                                            S8("-fno-machine-fallback"), S8("-fverify-codegen"), S8("-o"), int128_output, S8("tests/basic_c_int128.c")};
                ...
                BUSTER_TEST_RAW(arguments, int128_census.codegen_statistics.function_count != 0 &&
                                               int128_census.codegen_statistics.fallback_function_count == 0 &&
                                               int128_census.codegen_statistics.fallback_reason_counts[CODEGEN_FALLBACK_OPCODE] == 0 &&
                                               int128_census.codegen_statistics.fallback_opcode_counts[IR_OPCODE_ATOMIC_READ_MODIFY_WRITE] == 0 &&
    ...
    // New updates share one source. Strict object coverage is cross-target;
    // only the matching native desktop executes it.
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(atomic_pair_targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(atomic_update_modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-a64-atomic-update"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), atomic_pair_targets[target], atomic_update_modes[mode],
                ...
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-a64-atomic-update-run"), S8(".exe"));
                    String8 native_command[] = {atomic_update_modes[mode], frontends[frontend], S8("-fverify-codegen"), S8("-o"), executable,
                                                S8("tests/basic_c_aarch64_atomic_update_pair.c")};
                    CompilerDriverInvocation native_invocation = compiler_driver_parse_arguments(temporary.arena,
                        (SliceString8)BUSTER_ARRAY_TO_SLICE(native_command));
                    native_invocation.reject_machine_fallback = mode != 0;
    ...
    // frontend forms and execute only on the actual
    ...
    // Keep the independent host oracle honest while it checks MIR output:
    ...
    // This inline-assembly fixture is deliberately refused by the MIR emitter;
    // the former wide-signature fixture is now a supported MIR ABI case.
    ...
    for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(failure_modes); mode += 1)
    {
        ...
        ByteSlice sentinel = {.pointer = (u8*)"existing-output", .length = 15};
        BUSTER_TEST(arguments, file_write(output, sentinel));
        String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), S8("x86_64-unknown-windows"), failure_modes[mode],
        ...
        if (compiled.diagnostic_count == 1)
        {
            ...
            if (diagnostic.backend)
            {
                ...
                BUSTER_TEST(arguments, string_equal(diagnostic.backend->function, S8("main")));
        ...
        BUSTER_TEST(arguments, string_first_sequence(compiled.diagnostic, S8("function='main'")) < compiled.diagnostic.length);
    ...
    }
    {
        ...
        String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), S8("x86_64-unknown-windows"), S8("-fno-machine-fallback"),
                             S8("-o"), output, S8("tests/basic_c_asm_literal_register.c")};
        CompilerDriverInvocation invocation = compiler_driver_parse_arguments(temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(command));
        CompilerDriverResult compiled = compiler_driver_execute_invocation(temporary.arena, invocation);
        ...
        BUSTER_TEST(arguments, string_first_sequence(compiled.diagnostic, S8("allocator=fast")) < compiled.diagnostic.length);
        BUSTER_TEST(arguments, string_first_sequence(compiled.diagnostic, S8("kind=codegen.unsupported-instruction")) < compiled.diagnostic.length);
        BUSTER_TEST(arguments, string_first_sequence(compiled.diagnostic, S8("reason=opcode")) < compiled.diagnostic.length);
        scratch_end(temporary);
    }
    ...
    for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
    {
        ...
        String8 command[] = {S8("-g0"), S8("-target"), S8("x86_64-unknown-windows"), modes[mode], S8("-o"), output,
                             S8("tests/basic_c_multi_add.c"), S8("tests/basic_c_asm_literal_register.c"),
...
BUSTER_GLOBAL_LOCAL UnitTestResult compiler_driver_test_aarch64_i128_to_float(UnitTestArguments* arguments)
{
    ...
    for (u32 target = 0; target < BUSTER_ARRAY_LENGTH(targets); target += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(modes); mode += 1)
        {
            ...
            {
                TemporalArena temporary = scratch_begin(&arguments->arena, 1);
                String8 output = buster_test_temporary_path(temporary.arena, S8("buster-a64-i128_to_float"), S8(".o"));
                String8 command[] = {S8("-c"), S8("-g0"), S8("-target"), targets[target], modes[mode], frontends[frontend],
                ...
                if (native_target && compiled.error == COMPILER_DRIVER_ERROR_NONE)
                {
                    String8 executable = buster_test_temporary_path(temporary.arena, S8("buster-a64-i128_to_float-run"), S8(".exe"));
                    String8 native_command[] = {modes[mode], frontends[frontend], S8("-fverify-codegen"), S8("-o"), executable,
...
#undef BUSTER_DRIVER_TEST_MODE
    ...
    String8 census_disabled_command[] = {S8("-fcodegen-fallback-census"), S8("-fno-codegen-fallback-census"), S8("source.c")};
    CompilerDriverInvocation census_disabled = compiler_driver_parse_arguments(arguments->arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(census_disabled_command));
    BUSTER_TEST(arguments, census_disabled.error == COMPILER_DRIVER_ERROR_NONE && !census_disabled.record_codegen_fallbacks);
    String8 census_llvm_command[] = {S8("-fcodegen-fallback-census"), S8("-emit-llvm"), S8("source.c")};
    CompilerDriverInvocation census_llvm = compiler_driver_parse_arguments(arguments->arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(census_llvm_command));
    BUSTER_TEST(arguments, census_llvm.error == COMPILER_DRIVER_ERROR_ARGUMENT);
    ...
    BUSTER_TEST(arguments, strict_none.error == COMPILER_DRIVER_ERROR_NONE);
    BUSTER_TEST(arguments, strict_none.register_allocator == CODEGEN_REGISTER_ALLOCATOR_NONE);
    BUSTER_TEST(arguments, strict_none.register_allocator_explicit && strict_none.reject_machine_fallback);
    ...
    // direct i128 return ABI. Compile it through every allocator mode: every
    // supported native mode now uses the MIR machine path.
    {
    ...
    // 256-bit vector arguments through the default and MIR_STACK-compatible
    // allocator spellings. Both lanes exercise the same MIR machine path.
    ...
    // wrong on AArch64 and no fixture executed them: the MIR transfer had no
    // sized transfer for one- and two-byte parts and left the parameter slot
    // zero, and the machine callee's stack read-backs could land on an
    // argument register whose own capture had not run yet. Run both allocator
    // spellings through the same MIR path.
    ...
    // must reach an object at both models through both allocator spellings.
    {
    ...
    // reach an object through both allocator spellings. The wine block below runs the
    ...
    String8 c_cpython_shape_fixtures[] = {
        ...
        S8("tests/basic_c_inline_hints.c"),
        S8("tests/basic_c_asm_identity.c"),
        S8("tests/basic_c_asm_goto_identity.c"),
    ...
    String8 asm_goto_fixtures[] = {
        S8("tests/basic_c_asm_goto_identity.c"),
#if BUSTER_CPU_ARCH_AARCH64
        S8("tests/basic_c_asm_goto_range.c"),
#endif
    };
    for (u32 fixture = 0; fixture < BUSTER_ARRAY_LENGTH(asm_goto_fixtures); fixture += 1)
    {
        for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(asm_goto_modes); mode += 1)
        {
            for (u32 frontend = 0; frontend < BUSTER_ARRAY_LENGTH(asm_goto_frontends); frontend += 1)
            {
...
#if BUSTER_WINDOWS
                                                                    string_format(asm_goto_temporary.arena, S8("-{u32}-{u32}-{u32}.exe"),
                                                                                  fixture, mode, frontend));
#else
                                                                    string_format(asm_goto_temporary.arena, S8("-{u32}-{u32}-{u32}"),
                                                                                  fixture, mode, frontend));
#endif
                String8 asm_goto_command[] = {asm_goto_modes[mode], asm_goto_frontends[frontend], S8("-fno-machine-fallback"),
                                              S8("-fverify-codegen"), S8("-o"), asm_goto_path, asm_goto_fixtures[fixture]};
    ...
    CompilerDriverResult asm_goto_range = compiler_driver_execute_invocation(
        arguments->arena, compiler_driver_parse_arguments(arguments->arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(asm_goto_range_command_line)));
    BUSTER_TEST(arguments, asm_goto_range.error == COMPILER_DRIVER_ERROR_NONE && asm_goto_range.has_object);
    bool asm_goto_range_expanded = false;
    if (asm_goto_range.has_object)
    {
        ByteSlice text = asm_goto_range.object.sections[OBJECT_SECTION_TEXT].data;
        for (u32 symbol_index = 0; !asm_goto_range_expanded && symbol_index < asm_goto_range.object.symbol_count; symbol_index += 1)
        {
            ObjectSymbol* symbol = asm_goto_range.object.symbols + symbol_index;
            if (!string_equal(symbol->name, S8("asm_goto_test_range")) || symbol->section != OBJECT_SECTION_TEXT ||
                symbol->value > text.length || symbol->size > text.length - symbol->value)
            {
                continue;
            }
            u64 function_end = symbol->value + symbol->size;
            for (u64 offset = symbol->value + sizeof(u32); !asm_goto_range_expanded && offset + 4u * sizeof(u32) <= function_end;
                 offset += sizeof(u32))
            {
                u32 forward_internal = 0;
                u32 inverse_test = 0;
                u32 long_branch = 0;
                u32 backward_internal = 0;
                memcpy(&forward_internal, text.pointer + offset - sizeof(u32), sizeof(forward_internal));
                memcpy(&inverse_test, text.pointer + offset, sizeof(inverse_test));
                memcpy(&long_branch, text.pointer + offset + sizeof(u32), sizeof(long_branch));
                memcpy(&backward_internal, text.pointer + offset + 3u * sizeof(u32), sizeof(backward_internal));
                bool skip = (inverse_test & UINT32_C(0x7f000000)) == UINT32_C(0x37000000) &&
                            ((inverse_test >> 5) & UINT32_C(0x3fff)) == 2u;
                bool branch = (long_branch & UINT32_C(0xfc000000)) == UINT32_C(0x14000000);
                s64 displacement = (s64)((s32)((long_branch & UINT32_C(0x03ffffff)) << 6) >> 4);
                s64 target = (s64)(offset + sizeof(u32)) + displacement;
                s64 forward_displacement = (s64)((s32)((forward_internal & UINT32_C(0x03ffffff)) << 6) >> 4);
                s64 forward_target = (s64)(offset - sizeof(u32)) + forward_displacement;
                s64 backward_displacement = (s64)((s32)((backward_internal & UINT32_C(0x03ffffff)) << 6) >> 4);
                s64 backward_target = (s64)(offset + 3u * sizeof(u32)) + backward_displacement;
                bool internal_labels = (forward_internal & UINT32_C(0xfc000000)) == UINT32_C(0x14000000) &&
                                       (backward_internal & UINT32_C(0xfc000000)) == UINT32_C(0x14000000) &&
                                       forward_target == (s64)(offset + 3u * sizeof(u32)) &&
                                       backward_target == (s64)(offset - 2u * sizeof(u32));
                asm_goto_range_expanded = skip && branch && internal_labels && target >= (s64)symbol->value &&
                                          target < (s64)function_end && target - (s64)offset > INT32_C(0x7ffc);
            }
        }
    }
    BUSTER_TEST(arguments, asm_goto_range_expanded);
    buster_test_arena_end(arguments, driver_fixture, true);
    ...
    }
    {
        struct
        {
            String8 target;
            String8 source;
        } unsupported_asm_goto_controls[] = {
            {S8("x86_64-unknown-linux-gnu"),
             S8("int f(void) { int r = 0; __asm__ goto (\"call %l0\" : : : : target); goto done; target: r = 1; done: return r; }\n")},
            {S8("x86_64-unknown-linux-gnu"),
             S8("int f(void) { int r = 0; __asm__ goto (\"xbegin %l0\" : : : : target); goto done; target: r = 1; done: return r; }\n")},
            {S8("aarch64-unknown-linux-gnu"),
        ...
        {
            String8 source_path = buster_test_temporary_path(arguments->arena, S8("buster-unsupported-asm-goto-control"),
                                                              string_format(arguments->arena, S8("-{u32}.c"), control));
            String8 object_path = buster_test_temporary_path(arguments->arena, S8("buster-unsupported-asm-goto-control"),
    ...
    if (c_thread_local_aarch64.error == COMPILER_DRIVER_ERROR_NONE)
    {
        ...
        // resolved immediate, both on one register; the register is whatever
        // the MIR machine path allocated, so the probe matches the pair's
        // form rather than one fixed word.
    ...
    // machine ABI boundary against the actual AArch64 ABI.
    ...
    //
    // The native success and zero-fallback counters are pinned so a selector
    // that stopped firing cannot silently retire the coverage.
    ...
    // Older machine selectors refused it and a separate lowering loaded
    // through it, so the program was right and the census was not; the bitcode
    // writer emitted an invalid getelementptr for the same shape. The current
    // machine path now owns this row and pins a native result.
    //
    ...
    //
    // Both pin the zero-fallback count so the machine path remains the
    // producer. Both build for both targets and run wherever there is
    ...
    // the machine placement must count the same padding eightbytes as the
    // ABI model and clang — its callee used to read a stacked
    ...
    // ever turn into a refusal, which would silently retire this coverage.
    // On an x86-64 Linux host each image also runs.
    ...
    for (u32 allocator_index = 0; allocator_index < BUSTER_ARRAY_LENGTH(x64_i128_stack_allocators); allocator_index += 1)
    {
        ...
        // The caller's aligned i128 stack pair leaves an eightbyte hole.
        // Its complete argument area now stays on MIR with the callees.
        BUSTER_TEST(arguments, x64_i128_stack.codegen_statistics.fallback_function_count == 0);
    ...
    String8 c_vector_path = buster_test_temporary_path(arguments->arena, S8("buster-c-vector"),
                                                       ...
                                                       S8(""));
#endif
    // These counters describe MIR packing/forwarding choices. Keep this
    // independent object observer explicit; the strict MIR execution matrix
    ...
    if (target_native.cpu_arch == CPU_ARCH_X86_64)
    {
        ...
        if (wine_clang_available)
        {
            ...
            if (wine_clang_available)
            {
                ...
                for (u32 half = 0; half < BUSTER_ARRAY_LENGTH(wide_defines); half += 1)
                {
                    ...
                    ProcessSpawnResult spawn = os_process_spawn((SliceString8)BUSTER_ARRAY_TO_SLICE(clang_command), (SliceString8){0},
                                                                 (SliceString8){0}, wine_options);
                    wide_clang_ready &= spawn.handle && os_process_wait_sync(wine_mixed_arena, spawn).result == PROCESS_RESULT_SUCCESS;
                }
                BUSTER_TEST(arguments, wide_clang_ready);
                for (u32 mode = 0; wide_clang_ready && mode < BUSTER_ARRAY_LENGTH(wide_modes); mode += 1)
                {
                    for (u32 direction = 0; direction < 2; direction += 1)
                    {
                        TemporalArena wide_temporary = scratch_begin(&wine_mixed_arena, 1);
                        String8 buster_object = buster_test_temporary_path(wide_temporary.arena, S8("buster-win64-wide-mir"), S8(".obj"));
                        String8 executable = buster_test_temporary_path(wide_temporary.arena, S8("buster-win64-wide-mixed"), S8(".exe"));
                        String8 compile_command[] = {S8("-c"), S8("-g0"), S8("-target"), S8("x86_64-pc-windows-msvc"), S8("-march=baseline"),
                            wide_modes[mode], S8("-fno-machine-fallback"), S8("-fverify-codegen"), wide_defines[1u - direction],
                            S8("tests/basic_c_vector_argument_wide.c"), S8("-o"), buster_object};
                        CompilerDriverResult compiled = compiler_driver_execute_invocation(wide_temporary.arena,
                            compiler_driver_parse_arguments(wide_temporary.arena, (SliceString8)BUSTER_ARRAY_TO_SLICE(compile_command)));
                        BUSTER_TEST(arguments, compiled.error == COMPILER_DRIVER_ERROR_NONE && compiled.has_object &&
                            compiled.codegen_statistics.fallback_function_count == 0);
                        if (compiled.error == COMPILER_DRIVER_ERROR_NONE)
                        {
    ...
    // Keep the full vector corpus, then require zero MIR fallback for the
    // scalar signature slice under both frontend representations.
    for (u32 stack_variant = 0; stack_variant < 3; stack_variant += 1)
    {
        ...
        for (u64 allocator_index = 0; host_stack_compiled && allocator_index < BUSTER_ARRAY_LENGTH(c_lz4_regression_allocators); allocator_index += 1)
        {
            ...
            for (u32 source_index = 0; source_index < BUSTER_ARRAY_LENGTH(buster_stack_objects); source_index += 1)
            {
                ...
                BUSTER_TEST_RAW(arguments, buster_stack.error == COMPILER_DRIVER_ERROR_NONE, buster_stack.diagnostic);
                if (stack_variant)
                {
                    BUSTER_TEST(arguments, buster_stack.codegen_statistics.function_count > 0 &&
                                           buster_stack.codegen_statistics.fallback_function_count == 0);
                }
    ...
    // under every allocator. Supported operations select into MIR; an
    // unsupported ABI or opcode must be a structured refusal with no output.
    ...
    // The bounded MIR vocabulary must remain selected, including calls
    // through function pointers whose relocations the standalone link owns.
    for (u32 mode = 0; mode < BUSTER_ARRAY_LENGTH(c_long_double_allocators); mode += 1)
    {
        for (u32 memory = 0; memory < 2; memory += 1)
        {
            ...
            if (compiled.error == COMPILER_DRIVER_ERROR_NONE)
            {
                ...
                if (process.handle)
                {
                    BUSTER_TEST(arguments, os_process_wait_sync(temporary.arena, process).result == PROCESS_RESULT_SUCCESS);
                }
            }
            scratch_end(temporary);
        }
    ...
    // that a function carrying an f80 either succeeds through MIR or reports
    // its structured unsupported ABI/opcode reason.
    // The fixture compiles to an empty program wherever `long double` is not
    // the x87 format, so it stays registered on every host.
    for (u64 allocator_index = 0; allocator_index < BUSTER_ARRAY_LENGTH(c_long_double_allocators); allocator_index += 1)
    {
    ...
    // checked for is that a function carrying an f80 either succeeds through
    // MIR or reports a structured unsupported ABI/opcode reason. The fixture
    // compiles to an empty program wherever
    ...
    String8 c_quickjs_regression_names[] = {
        ...
        S8("buster-c-macro-task-batches"),
    ...
    // The zero-fallback count is the assertion that says the byte rows are
    // reached. It is pinned under every allocator, including NONE's MIR_STACK
    // compatibility spelling, because nothing in this fixture has any other
    // reason to refuse.
    ...
    // An inline-assembly template the emitter refuses must retain its
    // structured opcode reason (#831). The refusal itself is by design, so the
    ...
    BUSTER_TEST(arguments, string_first_sequence(c_asm_literal_register.diagnostic, S8("kind=codegen.unsupported-instruction")) != BUSTER_STRING_NO_MATCH);
    BUSTER_TEST(arguments, string_first_sequence(c_asm_literal_register.diagnostic, S8("reason=opcode")) != BUSTER_STRING_NO_MATCH);
    ...
    {
        BUSTER_TEST(arguments, c_call_abi.codegen_statistics.fallback_function_count == 0);
    ...
    // the MIR machine path explicitly; all allocator modes are covered by
    ...
    for (u32 windows_machine_source = 0; windows_machine_source < BUSTER_ARRAY_LENGTH(windows_machine_sources); windows_machine_source += 1)
    {
        for (u32 windows_machine_index = 0; windows_machine_index < BUSTER_ARRAY_LENGTH(windows_machine_modes); windows_machine_index += 1)
        {
            ...
            if (windows_machine.error == COMPILER_DRIVER_ERROR_NONE)
            {
                ...
                BUSTER_TEST(arguments, windows_machine.codegen_statistics.fallback_function_count == 0);
    ...
    // MIR allocator because the operand's frame slot is placed differently by each.
#if BUSTER_LINUX && BUSTER_CPU_ARCH_X86_64
    {
    ...
    // what the assembler actually encoded.
    {
        ...
        // the -O flag selects the MIR allocator. NONE remains an alias for
        // MIR_STACK, and no mode records a direct-emitter fallback.
        ...
        // configure's own -O3.)
        {
            ...
            if (sticky_none.error == COMPILER_DRIVER_ERROR_NONE && sticky_fast.error == COMPILER_DRIVER_ERROR_NONE)
            {
                ...
                bool fast_request_observable = !identical;
                BUSTER_TEST(arguments, sticky_none.codegen_statistics.fallback_function_count == 0);
                BUSTER_TEST(arguments, sticky_fast.codegen_statistics.fallback_function_count == 0);
