...
static bool a64_alias_projection_string_is(String8 actual, char8 const* expected)
{
    ...
    u32 length = 0;
    while (expected[length]) length += 1;
    if (actual.length != length) return false;
    for (u32 index = 0; index < length; index += 1)
        if (actual.pointer[index] != expected[index]) return false;
    return true;
}
...
UnitTestResult aarch64_alias_projection_tests(UnitTestArguments* arguments)
{
    ...
    BUSTER_TEST(arguments, a64_alias_projection_string_is(buster_a64_alias_denominator_sha256(),
                                                            "fa030a65c5be661813f50a92f3027e152ab9c1b55701f851c6142390df5c25a8"));
    ...
    for (u32 ordinal = 0; ordinal < buster_a64_alias_count(); ordinal += 1)
    {
        ...
        if (rows_valid && row.target_owner < 7) owner_counts[row.target_owner] += 1;
    ...
    BUSTER_TEST(arguments, owner_counts[0] == 64 && owner_counts[1] == 59 && owner_counts[2] == 21 && owner_counts[3] == 10 &&
                              owner_counts[4] == 9 && owner_counts[5] == 7 && owner_counts[6] == 2);
    ...
    BUSTER_TEST(arguments, encode_status == BUSTER_A64_ALIAS_STATUS_OK && word == UINT32_C(0x1ac20820));
    BusterA64AliasResult decoded = {0};
    BUSTER_TEST(arguments, buster_a64_alias_decode_row(target, asr_form, word, &decoded) == BUSTER_A64_ALIAS_STATUS_OK &&
                              decoded.alias_form_id == asr_form && decoded.operands[0].payload == 0 && decoded.operands[2].payload == 2);
