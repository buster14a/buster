...
UnitTestResult aarch64_stride_tests(UnitTestArguments* arguments)
{
    ...
    for (u32 case_index = 0; case_index < BUSTER_ARRAY_LENGTH(stride_cases); case_index += 1)
    {
        ...
        BUSTER_TEST(arguments, descriptor && descriptor->code_size != 0 &&
                                   (u64)descriptor->code_offset + descriptor->code_size <= generated.code.length);
        BUSTER_TEST(arguments, generated.statistics.fallback_function_count == 0);
        // The public module above and the independent selector below now reach
        // the same MIR encoder. Keep the public transaction/descriptor checks
        // separate from this exact materialization oracle so a successful but
        // truncated module cannot satisfy the instruction-shape assertion.
