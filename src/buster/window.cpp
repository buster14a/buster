...
BUSTER_F_IMPL OsWindowingHandle* os_windowing_initialize()
{
...
BUSTER_F_IMPL void os_windowing_deinitialize(OsWindowingHandle* windowing)
{
#if defined(__linux__)
...
BUSTER_F_IMPL OsWindowHandle* os_window_create(OsWindowingHandle* windowing, OsWindowCreate create)
{
...
BUSTER_F_IMPL OsWindowSize os_window_get_framebuffer_size(OsWindowingHandle* windowing, OsWindowHandle* os_window)
{
    OsWindowSize result = {};
...
BUSTER_F_IMPL void* native_windowing_handle_from_os_windowing_handle(OsWindowingHandle* windowing)
{
    return windowing->connection;
}

BUSTER_F_IMPL void* native_window_handle_from_os_window_handle(OsWindowHandle* window)
{
    return window;
}

BUSTER_F_IMPL OsWindowingEventList os_windowing_poll_events(Arena* arena, OsWindowingHandle* windowing)
{
            ...
            break; case XCB_CLIENT_MESSAGE: // 33
            {
                ...
                if (client_message_event->data.data32[0] == atom_replies[X11_ATOM_WM_DELETE_WINDOW]->atom)
                {
                    ...
                    *os_event = (OsWindowingEvent) {
                        .kind = OsWindowingEventKind::OS_WINDOWING_EVENT_WINDOW_CLOSE,
