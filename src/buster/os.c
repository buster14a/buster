...
BUSTER_IMPL ProcessHandle* os_process_spawn(StringOs first_argument, StringOsList argv, StringOsList envp)
{
    ...
    {
        let spawn_result = posix_spawn(&pid, first_argument.pointer, &file_actions, &attributes, (char**)argv, (char**)envp);
...
BUSTER_IMPL StringOs os_get_environment_variable(StringOs variable)
{
    ...
    while (*it)
    {
        ...
        let full_env = os_string_from_pointer_length(it, length);
        it += length + 1;
        let key_index = string_first_code_point(full_env, '=');
        let key = os_string_from_pointer_length(full_env.pointer, key_index);
        if (string_equal(key, variable))
        {
            result = os_string_from_pointer_length(full_env.pointer + (key_index + 1), full_env.length - (key_index + 1));
            break;
        }
    }
#else
