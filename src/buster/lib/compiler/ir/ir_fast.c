...
// input, promotion-output and FAST-output certification boundaries.
...
BUSTER_GLOBAL_LOCAL void ir_fast_function(IrProgram* program, IrFunction* function, IrFastStatistics* statistics)
{
    ...
    else if (scratch_bytes > IR_FAST_SCRATCH_BUDGET || retained_bytes > IR_FAST_RETAINED_BUDGET || work > IR_FAST_WORK_BUDGET) statistics->budget_skips += 1;
    else
    {
