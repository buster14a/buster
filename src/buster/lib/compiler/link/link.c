...
// The freestanding shape without initializers has a fixed size. Hosted
// shapes also register the loader finalizer; both architectures report their
// actual stub size so the shared ELF layout can reserve the larger one.
enum
{
...
// entry's target at layout time, so it calls them directly rather than laying
// the arrays out and looping over them.  The two array sections are dropped
// from the image for exactly that reason: nothing in it would read them.
//
// `.fini_array` runs backwards, which is the order `destructors` is already
// in: the plan reverses it while it is built so every consumer walks both
// lists forwards.
typedef struct LinkInitializerPlan LinkInitializerPlan;
struct LinkInitializerPlan
{
    LinkInitializerEntry* constructors;
...
// place every non-debug section they are given, and an array nothing reads is
// storage the image does not need.  The object is copied rather than edited,
// because a caller may link the same merged file twice -- the AArch64 dynamic
// writer does exactly that.
BUSTER_GLOBAL_LOCAL bool link_initializer_plan_build(Arena* arena, ObjectFile* object, ObjectFile* stripped, LinkInitializerPlan* plan)
{
    bool result = true;
    *stripped = *object;
    *plan = (LinkInitializerPlan){0};
    ...
    if (total)
    {
        u64 capacity = total / OBJECT_INITIALIZER_ENTRY_SIZE;
        ...
        {
            plan->constructors = arena_allocate(arena, LinkInitializerEntry, capacity ? capacity : 1);
            plan->destructors = plan->constructors + object->sections[OBJECT_SECTION_INIT_ARRAY].data.length / OBJECT_INITIALIZER_ENTRY_SIZE;
            plan->constructor_count = link_initializer_entries_collect(object, OBJECT_SECTION_INIT_ARRAY, plan->constructors, false);
...
// The ELF loader supplies its finalizer in RDX. Register it before the
// executable's destructor runner and before any constructor can register a
// user handler, so reverse exit order leaves the shared libraries until last.
BUSTER_GLOBAL_LOCAL bool link_x86_register_loader_finalizer(LinkX86InstructionBuilder* builder, u32* call_offset)
{
    BusterX86MetadataPhysicalOperand operands[2] = {link_x86_register(2, 64), link_x86_register(2, 64)};
    bool valid = link_x86_emit(builder, S8("TEST"), operands, 2);
    operands[0] = link_x86_relative(0, 32);
    valid = valid && link_x86_emit(builder, S8("JZ"), operands, 1);
    u32 skip_field = builder->count >= 4 ? builder->count - 4 : 0;
    operands[0] = link_x86_register(7, 64);
    operands[1] = link_x86_register(2, 64);
    valid = valid && link_x86_emit(builder, S8("MOV"), operands, 2);
    operands[0] = link_x86_register(6, 32);
    operands[1] = link_x86_register(6, 32);
    valid = valid && link_x86_emit(builder, S8("XOR"), operands, 2);
    operands[0] = link_x86_register(2, 32);
    operands[1] = link_x86_register(2, 32);
    valid = valid && link_x86_emit(builder, S8("XOR"), operands, 2);
    if (call_offset) *call_offset = builder->count + 1;
    operands[0] = link_x86_relative(0, 32);
    valid = valid && link_x86_emit(builder, S8("CALL"), operands, 1);
    if (valid)
    {
        link_write_u32(builder->bytes, skip_field, builder->count - (skip_field + 4));
    }
    return valid;
}
...
//
// Hosted startup also registers the loader's finalizer even when the program
// has no initializer arrays of its own.
BUSTER_GLOBAL_LOCAL bool link_x86_build_elf_entry_stub(u8* bytes, u32 capacity, u32* byte_count, u32* call_displacement_offset, bool hosted,
                                                       u32* exit_displacement_offset, u32* atexit_displacement_offset, u32* loader_atexit_offset, LinkInitializerPlan const* plan,
                                                       u32* initializer_displacement_offsets)
{
    ...
    bool registered = hosted || constructor_count || destructor_count;
    bool deferred_destructors = link_initializer_plan_defers_destructors(hosted, plan);
    // rbx, r12 and r13 preserve the arguments across runtime registration and
    // initializer calls. In hosted startup this also keeps incoming RDX intact
    // until link_x86_register_loader_finalizer consumes the loader finalizer.
    ...
    if (!link_x86_emit(&builder, S8("AND"), operands, 2) ||
        (hosted && !link_x86_register_loader_finalizer(&builder, loader_atexit_offset))) return false;
    ...
    {
        operands[0] = link_x86_register(7, 64);
        operands[1] = link_x86_register(argc_register, 64);
        if (!link_x86_emit(&builder, S8("MOV"), operands, 2)) return false;
        operands[0] = link_x86_register(6, 64);
        operands[1] = link_x86_register(argv_register, 64);
        if (!link_x86_emit(&builder, S8("MOV"), operands, 2)) return false;
...
BUSTER_GLOBAL_LOCAL u32 link_aarch64_elf_entry_stub_words(bool hosted, LinkInitializerPlan const* plan)
{
    ...
    return 4 + (hosted ? 4u : 0u) + (deferred ? 4u : 0u) + (registered ? 3u : 0u) + constructor_count * 4 + 1 + inline_destructors + (hosted ? 1u : 2u) + 1 + runner;
}
...
BUSTER_GLOBAL_LOCAL u32 link_aarch64_build_elf_entry_stub(u32* words, u32 capacity, bool hosted, LinkInitializerPlan const* plan, u32* entry_word,
                                                          u32* exit_word, u32* atexit_word, u32* loader_atexit_word, u32* initializer_words)
{
    u32 count = 0;
    u32 constructor_count = plan ? plan->constructor_count : 0;
    u32 destructor_count = plan ? plan->destructor_count : 0;
    ...
    u32 argv_register = registered ? 20u : 1u;
    u32 envp_register = registered ? 21u : 2u;
    u32 initializer_index = 0;
    if (capacity >= link_aarch64_elf_entry_stub_words(hosted, plan))
    {
        words[count++] = 0xf94003e0u | argc_register;
        ...
        if (hosted)
        {
            // X0 still holds the loader finalizer: the argument reads above
            // use X19-X21. Skip a null finalizer, then __cxa_atexit(X0, 0, 0).
            words[count++] = 0xb4000080u;
            words[count++] = 0xd2800001u;
            words[count++] = 0xd2800002u;
            if (loader_atexit_word) *loader_atexit_word = count;
        ...
        for (u32 constructor = 0; constructor < constructor_count; constructor += 1)
        {
            ...
            words[count++] = 0xaa0003e0u | (argv_register << 16) | 1u;
            words[count++] = 0xaa0003e0u | (envp_register << 16) | 2u;
...
// The AArch64 dynamic writer overlays the x86-64 writer's image. Reserve
// whichever complete startup shape is larger: their registration, constructor
// and destructor sequences have different fixed and per-entry sizes.
...
// Windows convention for an initializer is MSVC's `.CRT$XCU` one, which takes
// none, and there is no argc or argv to hand over at that point anyway.  That
// is the one way this differs from the ELF stub above.  The prologue is left
// exactly as it was because the unwind record beside it describes that and
// nothing else; `sub rsp, 0x38` also leaves the stack aligned and reserves
                                                      ...
                                                      u32* atexit_displacement_offset, u32* initializer_displacement_offsets)
{
    ...
    if (startup_displacement_offsets) startup_displacement_offsets[0] = builder.count + 2;
    if (!link_x86_emit(&builder, S8("CALL"), &indirect, 1)) return false;
    if (startup_displacement_offsets) startup_displacement_offsets[1] = builder.count + 2;
    if (!link_x86_emit(&builder, S8("CALL"), &indirect, 1)) return false;
    operands[0] = link_x86_register(0, 32);
    ...
    operands[1] = link_x86_register(0, 32);
    if (!link_x86_emit(&builder, S8("MOV"), operands, 2)) return false;
    if (startup_displacement_offsets) startup_displacement_offsets[2] = builder.count + 2;
    if (!link_x86_emit(&builder, S8("CALL"), &indirect, 1)) return false;
    operands[0] = link_x86_register(2, 64);
...
BUSTER_GLOBAL_LOCAL u32 link_aarch64_pe_entry_stub_words(LinkInitializerPlan const* plan)
{
    ...
    // Two saved-register pairs, optional destructor registration, the
    // constructors, UCRT argv setup, main/exit, and the trap. A deferred
    // destructor runner has the same two-pair frame and restores both pairs.
    return 12 + constructor_count + (deferred ? 7u + destructor_count : 0u);
}
...
BUSTER_GLOBAL_LOCAL u32 link_aarch64_build_pe_entry_stub(u32* words, u32 capacity, LinkInitializerPlan const* plan, u32* entry_word, u32* exit_word,
                                                         u32* atexit_word, u32* startup_words, u32* initializer_words, u32* runner_word)
{
    u32 count = 0;
    u32 constructor_count = plan ? plan->constructor_count : 0;
    u32 destructor_count = plan ? plan->destructor_count : 0;
    ...
    {
        // Preserve x19 across the UCRT queries so argc survives the argv
        // query. The pair shapes also have compact, exact ARM64 unwind codes.
        words[count++] = 0xa9bf53f3u;
        words[count++] = 0xa9bf7bfdu;
        if (deferred)
        {
        ...
        // UCRT owns the process command-line conversion. Configure narrow
        // argv, obtain argc and argv through its stable accessor exports, and
        // only then enter the C program with the platform ABI registers.
        words[count++] = 0x52800020u;
        if (startup_words) startup_words[0] = count;
        words[count++] = 0x94000000u;
        if (startup_words) startup_words[1] = count;
        words[count++] = 0x94000000u;
        words[count++] = 0xb9400013u;
        if (startup_words) startup_words[2] = count;
        words[count++] = 0x94000000u;
        words[count++] = 0xf9400001u;
        words[count++] = 0x2a1303e0u;
        if (entry_word) *entry_word = count;
        words[count++] = 0x94000000u;
        if (exit_word) *exit_word = count;
        ...
        {
            words[runner_adr_word] = link_aarch64_adr(0, ((s64)count - (s64)runner_adr_word) * (s64)sizeof(u32), &valid);
            if (runner_word) *runner_word = count;
            words[count++] = 0xa9bf53f3u;
            words[count++] = 0xa9bf7bfdu;
            for (u32 destructor = 0; destructor < destructor_count; destructor += 1)
            {
...
// The writable segment must obey the object's alignment as well as page
// alignment. Align its virtual address rather than just its file offset:
// requested alignments may exceed the fixed image base itself.
BUSTER_GLOBAL_LOCAL bool link_elf_virtual_align(u64 start, u64 alignment, u64* offset)
{
    u64 address = 0;
    bool valid = link_u64_add(ELF_IMAGE_BASE, start, &address) && link_u64_align_forward(address, alignment, &address);
    if (valid)
    {
        *offset = address - ELF_IMAGE_BASE;
    }
    return valid;
}
...
// The name of the first initializer a refusing writer could not place, so the
// diagnostic points at a function in the program rather than at the format.
BUSTER_GLOBAL_LOCAL String8 link_initializer_plan_first_name(ObjectFile* object, LinkInitializerPlan const* plan)
{
    String8 result = {0};
    u32 symbol = plan->constructor_count ? plan->constructors[0].symbol : plan->destructor_count ? plan->destructors[0].symbol : UINT32_MAX;
    if (symbol < object->symbol_count)
    {
        result = object->symbols[symbol].name;
    }

    return result;
}

// Patches the rel32 of every initializer call the entry stub emitted.  Each
// target is the symbol that initializer's array slot named plus that slot's
// addend, at the image address this layout gave it -- the same arithmetic the
// call to `main` beside it takes.
BUSTER_GLOBAL_LOCAL bool link_x86_patch_initializer_calls(u8* bytes, ObjectFile* object, LinkInitializerPlan const* plan, u64 const* section_offsets,
                                                          u64 image_base, u64 entry_stub_offset, u32 const* displacement_offsets)
{
    bool result = true;
    u32 count = plan->constructor_count + plan->destructor_count;
    for (u32 index = 0; index < count && result; index += 1)
    {
        LinkInitializerEntry entry = index < plan->constructor_count ? plan->constructors[index] : plan->destructors[index - plan->constructor_count];
        ObjectSymbol* symbol = entry.symbol < object->symbol_count ? object->symbols + entry.symbol : 0;
        if (!symbol || symbol->section == OBJECT_SECTION_UNDEFINED || symbol->section >= OBJECT_SECTION_COUNT)
        {
            result = false;
        }
        else
        {
            u64 target = image_base + section_offsets[symbol->section] + symbol->value + (u64)entry.addend;
            u64 field = entry_stub_offset + displacement_offsets[index];
            s64 displacement = (s64)target - (s64)(image_base + field + 4);
            result = displacement >= INT32_MIN && displacement <= INT32_MAX;
            if (result)
            {
...
BUSTER_GLOBAL_LOCAL NativeExecutableLinkResult link_native_executable_elf64_x86_64(Arena* arena, ObjectFile* object, NativeExecutableLinkOptions options)
{
    ...
    u8* entry_stub = arena_allocate(arena, u8, link_x86_elf_entry_stub_capacity(&plan));
    u32* initializer_displacement_offsets =
    ...
    if (!link_x86_build_elf_entry_stub(entry_stub, link_x86_elf_entry_stub_capacity(&plan), &entry_stub_size, &entry_call_displacement_offset, false, 0, 0, 0,
    ...
    u64 file_size = object->sections[OBJECT_SECTION_DATA].data.length ? section_offsets[OBJECT_SECTION_DATA] + object->sections[OBJECT_SECTION_DATA].data.length
                                                                      : read_only_end;
    if (!link_elf_virtual_align(BUSTER_MAX(file_size, section_offsets[OBJECT_SECTION_DATA]), object->sections[OBJECT_SECTION_ZERO].alignment,
                                &section_offsets[OBJECT_SECTION_ZERO]))
    {
        result.error = LINK_ERROR_INVALID_INPUT;
        return result;
    }
...
//
// Every hosted image registers the loader finalizer through __cxa_atexit.
// A program with its own destructors uses that same import for its runner.
// The name is __cxa_atexit because plain atexit lives in libc_nonshared.a,
// while the shared libc exports this three-argument form.
                                                     ...
                                                     u32* atexit_symbol_index_out)
{
    ...
    bool valid = true;
    u32 wanted = BUSTER_ARRAY_LENGTH(hosted_names);
                                                                                           ...
                                                                                           NativeExecutableLinkOptions options)
{
    ...
    if (!link_x86_build_elf_entry_stub(entry_stub, link_x86_elf_entry_stub_capacity(&plan), &entry_stub_size, &entry_call_displacement_offset, true,
                                       &entry_exit_displacement_offset, &entry_atexit_displacement_offset, &loader_atexit_displacement_offset, &plan, initializer_displacement_offsets))
    {
        result.error = LINK_ERROR_RELOCATION;
        return result;
    }
    ...
    // Register the loader finalizer first, then the optional executable runner.
    u32 registration_offsets[] = {loader_atexit_displacement_offset, entry_atexit_displacement_offset};
    for (u32 registration = 0; registration < 1u + (plan.destructor_count != 0); registration += 1)
    {
        ...
        u64 atexit_displacement_offset = entry_stub_offset + registration_offsets[registration];
...
// The AArch64 counterpart of link_x86_patch_initializer_calls: one `bl` per
// initializer, at the word index the stub builder reported.
BUSTER_GLOBAL_LOCAL bool link_aarch64_patch_initializer_calls(u8* bytes, ObjectFile* object, LinkInitializerPlan const* plan, u64 const* section_offsets,
                                                              u64 image_base, u64 entry_stub_offset, u32 const* initializer_words)
{
    bool result = true;
    u32 count = plan->constructor_count + plan->destructor_count;
    for (u32 index = 0; index < count && result; index += 1)
    {
        ...
        {
            u64 target = image_base + section_offsets[symbol->section] + symbol->value + (u64)entry.addend;
            u64 offset = entry_stub_offset + (u64)initializer_words[index] * sizeof(u32);
            u32 instruction = 0;
            result = link_aarch64_branch_encode(A64_OPCODE_BL, (s64)target - (s64)(image_base + offset), &instruction);
            if (result)
            {
...
BUSTER_GLOBAL_LOCAL NativeExecutableLinkResult link_native_executable_elf64_aarch64(Arena* arena, ObjectFile* object, NativeExecutableLinkOptions options)
{
    ...
    object = &stripped_object;
    u32 entry_stub_capacity = link_aarch64_elf_entry_stub_words(false, &plan);
    u32* entry_stub = arena_allocate(arena, u32, entry_stub_capacity);
    u32* initializer_words =
        arena_allocate(arena, u32, plan.constructor_count + plan.destructor_count ? plan.constructor_count + plan.destructor_count : 1);
    u32 entry_stub_word = 0;
    u32 entry_stub_words = link_aarch64_build_elf_entry_stub(entry_stub, entry_stub_capacity, false, &plan, &entry_stub_word, 0, 0, 0, initializer_words);
    ...
    ObjectSymbol* entry_symbol = &object->symbols[entry_symbol_index];
    u64 entry_address = image_base + section_offsets[entry_symbol->section] + entry_symbol->value;
    u64 call_offset = entry_stub_offset + (u64)entry_stub_word * sizeof(u32);
                                                                                            ...
                                                                                            NativeExecutableLinkOptions options)
{
    ...
    // words.  The hosted shape calls libc's exit for the reason
    // link_x86_build_elf_entry_stub states; the freestanding writer above
    // keeps `mov x8,#93 / svc #0`.
    ...
    u32 entry_stub_capacity = link_aarch64_elf_entry_stub_words(true, &plan);
    u32* entry_stub = arena_allocate(arena, u32, entry_stub_capacity);
    ...
    u32 entry_stub_words = link_aarch64_build_elf_entry_stub(entry_stub, entry_stub_capacity, true, &plan, &entry_stub_word, &exit_stub_word,
                                                             &atexit_stub_word, &loader_atexit_stub_word, initializer_words);
    ...
    u32 x86_stub_capacity = link_x86_elf_entry_stub_capacity(&plan);
    u8* x86_stub = arena_allocate(arena, u8, x86_stub_capacity);
    u32 x86_stub_size = 0;
    buster_x86_metadata_prewarm();
    if (!entry_stub_words || !link_x86_build_elf_entry_stub(x86_stub, x86_stub_capacity, &x86_stub_size, 0, true, 0, 0, 0, &plan, 0))
    {
        result.error = LINK_ERROR_RELOCATION;
        return result;
    }
    ...
    // on it, because the plan it derives from them is what decides the entry
    ...
    // trap.
    memset(bytes + entry_stub_offset, 0, link_elf_entry_stub_slot(x86_stub_size, &plan));
    ...
    // The same registrations as the x86-64 staging writer, in AArch64 words.
    u32 registration_words[] = {loader_atexit_stub_word, atexit_stub_word};
    for (u32 registration = 0; registration < 1u + (plan.destructor_count != 0); registration += 1)
    {
        u64 atexit_address = 0;
        if (atexit_symbol_index >= object->symbol_count ||
        ...
        u64 atexit_call_offset = entry_stub_offset + (u64)registration_words[registration] * sizeof(u32);
...
BUSTER_GLOBAL_LOCAL NativeExecutableLinkResult link_native_executable_pe64(Arena* arena, ObjectFile* object, NativeExecutableLinkOptions options)
{
    ...
    u8* entry_stub_x86_64 = 0;
    u32 entry_stub_x86_64_size = 0;
    u32* entry_stub_aarch64 = 0;
    u32 entry_stub_aarch64_words = 0;
    ...
    // rather than a fourth entry in `startup_names`; the layout below is
    ...
    u32 startup_stub_words[BUSTER_ARRAY_LENGTH(startup_names)] = {0};
    u32 runner_stub_word = 0;
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        initializer_displacement_offsets = arena_allocate(arena, u32, initializer_count ? initializer_count : 1);
        if (aarch64)
        {
            ...
            u32 built_words = link_aarch64_build_pe_entry_stub(entry_stub_aarch64, entry_stub_aarch64_words, &plan, &entry_stub_word, &exit_stub_word,
                                                               &atexit_stub_word, startup_stub_words, initializer_displacement_offsets,
                                                               &runner_stub_word);
            if (built_words != entry_stub_aarch64_words)
            {
                result.error = LINK_ERROR_RELOCATION;
            }
        }
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        entry_stub_size = aarch64 ? (u64)entry_stub_aarch64_words * sizeof(u32) : entry_stub_x86_64_size;
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        u32 startup_thunk_count = aarch64 ? (u32)BUSTER_ARRAY_LENGTH(startup_names) : 0;
        thunk_size = (u64)(import_count + startup_thunk_count + (aarch64 ? terminate_import_count : 0)) * thunk_entry_size;
    }
    ...
    u32 startup_runtime_function_count = 0;
    if (result.error == LINK_ERROR_NONE)
    {
        startup_runtime_function_count = aarch64 && plan.destructor_count ? 2 : 1;
    }
    u32 startup_xdata_size = 0;
    if (result.error == LINK_ERROR_NONE)
    {
        startup_xdata_size = aarch64 ? startup_runtime_function_count * 8 : 8;
    }
    u64 pdata_virtual_size = 0;
    if (result.error == LINK_ERROR_NONE)
    {
        pdata_virtual_size = (u64)runtime_function_size * startup_runtime_function_count + object->sections[OBJECT_SECTION_WINDOWS_PDATA].data.length;
    }
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        ...
        object_section_offsets[OBJECT_SECTION_WINDOWS_PDATA] = (u64)runtime_function_size * startup_runtime_function_count;
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        ...
        if (aarch64)
        {
            ...
            u32 entry_function_words = runner_stub_word ? runner_stub_word : entry_stub_aarch64_words;
            link_write_u32(bytes, startup_xdata_raw, entry_function_words | (1u << 27));
            bytes[startup_xdata_raw + 4] = 0x81;
            bytes[startup_xdata_raw + 5] = 0xcc;
            bytes[startup_xdata_raw + 6] = 0x01;
            bytes[startup_xdata_raw + 7] = 0xe4;
            if (runner_stub_word)
            {
                link_write_u32(bytes, startup_pdata_raw + runtime_function_size,
                               section_rvas[PE_SECTION_TEXT] + (u32)(entry_stub_offset + (u64)runner_stub_word * sizeof(u32)));
                link_write_u32(bytes, startup_pdata_raw + runtime_function_size + 4, section_rvas[PE_SECTION_XDATA] + 8);
                link_write_u32(bytes, startup_xdata_raw + 8, (entry_stub_aarch64_words - runner_stub_word) | (1u << 27));
                bytes[startup_xdata_raw + 12] = 0x81;
                bytes[startup_xdata_raw + 13] = 0xcc;
                bytes[startup_xdata_raw + 14] = 0x01;
                bytes[startup_xdata_raw + 15] = 0xe4;
            }
        }
        else
        {
    ...
    if (result.error == LINK_ERROR_NONE)
    {
        ...
        if (!aarch64)
        {
            ...
            // Each rip-relative field ends four bytes past its own offset,
            // which is where the displacement is measured from; the builder
            // reports every offset because an initializer call moves them all.
            s64 configure_argv_displacement = (s64)configure_argv_iat_rva - (s64)(entry_rva + startup_displacement_offsets[0] + 4);
            s64 argc_displacement = (s64)argc_iat_rva - (s64)(entry_rva + startup_displacement_offsets[1] + 4);
            s64 argv_displacement = (s64)argv_iat_rva - (s64)(entry_rva + startup_displacement_offsets[2] + 4);
            s64 main_displacement = (s64)main_rva - (s64)(entry_rva + main_displacement_offset + 4);
            s64 exit_displacement = (s64)exit_iat_rva - (s64)(entry_rva + exit_displacement_offset + 4);
            ...
            if (result.error == LINK_ERROR_NONE)
            {
                link_write_u32(bytes, section_raw_offsets[PE_SECTION_TEXT] + startup_displacement_offsets[0], (u32)(s32)configure_argv_displacement);
                link_write_u32(bytes, section_raw_offsets[PE_SECTION_TEXT] + startup_displacement_offsets[1], (u32)(s32)argc_displacement);
                ...
                for (u32 index = 0; index < initializer_count && result.error == LINK_ERROR_NONE; index += 1)
                {
                    LinkInitializerEntry entry =
                        index < plan.constructor_count ? plan.constructors[index] : plan.destructors[index - plan.constructor_count];
                    ObjectSymbol* initializer = entry.symbol < object->symbol_count ? object->symbols + entry.symbol : 0;
                    if (!initializer || initializer->section >= OBJECT_SECTION_COUNT ||
                        object_output_sections[initializer->section] >= pe_section_count)
                    {
                        result.error = LINK_ERROR_RELOCATION;
                        break;
                    }
                    u64 initializer_rva = section_rvas[object_output_sections[initializer->section]] + object_section_offsets[initializer->section] +
                                          initializer->value + (u64)entry.addend;
                    u32 field = initializer_displacement_offsets[index];
                    s64 displacement = (s64)initializer_rva - (s64)(entry_rva + field + 4);
                    if (displacement < INT32_MIN || displacement > INT32_MAX)
                    {
                        result.error = LINK_ERROR_RELOCATION;
                        break;
                    }
                    link_write_u32(bytes, section_raw_offsets[PE_SECTION_TEXT] + field, (u32)(s32)displacement);
                }
            }
        }
        else
        {
            ...
            u64 startup_thunk_rva[BUSTER_ARRAY_LENGTH(startup_names)] = {0};
            for (u32 startup_index = 0; startup_index < BUSTER_ARRAY_LENGTH(startup_names) && result.error == LINK_ERROR_NONE; startup_index += 1)
            {
                u64 thunk_delta = ((u64)import_count + startup_index) * thunk_entry_size;
                u64 thunk_raw = section_raw_offsets[PE_SECTION_TEXT] + thunk_offset + thunk_delta;
                u64 thunk_rva = section_rvas[PE_SECTION_TEXT] + thunk_offset + thunk_delta;
                u64 iat_rva = import_section_rva + runtime_address_offset + (u64)startup_import_slots[startup_index] * sizeof(u64);
                u64 slot_address = image_base + iat_rva;
                u64 thunk_address = image_base + thunk_rva;
                u64 page_offset = slot_address & 0xfff;
                bool adrp_valid = true;
                startup_thunk_rva[startup_index] = thunk_rva;
                link_write_u32(bytes, thunk_raw, link_aarch64_adrp(16, thunk_address, slot_address, &adrp_valid));
                if (!adrp_valid || page_offset % 8)
                {
                    result.error = LINK_ERROR_RELOCATION;
            ...
            // One thunk per terminating import, past the program and startup
            // imports: `exit` first and `_crt_atexit` beside it when needed.
            ...
            {
                u64 thunk_delta = ((u64)import_count + BUSTER_ARRAY_LENGTH(startup_names) + terminate_index) * thunk_entry_size;
            ...
            if (result.error == LINK_ERROR_NONE)
            {
                main_displacement = (s64)main_rva - (s64)(entry_rva + (u64)entry_stub_word * sizeof(u32));
            }
            s64 exit_displacement = 0;
            if (result.error == LINK_ERROR_NONE)
            {
                exit_displacement = (s64)exit_thunk_rva - (s64)(entry_rva + (u64)exit_stub_word * sizeof(u32));
            }
            ...
            if (result.error == LINK_ERROR_NONE)
            {
                ...
                for (u32 startup_index = 0; startup_index < BUSTER_ARRAY_LENGTH(startup_names) && result.error == LINK_ERROR_NONE; startup_index += 1)
                {
                    s64 startup_displacement = (s64)startup_thunk_rva[startup_index] -
                                               (s64)(entry_rva + (u64)startup_stub_words[startup_index] * sizeof(u32));
                    u32 startup_instruction = 0;
                    if (!link_aarch64_branch_encode(A64_OPCODE_BL, startup_displacement, &startup_instruction))
                    {
                        result.error = LINK_ERROR_RELOCATION;
                    }
                    else
                    {
                        link_write_u32(bytes, section_raw_offsets[PE_SECTION_TEXT] + (u64)startup_stub_words[startup_index] * sizeof(u32),
                                       startup_instruction);
                    }
                }
                if (plan.destructor_count)
                ...
                for (u32 index = 0; index < initializer_count && result.error == LINK_ERROR_NONE; index += 1)
                {
                    ...
                    u64 initializer_rva = section_rvas[object_output_sections[initializer->section]] + object_section_offsets[initializer->section] +
                                          initializer->value + (u64)entry.addend;
                    u64 field = (u64)initializer_displacement_offsets[index] * sizeof(u32);
                    u32 instruction = 0;
                    if (!link_aarch64_branch_encode(A64_OPCODE_BL, (s64)initializer_rva - (s64)(entry_rva + field), &instruction))
                    {
                        result.error = LINK_ERROR_RELOCATION;
                        break;
                    }
                    link_write_u32(bytes, section_raw_offsets[PE_SECTION_TEXT] + field, instruction);
                }
            }
        }
    }
