...
// machine_x64_value_shape and the call/return/argument selectors carry
...
struct MachineX64ValueShape
{
    ...
    bool indirect;
    // One 64-byte vector value in a ZMM register. System V places its direct
    // part in the floating register sequence. Win64 arguments carry this
    // value through an indirect pointer part; results use ZMM0 directly.
    bool vector;
    u8 f80_result;
    ...
    bool xmm128;
    // Frame-backed vectors split into consecutive XMM/YMM/ZMM ABI parts.
    // Scalar/aggregate parts include the three-lane padded-vector return.
    // Vector offsets are still part_index * vector_part_bytes and need no extra storage.
    u8 vector_part_bytes;
    // A GNU padded vector can expose scalar Win64 lanes instead of one
    // by-reference vector part. The lane size drives sized frame transfers;
    // floating lanes occupy the positional XMM file.
    u8 windows_scalar_lane_size;
    bool windows_scalar_float;
    // A three-lane double result uses XMM0/XMM1 and ST(0). The first two
    // parts retain part_is_float; the third is bridged explicitly through x87.
    bool x87_f64_result;
...
BUSTER_CT_CHECK(sizeof(MachineX64ValueShape) == 56);
...
struct MachineX64ArgumentPlacement
{
    u16 first_integer;
    u16 first_float;
    u32 first_stack_part;
    u32 stack_part_count;
    bool on_stack;
    // Win64 vectors wider than the target's widest register consume one
    // pointer slot per register-sized piece. The prefix can occupy the
    // remaining argument GPRs while the tail continues on the stack.
    u8 register_part_count;
    u8 reserved[2];
};
BUSTER_CT_CHECK(sizeof(MachineX64ArgumentPlacement) == 16);
...
struct MachineX64Selector
{
    ...
    MachineX64ValueShape* parameter_shapes;
    MachineX64ArgumentPlacement* parameter_placements;
    MachineX64ValueShape return_shape;
    // One reusable call workspace, sized by the existing canonical row walk.
...
// Win64 and x86-64 UEFI represent va_list as the address of the current
// argument. Keep that ABI-visible eight-byte object in the ordinary integer
// register path; VA_START/VA_COPY still own their frame-backed temporary and
// Windows VA_ARG continues to consume the pointer it loads from that slot.
BUSTER_GLOBAL_LOCAL bool machine_x64_type_is_windows_va_list(IrType* type, Target target)
{
    return machine_x64_target_is_windows(target) && type && type->layout.resolved && type->kind == IR_TYPE_VA_LIST && type->layout.size == 8;
}
...
BUSTER_GLOBAL_LOCAL bool machine_x64_type_is_vector_register(IrProgram* program, IrType* type)
{
    bool result = program && type && type->layout.resolved && type->kind == IR_TYPE_VECTOR && type->layout.size == 64;
    if (result)
    {
...
BUSTER_GLOBAL_LOCAL bool machine_x64_type_is_f80(MachineX64Selector const* selector, IrTypeId type_id)
{
    ...
    bool result = !machine_x64_target_is_windows(selector->target) && type_class.kind == IR_TYPE_FLOAT && type_class.size_log2 == 4;
    if (result)
    {
...
typedef struct MachineX64PaddedVector MachineX64PaddedVector;
struct MachineX64PaddedVector
{
    u32 lane_count;
    u32 lane_size;
    u32 storage_size;
...
// GNU vector_size preserves the requested logical lane count while rounding
// the object image to the next power of two. Keep both facts explicit at the
// machine boundary: lane operations use lane_count/lane_size, while placement,
// spills and copies always reserve storage_size bytes.
BUSTER_GLOBAL_LOCAL bool machine_x64_padded_vector(IrProgram* program, IrType* type, MachineX64PaddedVector* info)
{
    bool result = false;
        ...
        type->element_count <= UINT64_MAX / element->layout.size)
    {
        ...
        result = logical_size < type->layout.size && type->layout.size == next_power_of_two(logical_size);
        if (result)
        {
            *info = (MachineX64PaddedVector){
...
BUSTER_GLOBAL_LOCAL bool machine_x64_padded_vector_native(Target target, MachineX64PaddedVector* info)
{
    u32 native_width = BUSTER_MAX(16u, target_vector_register_size(target));
    return info && info->storage_size <= native_width;
}

BUSTER_GLOBAL_LOCAL bool machine_x64_windows_padded_vector_indirect(Target target, MachineX64PaddedVector* info)
{
    bool result = false;
    if (info)
    {
        u32 native_width = BUSTER_MAX(16u, target_vector_register_size(target));
        result = (info->floating && info->lane_size == 2) ||
                 (info->storage_size >= 16 && info->storage_size <= native_width);
    }
    return result;
}
...
BUSTER_GLOBAL_LOCAL bool machine_x64_value_shape(IrProgram* program, IrTypeId type_id, IrAbiUse use, Target target, MachineX64ValueShape* shape)
{
    ...
    MachineX64PaddedVector padded = {0};
    if (machine_x64_padded_vector(program, type, &padded))
    {
        bool windows = convention == IR_ABI_CONVENTION_WIN64_X86_64;
        bool native = windows ? machine_x64_windows_padded_vector_indirect(target, &padded)
                              : machine_x64_padded_vector_native(target, &padded);
        u32 stack_alignment = machine_x64_vector_stack_alignment(type, target);
        if (use == IR_ABI_USE_RESULT)
        {
            if (native && padded.storage_size >= 16)
            {
                *shape = (MachineX64ValueShape){
                    .part_count = 1,
                    .byte_size = padded.storage_size,
                    .exact_byte_size = padded.storage_size,
                    .aggregate = true,
                    .vector_part_bytes = (u8)padded.storage_size,
                    .stack_alignment = stack_alignment,
                };
                return true;
            }
            if (!native && padded.lane_count == 3 && (!padded.floating || padded.lane_size == 8))
            {
                *shape = (MachineX64ValueShape){
                    .part_offsets = {0, padded.lane_size, padded.lane_size * 2},
                    .part_is_float = {padded.floating, padded.floating, 0},
                    .part_sizes = {(u8)padded.lane_size, (u8)padded.lane_size, (u8)padded.lane_size},
                    .part_count = 3,
                    .byte_size = padded.storage_size,
                    .exact_byte_size = padded.storage_size,
                    .aggregate = true,
                    .x87_f64_result = padded.floating,
                    .stack_alignment = stack_alignment,
                };
                return true;
            }
        ...
        else if (windows)
        {
            if (native)
            {
                *shape = (MachineX64ValueShape){
                    ...
                    .aggregate = true,
                    .indirect = true,
                    .stack_alignment = BUSTER_MAX(stack_alignment, 16u),
                };
            }
            else
            {
                *shape = (MachineX64ValueShape){
                    .part_count = padded.lane_count,
                    .byte_size = padded.storage_size,
                    .exact_byte_size = padded.storage_size,
                    .aggregate = true,
                    .windows_scalar_lane_size = (u8)padded.lane_size,
                    .windows_scalar_float = padded.floating,
                    .stack_alignment = 8,
                };
            }
            return true;
        ...
        else if (padded.storage_size >= 16)
        {
            *shape = (MachineX64ValueShape){
                .part_count = 1,
                .byte_size = padded.storage_size,
          ...
          !(type->layout.size & (type->layout.size - 1)))))
    {
        ...
        bool indirect = convention == IR_ABI_CONVENTION_WIN64_X86_64 && use != IR_ABI_USE_RESULT;
        u32 reference_piece_bytes = 0;
        u32 reference_pieces = indirect ? codegen_canonical_x64_windows_vector_argument_pieces(&target, type, &reference_piece_bytes) : 0;
        u32 result_register_bytes = 0;
        u32 result_registers = !indirect && convention == IR_ABI_CONVENTION_WIN64_X86_64
                                   ? codegen_canonical_x64_vector_part_registers(&target, (u32)type->layout.size, &result_register_bytes)
                                   : 0;
        bool indirect_result = !indirect && convention == IR_ABI_CONVENTION_WIN64_X86_64 && type->layout.size > 64 &&
                               (!result_registers || result_registers > 4);
        bool direct_result_override = !indirect && convention == IR_ABI_CONVENTION_WIN64_X86_64 && type->layout.size > 64 &&
                                      result_registers && result_registers <= 4;
        bool direct_abi = !abi.indirect && !abi.memory && abi.part_count == 1 &&
                          abi.parts[0].abi_class == IR_ABI_CLASS_VECTOR && abi.parts[0].size == type->layout.size;
        bool supported = part_bytes && (part_bytes == 16 || part_bytes == 32 || part_bytes == 64) && !abi.memory &&
                         ((indirect && abi.indirect && abi.part_count == 1 && reference_pieces) ||
                          (indirect_result && abi.indirect) || (!indirect_result && (direct_abi || direct_result_override)));
        if (supported)
        {
            if (indirect_result)
            {
            ...
            else
            {
                ...
                *shape = (MachineX64ValueShape){
                    .part_count = indirect ? reference_pieces : force_stack ? 0u : (u32)type->layout.size / part_bytes,
                    .byte_size = (u32)type->layout.size,
                    .exact_byte_size = (u32)type->layout.size,
                    .aggregate = true,
                    .indirect = indirect,
                    .vector_part_bytes = force_stack ? 0 : (u8)(indirect && reference_pieces > 1 ? reference_piece_bytes : part_bytes),
                    .force_stack = force_stack,
                    .stack_alignment = machine_x64_vector_stack_alignment(type, target),
                };
                if (indirect && reference_pieces == 1)
                {
                    shape->vector_part_bytes = 0;
                }
            }
        }
        return supported;
...
BUSTER_GLOBAL_LOCAL u32 machine_x64_shape_class_parts(MachineX64ValueShape* shape, bool floats)
{
    ...
    {
        count = shape->windows_scalar_float == floats ? shape->part_count : 0;
    }
    else if (shape->vector_part_bytes)
    {
        // Direct vector pieces consume the float register file; split
        // by-reference pieces are pointers and consume the integer file.
        count = (shape->indirect ? !floats : floats) ? shape->part_count : 0;
    }
    else
    {
...
// Incoming and outgoing addressing use signed 32-bit frame displacements.
// Check in bytes before placement narrows or advances any stack cursor.
BUSTER_GLOBAL_LOCAL bool machine_x64_argument_area_fits(MachineX64ValueShape* shape, u32 stack_parts)
{
    u64 bytes = shape->windows_scalar_lane_size ? (u64)shape->part_count * 8u
                                               : shape->indirect ? (u64)shape->part_count * 8u : shape->byte_size;
    return (u64)stack_parts * 8u + bytes + shape->stack_alignment + 32u <= INT32_MAX;
}
                                                    ...
                                                    MachineX64ArgumentPlacement* placement)
{
    ...
    if (windows)
    {
        // Ordinary arguments occupy one positional slot. Split references
        // are the Win64 exception: each piece is a pointer slot, and a value
        // can straddle the remaining GPRs and the outgoing stack tail.
        u32 slot = BUSTER_MAX(*integer_count, *float_count);
        if (shape->windows_scalar_lane_size)
        {
            u32 available = slot < BUSTER_ARRAY_LENGTH(machine_x64_windows_arguments)
                                ? BUSTER_ARRAY_LENGTH(machine_x64_windows_arguments) - slot
                                : 0;
            u32 register_parts = BUSTER_MIN(shape->part_count, available);
            u32 stack_parts = shape->part_count - register_parts;
            *placement = (MachineX64ArgumentPlacement){
                .first_integer = (u16)slot,
                .first_float = (u16)slot,
                .first_stack_part = *stack_part_count,
                .stack_part_count = stack_parts,
                .on_stack = stack_parts != 0,
                .register_part_count = (u8)register_parts,
            };
            *integer_count = slot + register_parts;
            *float_count = slot + register_parts;
            *stack_part_count += stack_parts;
        }
        else if (shape->indirect && shape->vector_part_bytes)
        {
            u32 available = slot < BUSTER_ARRAY_LENGTH(machine_x64_windows_arguments)
                                ? BUSTER_ARRAY_LENGTH(machine_x64_windows_arguments) - slot
                                : 0;
        ...
        else if (shape->force_stack || shape->part_count != 1 || slot >= BUSTER_ARRAY_LENGTH(machine_x64_windows_arguments))
        {
        ...
        else
        {
            *placement = (MachineX64ArgumentPlacement){
                .first_integer = (u16)slot,
                .first_float = (u16)slot,
                .register_part_count = 1,
            };
            *integer_count = slot + 1;
            *float_count = slot + 1;
        }
    }
    else if (shape->force_stack || *integer_count + integer_parts > BUSTER_ARRAY_LENGTH(machine_x64_system_v_arguments) ||
             *float_count + float_parts > 8)
    {
...
BUSTER_GLOBAL_LOCAL void machine_x64_va_named_cursors(MachineX64Selector* selector, u32* integer_count, u32* float_count, u32* stack_end)
{
    ...
    *stack_end = 0;
    ...
    for (u32 argument = 0; argument < function_type->parameter_count; argument += 1)
    {
...
// The two shift directions beside the unsigned right one, over the same
// slot-backed pair and by the same bounded constant amount. Each is the two
// halves shifted in place plus one cross term: a left shift carries the low
// half's top bits into the high one, and both right shifts carry the high
// half's low bits down. The registers the frame loads produce are freshly
// synthesized, which is what licenses the in-place shift rows below -- the same
// shape machine_x64_select_i128_unsigned_shift_right already uses.
BUSTER_GLOBAL_LOCAL bool machine_x64_select_i128_shift_left(MachineX64Selector* selector, IrInstruction* instruction, u32 amount)
{
    IrFunction* function = selector->function;

    bool selected = instruction->operand_count >= 2 && instruction->result.value < function->value_count &&
                    instruction->operands[0].value < function->value_count && amount <= 127;
    u32 source_slot = selected ? selector->value_stack_slots[instruction->operands[0].value] : UINT32_MAX;
    u32 result_slot = selected ? selector->value_stack_slots[instruction->result.value] : UINT32_MAX;
    selected = selected && source_slot != UINT32_MAX && result_slot != UINT32_MAX;
    ...
    {
        u32 high = machine_x64_select_frame_load64(selector, source_slot, 0);
        if (amount > 64)
        {
        ...
        machine_x64_select_frame_store64(selector, result_slot, 8, high);
        machine_x64_select_frame_store64(selector, result_slot, 0, machine_x64_select_immediate_register(selector, 0));
    }

    return selected;
}

BUSTER_GLOBAL_LOCAL bool machine_x64_select_i128_signed_shift_right(MachineX64Selector* selector, IrInstruction* instruction, u32 amount)
{
    IrFunction* function = selector->function;
    ...
    {
        // Past the halfway point the high half becomes the low one and the
        // sign fills what is left, which is that half shifted right by 63.
        u32 low = machine_x64_select_frame_load64(selector, source_slot, 8);
        u32 sign = machine_x64_synthesize_register(selector);
        machine_x64_select_row(selector, (MachineInstruction){
                                             .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, sign),
                                                          machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, low)},
                                             .opcode = MACHINE_X64_MOV_RR,
                                         });
        machine_x64_select_row(selector, (MachineInstruction){
                                             .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, sign),
                                                          machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, machine_x64_select_immediate_register(selector, 63))},
                                             .opcode = MACHINE_X64_SAR64,
                                         });
        if (amount > 64)
        {
        ...
        machine_x64_select_frame_store64(selector, result_slot, 0, low);
        machine_x64_select_frame_store64(selector, result_slot, 8, sign);
    }
...
// followed by stack arguments. Indirect aggregates occupy a pointer slot.
BUSTER_GLOBAL_LOCAL bool machine_x64_select_windows_va_arg(MachineX64Selector* selector, IrType* type, u32 list_register,
                                                           u32 result_register, u32 result_slot, bool aggregate)
{
    ...
    if (type && type->layout.resolved && type->layout.size && type->layout.size <= UINT32_MAX)
    {
        ...
        bool vector = selector->vector_registers_supported && machine_x64_type_is_vector_register(selector->program, type);
        if (abi.part_count == 1 && !abi.memory && (aggregate || vector || type->layout.size <= 8))
        {
            u32 cursor = machine_x64_synthesize_register(selector);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, cursor), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, list_register)},
                .opcode = MACHINE_X64_LOAD_PTR64,
            });
            u32 next = machine_x64_synthesize_register(selector);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, next), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, cursor)},
                .payload = 8,
                .opcode = MACHINE_X64_LEA_OFFSET,
            });
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, list_register), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, next)},
                .opcode = MACHINE_X64_STORE_PTR64,
            });
            u32 address = cursor;
            if (abi.indirect)
            {
                address = machine_x64_synthesize_register(selector);
                machine_x64_select_row(selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, cursor)},
                    .opcode = MACHINE_X64_LOAD_PTR64,
                });
            }
            if (aggregate)
            {
                machine_x64_select_row(selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address)},
                    .payload = (u32)type->layout.size,
                    .opcode = MACHINE_X64_COPY_FRAME_FROM_PTR,
                });
            }
            else
            {
                u16 opcode = vector ? MACHINE_X64_VLOAD_PTR : type->layout.size == 1 ? MACHINE_X64_LOAD_PTR8 : type->layout.size == 2 ? MACHINE_X64_LOAD_PTR16
                             : type->layout.size == 4 ? MACHINE_X64_LOAD_PTR32 : MACHINE_X64_LOAD_PTR64;
                u32 row = machine_x64_select_row(selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address)},
                    .opcode = opcode,
                });
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_va_arg(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
        ...
        machine_x64_operand_register(selector, instruction->operands[0], &source_register))
    {
        ...
        else if ((scalar || aggregate) && result_is_frame == aggregate &&
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_va_start(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    bool selected = false;
    bool windows = machine_x64_target_is_windows(selector->target);
    IrType* list_type = ir_type_from_id(&selector->program->types, instruction->canonical_type);
    if (result_slot != UINT32_MAX && windows && list_type && list_type->kind == IR_TYPE_VA_LIST && list_type->layout.size == 8)
    {
        IrType* function_type = ir_type_from_id(&selector->program->types, function->canonical_type);
        // The hidden result pointer precedes the named sequence. Ordinary
        // parameters occupy one slot; split vector references occupy one
        // per piece, including pieces whose fixed-argument placement spilled
        // past the four register homes.
        u32 named_slots = selector->return_shape.indirect ? 1u : 0u;
        for (u32 argument = 0; argument < function_type->parameter_count; argument += 1)
        {
            MachineX64ValueShape* shape = selector->parameter_shapes + argument;
            named_slots += shape->windows_scalar_lane_size ? shape->part_count
                           : shape->indirect && shape->vector_part_bytes ? shape->part_count : 1u;
        }
        u32 cursor = machine_x64_synthesize_register(selector);
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, cursor)},
            .payload = named_slots * 8u,
            .opcode = MACHINE_X64_LEA_INCOMING,
        });
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, cursor)},
            .opcode = MACHINE_X64_STORE_FRAME64,
        });
        selected = true;
    }
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_argument(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    ...
    if (instruction->immediate_count && instruction->immediates)
    {
        ...
        selected = argument_index < selector->parameter_count &&
...
// Build the zero/2^63 correction as two initialized frame words. The mask is
// zero or all ones; padding and the exponent are zero when the bias is zero.
BUSTER_GLOBAL_LOCAL u32 machine_x64_select_f80_u64_bias(MachineX64Selector* selector, u32 slot, u32 mask)
{
    u32 sign_bit = machine_x64_select_immediate_register(selector, UINT64_C(0x8000000000000000));
    u32 exponent_bits = machine_x64_select_immediate_register(selector, UINT64_C(0x403e));
    u32 significand = machine_x64_synthesize_register(selector);
    machine_x64_select_row(selector, (MachineInstruction){
        .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, significand), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, mask),
                     machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, sign_bit)}, .opcode = MACHINE_X64_AND64});
    u32 exponent = machine_x64_synthesize_register(selector);
    machine_x64_select_row(selector, (MachineInstruction){
        .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, exponent), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, mask),
                     machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, exponent_bits)}, .opcode = MACHINE_X64_AND64});
    machine_x64_select_frame_store64(selector, slot, 0, significand);
    machine_x64_select_frame_store64(selector, slot, 8, exponent);
    return significand;
}

// Unsigned eightbytes need only signed conversions plus an exact correction.
// With the backend's extended-precision x87 contract, both corrections are
// exact for every defined input and leave the caller's rounding mode intact.
BUSTER_GLOBAL_LOCAL bool machine_x64_select_f80_unsigned64_cast(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    IrFunction* function = selector->function;
    IrValueId source = instruction->operands[0];
    IrType* source_type = ir_type_from_id(&selector->program->types, function->values[source.value].canonical_type);
    IrType* result_type = ir_type_from_id(&selector->program->types, instruction->canonical_type);
    bool from_unsigned = instruction->conversion_operation == IR_CONVERSION_UNSIGNED_INTEGER_TO_FLOAT &&
                         source_type && source_type->kind == IR_TYPE_INTEGER && source_type->bit_width == 64 &&
                         machine_x64_type_is_f80(selector, instruction->canonical_type);
    bool to_unsigned = instruction->conversion_operation == IR_CONVERSION_FLOAT_TO_UNSIGNED_INTEGER &&
                       result_type && result_type->kind == IR_TYPE_INTEGER && result_type->bit_width == 64 &&
                       machine_x64_type_is_f80(selector, function->values[source.value].canonical_type);
    u32 source_slot = selector->value_stack_slots[source.value];
    u32 result_slot = selector->value_stack_slots[instruction->result.value];
    u32 source_register = UINT32_MAX;
    bool selected = (from_unsigned && result_slot != UINT32_MAX && machine_x64_operand_register(selector, source, &source_register)) ||
                    (to_unsigned && source_slot != UINT32_MAX && result_register != UINT32_MAX);
    if (selected)
    {
        ...
        if (from_unsigned)
        {
            u32 count = machine_x64_select_immediate_register(selector, 63);
            mask = machine_x64_synthesize_register(selector);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, mask), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, source_register)},
                .opcode = MACHINE_X64_MOV_RR});
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, mask), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, count)},
                .opcode = MACHINE_X64_SAR64});
            u32 low_mask = machine_x64_select_immediate_register(selector, UINT64_C(0x7fffffffffffffff));
            u32 low = machine_x64_synthesize_register(selector);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, low), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, source_register),
                             machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, low_mask)}, .opcode = MACHINE_X64_AND64});
            machine_x64_select_frame_store64(selector, integer, 0, low);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot), machine_ref_make(MACHINE_REF_STACK_SLOT, integer),
                             machine_ref_make(MACHINE_REF_STACK_SLOT, scratch)}, .payload = 4, .opcode = MACHINE_X64_F80_CONVERT});
        }
        else
        {
            machine_x64_select_frame_store64(selector, bias, 0, machine_x64_select_immediate_register(selector, UINT64_C(0x8000000000000000)));
            ...
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, high), machine_ref_make(MACHINE_REF_STACK_SLOT, source_slot),
                             machine_ref_make(MACHINE_REF_STACK_SLOT, bias)}, .payload = 5, .opcode = MACHINE_X64_F80_COMPARE});
            u32 zero = machine_x64_select_immediate_register(selector, 0);
            mask = machine_x64_synthesize_register(selector);
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, mask), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, zero),
                             machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, high)}, .opcode = MACHINE_X64_SUB64});
        }
        u32 high_bit = machine_x64_select_f80_u64_bias(selector, bias, mask);
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, from_unsigned ? result_slot : bias),
                         machine_ref_make(MACHINE_REF_STACK_SLOT, from_unsigned ? result_slot : source_slot),
                         machine_ref_make(MACHINE_REF_STACK_SLOT, bias)},
            .payload = from_unsigned ? 0u : 1u, .opcode = MACHINE_X64_F80_BINARY});
        if (to_unsigned)
        {
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, integer), machine_ref_make(MACHINE_REF_STACK_SLOT, bias),
                             machine_ref_make(MACHINE_REF_STACK_SLOT, scratch)}, .payload = 5, .opcode = MACHINE_X64_F80_CONVERT});
            u32 low = machine_x64_select_frame_load64(selector, integer, 0);
            u32 row = machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, low),
                             machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, high_bit)}, .opcode = MACHINE_X64_OR64});
            machine_x64_define(selector, result_register, row);
        }
    }
    return selected;
...
// f80 transactions never expose the x87 stack to allocation. General
// values cross a conversion through an eight-byte image, and the result
// returns to the ordinary scalar register after the closed transaction.
BUSTER_GLOBAL_LOCAL bool machine_x64_select_f80_cast(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    IrFunction* function = selector->function;
    IrTypeId source_id = function->values[instruction->operands[0].value].canonical_type;
    IrType* source_type = ir_type_from_id(&selector->program->types, source_id);
    IrType* result_type = ir_type_from_id(&selector->program->types, instruction->canonical_type);
    bool from_f80 = machine_x64_type_is_f80(selector, source_id);
    bool to_f80 = machine_x64_type_is_f80(selector, instruction->canonical_type);
    u32 mode = UINT32_MAX;
    bool source_signed = instruction->conversion_operation == IR_CONVERSION_SIGNED_INTEGER_TO_FLOAT;
    bool from_integer = to_f80 && (source_signed || instruction->conversion_operation == IR_CONVERSION_UNSIGNED_INTEGER_TO_FLOAT);
    if (from_f80 && result_type->kind == IR_TYPE_FLOAT && (result_type->bit_width == 32 || result_type->bit_width == 64))
    {
        mode = result_type->bit_width == 32 ? 2u : 3u;
    }
    else if (to_f80 && source_type->kind == IR_TYPE_FLOAT && (source_type->bit_width == 32 || source_type->bit_width == 64))
    {
        mode = source_type->bit_width == 32 ? 0u : 1u;
    }
    else if (from_integer && source_type->bit_width <= 64 && (source_signed || source_type->bit_width < 64))
    {
        mode = 4;
    }
    else if (from_f80 && result_type->kind == IR_TYPE_INTEGER && result_type->bit_width <= 64 &&
             (result_type->is_signed || result_type->bit_width < 64))
    {
        mode = 5;
    }
    u32 source_slot = selector->value_stack_slots[instruction->operands[0].value];
    u32 result_slot = selector->value_stack_slots[instruction->result.value];
    u32 source_register = UINT32_MAX;
    bool selected = mode != UINT32_MAX && (from_f80 ? source_slot != UINT32_MAX :
                        machine_x64_operand_register(selector, instruction->operands[0], &source_register)) &&
                    (to_f80 ? result_slot != UINT32_MAX : result_register != UINT32_MAX);
    if (selected)
    {
        if (!from_f80)
        {
            if (from_integer && source_type->bit_width < 64)
            {
                u32 bits = source_type->bit_width;
                u16 opcode = (u16)(source_signed ? (bits == 8 ? MACHINE_X64_MOVSX8_RR : bits == 16 ? MACHINE_X64_MOVSX16_RR : MACHINE_X64_MOVSX32_RR)
                                                : (bits == 8 ? MACHINE_X64_MOVZX8_RR : bits == 16 ? MACHINE_X64_MOVZX16_RR : MACHINE_X64_MOV32_RR));
                u32 extended = machine_x64_synthesize_register(selector);
                machine_x64_select_row(selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, extended), machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, source_register)},
                    .opcode = opcode});
                source_register = extended;
            }
            source_slot = machine_x64_append_slot(selector, 8, 8);
            machine_x64_select_frame_store64(selector, source_slot, 0, source_register);
        }
        if (!to_f80)
        {
            result_slot = machine_x64_append_slot(selector, 8, 8);
        }
        u32 scratch_slot = machine_x64_append_slot(selector, 8, 8);
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot), machine_ref_make(MACHINE_REF_STACK_SLOT, source_slot),
                         machine_ref_make(MACHINE_REF_STACK_SLOT, scratch_slot)},
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_f80_binary(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    u32 left = selector->value_stack_slots[instruction->operands[0].value];
    u32 right = selector->value_stack_slots[instruction->operands[1].value];
    ...
    {
    case IR_BINARY_FLOAT_ADD: operation = 0; break;
    case IR_BINARY_FLOAT_SUBTRACT: operation = 1; break;
    case IR_BINARY_FLOAT_MULTIPLY: operation = 2; break;
    case IR_BINARY_FLOAT_DIVIDE: operation = 3; break;
    case IR_BINARY_FLOAT_EQUAL: operation = 0; compare = true; break;
    case IR_BINARY_FLOAT_NOT_EQUAL: operation = 1; compare = true; break;
    case IR_BINARY_FLOAT_LESS: operation = 2; compare = true; break;
    case IR_BINARY_FLOAT_LESS_EQUAL: operation = 3; compare = true; break;
    case IR_BINARY_FLOAT_GREATER: operation = 4; compare = true; break;
    case IR_BINARY_FLOAT_GREATER_EQUAL: operation = 5; compare = true; break;
    default: break;
    }
    bool selected = operation != UINT32_MAX && left != UINT32_MAX && right != UINT32_MAX &&
                    (compare ? result_register != UINT32_MAX : destination != UINT32_MAX);
    if (selected)
    {
        MachineRef result_ref = compare ? machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register) : machine_ref_make(MACHINE_REF_STACK_SLOT, destination);
        u32 row = machine_x64_select_row(selector, (MachineInstruction){
            .operands = {result_ref, machine_ref_make(MACHINE_REF_STACK_SLOT, left), machine_ref_make(MACHINE_REF_STACK_SLOT, right)},
            .payload = operation, .opcode = (u16)(compare ? MACHINE_X64_F80_COMPARE : MACHINE_X64_F80_BINARY)});
        if (compare)
        {
            machine_x64_define(selector, result_register, row);
        }
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_f80_negate(MachineX64Selector* selector, IrInstruction* instruction)
{
    u32 source = selector->value_stack_slots[instruction->operands[0].value];
    u32 destination = selector->value_stack_slots[instruction->result.value];
    bool selected = instruction->unary_operation == IR_UNARY_FLOAT_NEGATE && source != UINT32_MAX && destination != UINT32_MAX;
    if (selected)
    {
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, destination), machine_ref_make(MACHINE_REF_STACK_SLOT, source)},
            .opcode = MACHINE_X64_F80_NEGATE});
    }
    return selected;
}
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_constant(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
             ...
             !(instruction->immediates[1] & ~UINT64_C(0xffff)))
    {
        ...
        if (selected)
        {
            machine_x64_select_frame_store64(selector, slot, 0, machine_x64_select_immediate_register(selector, instruction->immediates[0]));
            machine_x64_select_frame_store64(selector, slot, 8, machine_x64_select_immediate_register(selector, instruction->immediates[1]));
        }
    }
    ...
    {
        // A 128-bit integer constant is slot-backed like every i128 value, so
        // it has no result register and is written half by half. The second
        // eightbyte is the canonical emitters' selection: an explicit second
        // immediate when the frontend recorded one, otherwise the negated low
        // half's sign fill. This mirrors machine_a64_select_constant.
        IrType* constant_type = ir_type_from_id(&program->types, instruction->canonical_type);
        if (constant_type && constant_type->kind == IR_TYPE_INTEGER && constant_type->bit_width == 128 && instruction->immediate_count &&
            instruction->immediates)
        {
            ...
            {
                machine_x64_select_frame_store64(selector, result_slot, half_index * 8,
                                                 machine_x64_select_immediate_register(selector, halves[half_index]));
            }
            selected = true;
        }
    }
                                                          ...
                                                          u32 right_register, u32 result_register)
{
    ...
    if (machine_x64_type_is_vector_register(selector->program, operand_type) && selector->vector_registers_supported && element &&
...
// A 128-bit shift in any of the three directions: the result is 128 bits wide
// too, which is what separates it from the comparisons below, whose operands
...
// A binary whose *operands* are 128-bit integers, which is the whole class this
// selector dispatches: the shifts and the arithmetic answer 128 bits, the
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_i128_halves(MachineX64Selector* selector, IrInstruction* instruction, u32* left_low, u32* left_high,
                                                        u32* right_low, u32* right_high)
{
    ...
    {
        *left_low = machine_x64_select_frame_load64(selector, left_slot, 0);
        *left_high = machine_x64_select_frame_load64(selector, left_slot, 8);
        *right_low = machine_x64_select_frame_load64(selector, right_slot, 0);
        *right_high = machine_x64_select_frame_load64(selector, right_slot, 8);
        selected = true;
    }
    return selected;
}

// One three-operand SSA row into a fresh synthesized register. Only the
// two-address SSA arithmetic opcodes may be spelled this way; the shifts and
// the divides keep their constrained two-row shape.
BUSTER_GLOBAL_LOCAL u32 machine_x64_select_arithmetic_row(MachineX64Selector* selector, u16 opcode, u32 left_register, u32 right_register)
{
    u32 result = machine_x64_synthesize_register(selector);
    machine_x64_select_row(selector, (MachineInstruction){
                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result),
...
// a two-step sequence ending in MOVZX, so the boolean it leaves is zero
// extended to sixty-four bits and can be blended with AND64/OR64 below.
BUSTER_GLOBAL_LOCAL u32 machine_x64_select_compare_set(MachineX64Selector* selector, u32 left_register, u32 right_register, u32 condition)
{
    machine_x64_select_row(selector, (MachineInstruction){
                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, left_register),
                                                      machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, right_register)},
                                         .opcode = MACHINE_X64_CMP64,
                                     });
    u32 result = machine_x64_synthesize_register(selector);
    machine_x64_select_row(selector, (MachineInstruction){
                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result)},
...
// borrow as SETcc booleans off the low halves, equality through per-half XOR,
// and the eight ordered comparisons as the strict high-half relation blended
// with the unsigned low-half one behind high-half equality.
//
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_i128_binary(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    ...
    else if (machine_x64_binary_is_i128_shift(selector, instruction))
    {
        ...
        else
        {
            selected = machine_x64_select_i128_variable_shift(selector, instruction);
             ...
             result_slot != UINT32_MAX && machine_x64_select_i128_halves(selector, instruction, &left_low, &left_high, &right_low, &right_high))
    {
        u16 opcode = operation == IR_BINARY_INTEGER_BITWISE_AND  ? MACHINE_X64_AND64
                     : operation == IR_BINARY_INTEGER_BITWISE_OR ? MACHINE_X64_OR64
                                                                 : MACHINE_X64_XOR64;
        machine_x64_select_frame_store64(selector, result_slot, 0, machine_x64_select_arithmetic_row(selector, opcode, left_low, right_low));
        machine_x64_select_frame_store64(selector, result_slot, 8, machine_x64_select_arithmetic_row(selector, opcode, left_high, right_high));
        selected = true;
    }
             ...
             machine_x64_select_i128_halves(selector, instruction, &left_low, &left_high, &right_low, &right_high))
    {
        if (operation == IR_BINARY_INTEGER_ADD)
        {
            ...
            u32 sum_low = machine_x64_select_arithmetic_row(selector, MACHINE_X64_ADD64, left_low, right_low);
            u32 carry = machine_x64_select_compare_set(selector, sum_low, left_low, MACHINE_X64_CONDITION_BELOW);
            u32 sum_high = machine_x64_select_arithmetic_row(selector, MACHINE_X64_ADD64, left_high, right_high);
            machine_x64_select_frame_store64(selector, result_slot, 0, sum_low);
            machine_x64_select_frame_store64(selector, result_slot, 8, machine_x64_select_arithmetic_row(selector, MACHINE_X64_ADD64, sum_high, carry));
        }
        else
        {
            // The borrow is low_a <u low_b, read before the halves move.
            u32 borrow = machine_x64_select_compare_set(selector, left_low, right_low, MACHINE_X64_CONDITION_BELOW);
            u32 difference_low = machine_x64_select_arithmetic_row(selector, MACHINE_X64_SUB64, left_low, right_low);
            u32 difference_high = machine_x64_select_arithmetic_row(selector, MACHINE_X64_SUB64, left_high, right_high);
            machine_x64_select_frame_store64(selector, result_slot, 0, difference_low);
             ...
             machine_x64_select_i128_halves(selector, instruction, &left_low, &left_high, &right_low, &right_high))
    {
        ...
        selected = true;
             ...
             machine_x64_select_i128_halves(selector, instruction, &left_low, &left_high, &right_low, &right_high))
    {
        u32 low_difference = machine_x64_select_arithmetic_row(selector, MACHINE_X64_XOR64, left_low, right_low);
        u32 high_difference = machine_x64_select_arithmetic_row(selector, MACHINE_X64_XOR64, left_high, right_high);
        u32 any_difference = machine_x64_select_arithmetic_row(selector, MACHINE_X64_OR64, low_difference, high_difference);
        machine_x64_select_row(selector, (MachineInstruction){
                                             .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, any_difference),
                                                          machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, machine_x64_select_immediate_register(selector, 0))},
                                             .opcode = MACHINE_X64_CMP64,
                                         });
        u32 row = machine_x64_select_row(
            selector, (MachineInstruction){
                          .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register)},
                          .payload = operation == IR_BINARY_INTEGER_EQUAL ? MACHINE_X64_CONDITION_EQUAL : MACHINE_X64_CONDITION_NOT_EQUAL,
                          .opcode = MACHINE_X64_SETCC,
                      });
        machine_x64_define(selector, result_register, row);
        selected = true;
             ...
             result_register != UINT32_MAX && machine_x64_select_i128_halves(selector, instruction, &left_low, &left_high, &right_low, &right_high))
    {
        // The high halves decide with the signed or unsigned strict relation;
        // only exactly equal high halves defer to the low halves, which always
        // compare unsigned.
        bool less_directed = operation == IR_BINARY_SIGNED_LESS || operation == IR_BINARY_SIGNED_LESS_EQUAL || operation == IR_BINARY_UNSIGNED_LESS ||
                             operation == IR_BINARY_UNSIGNED_LESS_EQUAL;
        bool signed_compare = operation == IR_BINARY_SIGNED_LESS || operation == IR_BINARY_SIGNED_LESS_EQUAL || operation == IR_BINARY_SIGNED_GREATER ||
        ...
        u32 high_condition = signed_compare ? (less_directed ? MACHINE_X64_CONDITION_LESS : MACHINE_X64_CONDITION_GREATER)
                                            : (less_directed ? MACHINE_X64_CONDITION_BELOW : MACHINE_X64_CONDITION_ABOVE);
        u32 low_condition = less_directed ? (or_equal ? MACHINE_X64_CONDITION_BELOW_EQUAL : MACHINE_X64_CONDITION_BELOW)
                                          : (or_equal ? MACHINE_X64_CONDITION_ABOVE_EQUAL : MACHINE_X64_CONDITION_ABOVE);
        u32 high_decides = machine_x64_select_compare_set(selector, left_high, right_high, high_condition);
        u32 high_equal = machine_x64_select_compare_set(selector, left_high, right_high, MACHINE_X64_CONDITION_EQUAL);
        u32 low_decides = machine_x64_select_compare_set(selector, left_low, right_low, low_condition);
        u32 deferred = machine_x64_select_arithmetic_row(selector, MACHINE_X64_AND64, high_equal, low_decides);
        u32 row = machine_x64_select_row(selector, (MachineInstruction){
                                                       .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register),
...
BUSTER_GLOBAL_LOCAL u32 machine_x64_select_vector_snapshot(MachineX64Selector* selector, IrValueId value, IrType* vector)
{
    ...
    if (slot == UINT32_MAX && value_register != UINT32_MAX && machine_x64_type_is_vector_register(selector->program, vector) && selector->vector_registers_supported)
    {
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_vector_lanes(MachineX64Selector* selector, IrInstruction* instruction, IrType* vector)
{
    ...
    bool register_result = result_slot == UINT32_MAX && instruction->result.value < function->value_count &&
                           selector->value_virtual_registers[instruction->result.value] != UINT32_MAX &&
                           machine_x64_type_is_vector_register(selector->program, vector) && selector->vector_registers_supported;
    if (register_result)
    {
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_binary(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    ...
    else if (machine_x64_type_is_f80(selector, function->values[instruction->operands[0].value].canonical_type))
    {
        selected = machine_x64_select_f80_binary(selector, instruction, result_register);
    }
    else if (machine_x64_binary_is_i128(selector, instruction))
    {
        // i128 operands are slot-backed with no operand register, so the
        // register path below cannot see them at all: anything this subset
        // refuses refuses here and rides the canonical emitter.
        selected = machine_x64_select_i128_binary(selector, instruction, result_register);
    }
                                                           ...
                                                           u64 field_offset, u64 field_size, u8* member_emitted)
{
    ...
    for (u32 piece_index = 0; piece_index < piece_count && selected; piece_index += 1)
    {
        ...
        u16 load_opcode = pieces[piece_index].size == 1   ? MACHINE_X64_LOAD_PTR8
                          : pieces[piece_index].size == 2 ? MACHINE_X64_LOAD_PTR16
                          : pieces[piece_index].size == 4 ? MACHINE_X64_LOAD_PTR32
                                                         : MACHINE_X64_LOAD_PTR64;
        u16 store_opcode = pieces[piece_index].size == 1   ? MACHINE_X64_STORE_FRAME8
                           : pieces[piece_index].size == 2 ? MACHINE_X64_STORE_FRAME16
                           : pieces[piece_index].size == 4 ? MACHINE_X64_STORE_FRAME32
                                                          : MACHINE_X64_STORE_FRAME64;
        ...
        for (u32 sibling = first; sibling < instruction->operand_count && selected; sibling += 1)
        {
                ...
                ir_field_access_size(&selector->program->types, type->fields + sibling_field) == field_size)
            {
                ...
                {
                    u32 value_register;
                    selected = machine_x64_operand_register(selector, instruction->operands[sibling], &value_register);
                    if (selected)
                    {
                        ...
                        if (low > bit_offset)
                        {
                            u32 count_register = machine_x64_select_immediate_register(selector, low - bit_offset);
                            machine_x64_select_row(selector, (MachineInstruction){
                                                                 .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, masked_register),
                        ...
                        u32 mask_register = machine_x64_select_immediate_register(selector, high - low == 64 ? UINT64_MAX : (((u64)1 << (high - low)) - 1));
                        machine_x64_select_row(selector, (MachineInstruction){
                                                             .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, masked_register),
                        ...
                        if (low > piece_low)
                        {
                            u32 scale_register = machine_x64_select_immediate_register(selector, (u64)1 << (low - piece_low));
                            machine_x64_select_row(selector, (MachineInstruction){
                                                                 .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, masked_register),
                                                                              machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, masked_register),
    ...
    else
    {
        ...
        for (u32 index = 0; index < instruction->operand_count && selected; index += 1)
        {
            ...
            else if (!member_emitted[index])
            {
                ...
                {
                    selected = machine_x64_select_bit_field_unit(selector, instruction, type, slot, index, field_offset, field_size, member_emitted);
                }
            }
        }
    }
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_array(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    u32 slot = selector->value_stack_slots[instruction->result.value];
    u32 result_register = selector->value_virtual_registers[instruction->result.value];
    bool vector = selector->vector_registers_supported && machine_x64_type_is_vector_register(selector->program, type) && result_register != UINT32_MAX;
    if (vector)
    {
        // A vector literal uses the same canonical element constructor as an
    ...
    if (type && (type->kind == IR_TYPE_ARRAY || type->kind == IR_TYPE_VECTOR) && slot != UINT32_MAX)
    {
        ...
        u64 byte_size = (u64)instruction->operand_count * element_size;
        if (element_size && byte_size <= INT32_MAX && (!vector || byte_size == 64))
        {
            selected = true;
            for (u32 index = 0; index < instruction->operand_count && selected; index += 1)
            {
    ...
    {
        u32 row = machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register), machine_ref_make(MACHINE_REF_STACK_SLOT, slot)},
            .opcode = MACHINE_X64_VLOAD_FRAME,
        });
        machine_x64_define(selector, result_register, row);
    }
    ...
    else if (value_valid)
    {
        ...
        if (place_kind != MACHINE_X64_PLACE_NONE)
        {
            ...
            else if (is_aggregate_load & (instruction->opcode == IR_OPCODE_ATOMIC_LOAD))
            {
                ...
                u64 loaded_size = loaded_type && loaded_type->layout.resolved ? loaded_type->layout.size : 0;
                bool integer128 = loaded_class.kind == IR_TYPE_INTEGER && (loaded_class.flags & MACHINE_TYPE_CLASS_INTEGER128) != 0;
                if ((aggregate_kind || integer128) && atomic_width == 16 && loaded_size >= 9 && loaded_size <= 16 &&
                    target_cpu_feature_has(selector->target, TARGET_CPU_FEATURE_X86_CX16))
                {
                    u32 address_register = machine_x64_synthesize_register(selector);
                    ...
                    if (selected)
                    {
                        machine_x64_select_row(selector, (MachineInstruction){
                                                             .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot),
                                                                          ...
                                                                          machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot),
                                                                          machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address_register)},
                                                             .payload = (u32)loaded_size,
                                                             .opcode = MACHINE_X64_ATOMIC_LOAD16,
                                                         });
                    }
                }
                else if (aggregate_kind && width_index != UINT32_MAX)
                {
                    u32 address_register = machine_x64_synthesize_register(selector);
                    selected = machine_x64_select_place_address(selector, value_id, address_register);
                    if (selected)
                ...
                if (aggregate_supported)
                {
                    ...
                    if (selected)
                    {
                        ...
                        machine_x64_select_row(selector, (MachineInstruction){
                                                             ...
                                                             .payload = machine_x64_type_is_f80(selector, instruction->canonical_type) ? 10u : (u32)loaded_type->layout.size,
                                                             .opcode = opcode,
                                                         });
                    }
                }
                                                            ...
                                                            String8* source_out, AssemblyEncodeResult* encoded_out)
{
    ...
    while (selected && line_start < resolved.length)
    {
        ...
        while (line_end < resolved.length && resolved.pointer[line_end] != '\n')
        {
            line_end += 1;
        }
        String8 line = {.pointer = resolved.pointer + line_start, .length = line_end - line_start};
        while (line.length && (line.pointer[0] == ' ' || line.pointer[0] == '\t'))
        {
            line.pointer += 1;
            line.length -= 1;
        }
        if (line.length && line.pointer[0] == '.')
        {
            ...
            selected = codegen_global_assembly_apply_symbol_directive(selector->program, selector->target, line, extra.literal, &recognized) && recognized;
        }
        else if (line.length)
        {
            memcpy(instructions + instruction_length, line.pointer, line.length);
            instruction_length += line.length;
            instructions[instruction_length++] = '\n';
        }
        line_start = line_end < resolved.length ? line_end + 1 : resolved.length;
    }
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_inline_assembly(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    u8 sizes[MACHINE_X64_INLINE_ASSEMBLY_OPERAND_LIMIT] = {0};
    u8 operand_flags[MACHINE_X64_INLINE_ASSEMBLY_OPERAND_LIMIT] = {0};
    bool used[48] = {0};
    bool reserved[48] = {0};
    bool clobbered[48] = {0};
    u8 fixed_count[48] = {0};
    u8 fixed_output_count[48] = {0};
    u8 fixed_input_count[48] = {0};
    u8 fixed_early_clobber[48] = {0};
    ...
    for (u32 index = 0; selected && index < extra.clobber_count; index += 1)
    {
        ...
        else if (string_equal(extra.clobbers[index], S8("cc")))
        {
            effects |= MACHINE_INLINE_ASSEMBLY_EFFECT_FLAGS;
        }
        else if (codegen_inline_assembly_clobber_register(extra.clobbers[index], &clobber))
        {
    ...
    for (u32 index = 0; selected && index < instruction->operand_count; index += 1)
    {
        ...
        if (!(instruction->immediates[index] & IR_INLINE_ASSEMBLY_CONSTRAINT_MATCH) &&
            codegen_inline_assembly_constraint_register(instruction->immediates[index], &fixed))
        {
            // GNU asm deliberately permits multiple operands to alias one
            // fixed register (for example `=a` with an `a` input), but the
            // alias is only valid for one output and one input. Keep direct
            // IR fail-closed for duplicate inputs, duplicate outputs, and
            // early-clobber pairs; the C frontend rejects those before this
            // selector, while hand-built IR still needs the same boundary.
            selected = !clobbered[fixed];
            reserved[fixed] = true;
            fixed_count[fixed] = (u8)(fixed_count[fixed] + 1u);
            fixed_output_count[fixed] = (u8)(fixed_output_count[fixed] +
                                             (u8)((instruction->immediates[index] & IR_INLINE_ASSEMBLY_CONSTRAINT_OUTPUT) != 0));
            fixed_input_count[fixed] = (u8)(fixed_input_count[fixed] +
                                            (u8)((instruction->immediates[index] & IR_INLINE_ASSEMBLY_CONSTRAINT_OUTPUT) == 0 ||
                                                 (instruction->immediates[index] & IR_INLINE_ASSEMBLY_CONSTRAINT_READ_WRITE) != 0));
            fixed_early_clobber[fixed] |= (u8)((instruction->immediates[index] & IR_INLINE_ASSEMBLY_CONSTRAINT_EARLY_CLOBBER) != 0);
        }
    }
    for (u32 fixed = 0; selected && fixed < 48; fixed += 1)
    {
        bool repeated = fixed_count[fixed] > 1;
        bool valid_alias = fixed_count[fixed] == 2 && fixed_output_count[fixed] == 1 && fixed_input_count[fixed] == 1 && !fixed_early_clobber[fixed];
        selected = !repeated || valid_alias;
    }
    for (u32 index = 0; selected && index < instruction->operand_count; index += 1)
    {
        ...
        if (selected && (constraint & IR_INLINE_ASSEMBLY_CONSTRAINT_MATCH))
        {
            ...
            if (selected)
            {
                ...
                vector_registers[index] = vector_registers[match];
        ...
        {
            u32 candidate = 0;
            while (candidate < 8 && (used[16 + candidate] || reserved[16 + candidate]))
            {
                candidate += 1;
            }
        ...
        else if (selected && x87)
        {
            registers[index] = X64_REGISTER_RAX;
            slots[index] = machine_x64_append_slot(selector, 16, 16);
        }
        else if (selected)
        {
            X64Register fixed = X64_REGISTER_RAX;
            bool has_fixed = codegen_inline_assembly_constraint_register(constraint, &fixed);
            u32 candidate = has_fixed ? (u32)fixed : 0;
            while (!has_fixed && candidate < 16 &&
                   (candidate == MACHINE_X64_RSP || candidate == MACHINE_X64_RBP || used[candidate] || reserved[candidate]))
            {
                candidate += 1;
            }
            selected = candidate < 16 && (has_fixed || !used[candidate]);
            if (selected)
            {
                registers[index] = (X64Register)candidate;
                used[candidate] = true;
                exact_clobbers |= UINT64_C(1) << candidate;
                slots[index] = machine_x64_append_slot(selector, 8, 8);
            }
        }
        if (selected)
        {
            ...
            operand_flags[index] = (u8)(((constraint & IR_INLINE_ASSEMBLY_CONSTRAINT_OUTPUT) ? MACHINE_INLINE_ASSEMBLY_OPERAND_OUTPUT : 0) |
                                        ...
                                        (vector ? MACHINE_INLINE_ASSEMBLY_OPERAND_VECTOR : 0) |
                                        (constraint_class == IR_INLINE_ASSEMBLY_CONSTRAINT_T ? MACHINE_INLINE_ASSEMBLY_OPERAND_X87_TOP : 0) |
                                        (constraint_class == IR_INLINE_ASSEMBLY_CONSTRAINT_U ? MACHINE_INLINE_ASSEMBLY_OPERAND_X87_BELOW : 0) |
    ...
    for (u32 index = 0; selected && index < instruction->operand_count; index += 1)
    {
        if (!(operand_flags[index] & MACHINE_INLINE_ASSEMBLY_OPERAND_INPUT))
        {
            continue;
        }
        ...
        if (x87)
        {
            u32 source_slot = source.value < function->value_count ? selector->value_stack_slots[source.value] : UINT32_MAX;
            selected = source_slot != UINT32_MAX;
            if (selected && source_slot != slots[index])
            {
                machine_x64_select_row(selector, (MachineInstruction){
        ...
        else if (memory)
        {
            ...
            selected = machine_x64_select_place_address(selector, source, source_register);
        }
        else if ((constraint & IR_INLINE_ASSEMBLY_CONSTRAINT_READ_WRITE) != 0)
        {
            source_register = machine_x64_synthesize_register(selector);
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_store(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    if (instruction->operands[0].value < function->value_count && instruction->operands[1].value < function->value_count)
    {
        ...
        if (place_kind != MACHINE_X64_PLACE_NONE)
        {
            ...
            if (machine_x64_type_is_f80(selector, stored_type_id))
            {
                size = 10;
            }
                ...
                selector->value_virtual_registers[instruction->operands[1].value] == UINT32_MAX)
            {
                ...
                bool integer128 = stored_class.kind == IR_TYPE_INTEGER && (stored_class.flags & MACHINE_TYPE_CLASS_INTEGER128) != 0;
                bool wide_atomic = (aggregate_kind || integer128) && atomic_width == 16 && size >= 9 && size <= 16 &&
                                   target_cpu_feature_has(selector->target, TARGET_CPU_FEATURE_X86_CX16);
                if (wide_atomic)
                {
                    // CMPXCHG16B is the only machine form for a sixteen-byte
                    // atomic store. The desired image remains in its frame
                    // slot; the constrained row carries the place address in
                    // slot 3 so SysV uses RSI and Win64 uses volatile R11.
                    u32 address_register = machine_x64_synthesize_register(selector);
                    selected = machine_x64_select_place_address(selector, instruction->operands[0], address_register);
                    if (selected)
                    {
                ...
                else if (aggregate_kind && size && size <= 8 && width_index != UINT32_MAX)
                {
...
BUSTER_GLOBAL_LOCAL MachineX64SignaturePlan const* machine_x64_signature_plan(MachineX64Selector* selector, IrTypeId callee_type_id, IrType* callee_type)
{
    ...
    if (!plan->built)
    {
        ...
        // System V variadic vector arguments need a separate AL/overflow
        // contract. Win64 vectors use the ordinary indirect pointer placement.
        ...
        for (u32 parameter_index = 0; parameter_index < plan->parameter_count && arguments_supported; parameter_index += 1)
        {
            ...
            arguments_supported = machine_x64_value_shape(program, callee_type->parameter_types[parameter_index], IR_ABI_USE_ARGUMENT, target, shape) &&
                                  !(plan->variadic && !windows && shape->vector);
            arguments_supported = arguments_supported && machine_x64_argument_area_fits(shape, plan->stack_part_count);
    ...
    *plan = (MachineX64CallPlan){.callee_register = UINT32_MAX, .indirect_result_slot = UINT32_MAX,
    ...
    if (planned)
    {
        ...
        planned = callee_type && callee_type->kind == IR_TYPE_FUNCTION &&
                  ...
                  plan->argument_count <= selector->call_argument_capacity;
    }
    ...
    if (planned)
    {
        ...
        if (plan->argument_count > signature->parameter_count)
        {
            ...
            for (u32 argument_index = signature->parameter_count; argument_index < plan->argument_count && planned; argument_index += 1)
            {
                ...
                // System V's AL protocol would have to count vector registers;
...
// Moves one consecutive direct-result/argument vector part between a frame
// slot and its fixed ABI register. XMM/YMM have compact dedicated rows; a ZMM
// part uses the existing pointer transfer after materializing the subobject
// address, preserving the same unaligned frame contract.
BUSTER_GLOBAL_LOCAL void machine_x64_select_vector_frame_part(MachineX64Selector* selector, bool load, u32 physical_register,
                                                              u32 slot, u32 offset, u32 part_bytes)
{
    if (part_bytes == 64)
    {
        u32 address = machine_x64_synthesize_register(selector);
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address), machine_ref_make(MACHINE_REF_STACK_SLOT, slot)},
            .payload = offset,
            .opcode = MACHINE_X64_LEA_FRAME,
        });
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {load ? machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, physical_register)
                              : machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address),
                         load ? machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address)
                              : machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, physical_register)},
            .opcode = load ? MACHINE_X64_VLOAD_PTR : MACHINE_X64_VSTORE_PTR,
        });
    }
    else
    {
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, physical_register),
                         machine_ref_make(MACHINE_REF_STACK_SLOT, slot)},
            .payload = offset,
            .opcode = part_bytes == 32 ? (load ? MACHINE_X64_LOAD_YMM_FRAME256 : MACHINE_X64_STORE_YMM_FRAME256)
                                       : (load ? MACHINE_X64_LOAD_XMM_FRAME128 : MACHINE_X64_STORE_XMM_FRAME128),
        });
    }
}

// The scalar lane forms use the same sized memory vocabulary as ordinary
// scalar selection. Loads materialize an address because the frame has one
// generic 64-bit load row, while stores already publish exact-width rows.
BUSTER_GLOBAL_LOCAL u16 machine_x64_frame_store_opcode(u32 byte_size)
{
    u16 result = byte_size == 1   ? MACHINE_X64_STORE_FRAME8
                 : byte_size == 2 ? MACHINE_X64_STORE_FRAME16
                 : byte_size == 4 ? MACHINE_X64_STORE_FRAME32
                                  : MACHINE_X64_STORE_FRAME64;
    return result;
}

BUSTER_GLOBAL_LOCAL u32 machine_x64_select_frame_scalar_load(MachineX64Selector* selector, u32 slot, u32 offset, u32 byte_size)
{
    u32 address = machine_x64_synthesize_register(selector);
    machine_x64_select_row(selector, (MachineInstruction){
                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address),
    ...
    u32 value = machine_x64_synthesize_register(selector);
    u16 opcode = byte_size == 1   ? MACHINE_X64_LOAD_PTR8
                 : byte_size == 2 ? MACHINE_X64_LOAD_PTR16
                 : byte_size == 4 ? MACHINE_X64_LOAD_PTR32
                                  : MACHINE_X64_LOAD_PTR64;
    machine_x64_select_row(selector, (MachineInstruction){
                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, value),
                                                      machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address)},
...
BUSTER_GLOBAL_LOCAL u32 machine_x64_return_integer_register(u32 index)
{
    static u32 const registers[] = {MACHINE_X64_RAX, MACHINE_X64_RDX, MACHINE_X64_RCX};
    u32 result = index < BUSTER_ARRAY_LENGTH(registers) ? registers[index] : MACHINE_X64_RAX;
    return result;
}

BUSTER_GLOBAL_LOCAL u32 machine_x64_shape_part_size(MachineX64ValueShape const* shape, u32 part_index)
{
    u32 result = part_index < BUSTER_ARRAY_LENGTH(shape->part_sizes) && shape->part_sizes[part_index]
                     ? shape->part_sizes[part_index]
                     : 8u;
    return result;
}
...
{
    // Stack and indirect vector arguments bounce through a dedicated
    // 64-byte frame slot. Pushes and private-copy staging then have memory
    // to read, independent of the outgoing stack pointer.
    for (u32 argument_index = 0; argument_index < plan->argument_count; argument_index += 1)
    {
        if (!plan->argument_shapes[argument_index].vector ||
            (!plan->argument_placements[argument_index].on_stack && !plan->argument_shapes[argument_index].indirect))
        {
            continue;
        }
    ...
    if (plan->windows_call)
    {
        ...
        for (u32 argument_index = 0; argument_index < plan->argument_count; argument_index += 1)
        {
            ...
            for (u32 part_index = 0; part_index < argument_placement->stack_part_count; part_index += 1)
            {
                ...
                if (shape->windows_scalar_lane_size)
                {
                    u32 lane = (u32)argument_placement->register_part_count + part_index;
                    part_register = machine_x64_select_frame_scalar_load(selector, plan->argument_slots[argument_index],
                                                                          lane * shape->windows_scalar_lane_size,
                                                                          shape->windows_scalar_lane_size);
                }
                else if (shape->indirect && shape->vector_part_bytes)
                {
                    u32 piece = (u32)argument_placement->register_part_count + part_index;
                    if (piece)
                    {
                        ...
                        machine_x64_select_row(selector, (MachineInstruction){
                            .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, part_register),
                                         machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, plan->argument_registers[argument_index])},
                            .payload = piece * shape->vector_part_bytes,
                            .opcode = MACHINE_X64_LEA_OFFSET,
                        });
                    }
                }
                else if (shape->aggregate && !shape->indirect)
                {
                    part_register = machine_x64_synthesize_register(selector);
                    machine_x64_select_row(selector, (MachineInstruction){
                                                         .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, part_register),
    ...
    for (u32 populate_pass = 0; populate_pass < 2; populate_pass += 1)
    {
        ...
        for (u32 argument_index = 0; argument_index < plan->argument_count; argument_index += 1)
        {
            MachineX64ValueShape const* shape = plan->argument_shapes + argument_index;
            MachineX64ArgumentPlacement const* placement = plan->argument_placements + argument_index;
            if (placement->on_stack && (!plan->windows_call || placement->register_part_count == 0))
            {
                continue;
            }
            u32 next_integer = placement->first_integer;
            u32 next_float = placement->first_float;
            if (shape->windows_scalar_lane_size)
            {
                bool lane_float = shape->windows_scalar_float;
                bool stage_registers = lane_float == float_pass;
                bool duplicate_variadic = lane_float && !float_pass && plan->variadic_call;
                if (stage_registers || duplicate_variadic)
                {
                    for (u32 lane = 0; lane < placement->register_part_count; lane += 1)
                    {
                        ...
                        {
                            machine_x64_select_row(selector, (MachineInstruction){
                                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value)},
                                .payload = slot,
                        ...
                        {
                            machine_x64_select_row(selector, (MachineInstruction){
                                .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, machine_x64_argument_register(true, slot)),
                                             machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value)},
                                .opcode = MACHINE_X64_MOV_RR,
                            });
                        }
                    }
                    ...
                    for (u32 part = 0; part < register_parts; part += 1)
                    {
                        ...
                        machine_x64_select_row(selector, (MachineInstruction){
                            .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER,
                                         ...
                                         machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, pointer)},
                            .opcode = MACHINE_X64_MOV_RR,
                        });
                    }
                }
            ...
            if (shape->vector_part_bytes)
            {
                if (float_pass)
                {
                    for (u32 part = 0; part < shape->part_count; part += 1)
                    {
                        machine_x64_select_vector_frame_part(selector, true, MACHINE_X64_ZMM0 + next_float + part,
                                                             plan->argument_slots[argument_index], part * shape->vector_part_bytes,
                                                             shape->vector_part_bytes);
                    }
                    call_vector_registers |= shape->vector_part_bytes >= 32;
                }
                continue;
            }
            if (shape->xmm128)
            ...
            if (shape->part_is_float[0])
            {
                ...
                else if (plan->windows_call && plan->variadic_call)
                {
                    machine_x64_select_row(selector, (MachineInstruction){
                        .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, machine_x64_argument_register(true, next_integer)),
                                     machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, plan->argument_registers[argument_index])},
                        .opcode = MACHINE_X64_MOV_RR,
                    });
                }
    ...
    return (u16)((plan->variadic_call && !plan->windows_call ? (1u | (plan->float_count << 1)) : 0) |
                                                         ...
                                                         u32 result_register)
{
    ...
    if (instruction->result.value != IR_ID_UNDERLYING_INVALID)
    {
        if (plan->return_shape.f80_result)
        {
            u32 result_slot = selector->value_stack_slots[instruction->result.value];
            received = result_slot != UINT32_MAX;
            if (received)
            {
                for (u32 part = 0; part < plan->return_shape.f80_result; part += 1)
                {
                    machine_x64_select_row(selector, (MachineInstruction){
                        .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot)}, .payload = part * 16u,
                        .opcode = MACHINE_X64_F80_RESULT_STORE});
                }
            }
        }
        ...
        else if (plan->return_shape.vector_part_bytes)
        {
            u32 result_slot = selector->value_stack_slots[instruction->result.value];
            received = result_slot != UINT32_MAX;
            for (u32 part = 0; received && part < plan->return_shape.part_count; part += 1)
            {
                machine_x64_select_vector_frame_part(selector, false, MACHINE_X64_ZMM0 + part, result_slot,
                                                     part * plan->return_shape.vector_part_bytes, plan->return_shape.vector_part_bytes);
            }
        }
        ...
        else if (plan->return_shape.aggregate)
        {
            ...
            if (received && plan->return_shape.x87_f64_result)
            {
                machine_x64_select_row(selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot)},
                    .payload = 24,
            ...
            for (u32 capture_pass = 0; capture_pass < 2 && received; capture_pass += 1)
            {
                ...
                {
                    if (plan->return_shape.x87_f64_result && part_index == 2)
                    {
                        continue;
                    }
                        ...
                        machine_x64_select_row(selector, (MachineInstruction){
                                                             .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot),
                                                                          machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER,
                                                                                           machine_x64_return_integer_register(return_integer_index))},
                                                             .payload = plan->return_shape.part_offsets[part_index],
                                                             .opcode = machine_x64_frame_store_opcode(
                                                                 machine_x64_shape_part_size(&plan->return_shape, part_index)),
                                                         });
    ...
    if (instruction->result.value == IR_ID_UNDERLYING_INVALID && plan->return_shape.f80_result)
    {
        // Ignoring a C result still consumes the callee's x87 stack value.
        u32 discard = machine_x64_append_slot(selector, 16, 16);
        for (u32 part = 0; part < plan->return_shape.f80_result; part += 1)
        {
            machine_x64_select_row(selector, (MachineInstruction){
                .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, discard)}, .opcode = MACHINE_X64_F80_RESULT_STORE});
        }
    }
    else if (instruction->result.value == IR_ID_UNDERLYING_INVALID && plan->return_shape.x87_f64_result)
    {
        u32 discard = machine_x64_append_slot(selector, 8, 8);
        machine_x64_select_row(selector, (MachineInstruction){
            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, discard)},
            .payload = 8,
            .opcode = MACHINE_X64_F80_RESULT_STORE,
        });
    }
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_atomic_read_modify_write(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    ...
    if (size == 16)
    {
        IrFunction* function = selector->function;
        u32 result_slot = instruction->result.value < function->value_count ? selector->value_stack_slots[instruction->result.value] : UINT32_MAX;
        u32 value_slot = instruction->operand_count >= 2 && instruction->operands[1].value < function->value_count
                             ? selector->value_stack_slots[instruction->operands[1].value]
                             : UINT32_MAX;
        u32 address_register = machine_x64_synthesize_register(selector);
        selected = atomic_type && atomic_type->kind == IR_TYPE_INTEGER && instruction->operand_count >= 2 && result_slot != UINT32_MAX &&
                   value_slot != UINT32_MAX && instruction->atomic_operation < IR_ATOMIC_OPERATION_COUNT &&
                   target_cpu_feature_has(selector->target, TARGET_CPU_FEATURE_X86_CX16) &&
        ...
        if (selected)
        {
            machine_x64_select_row(selector, (MachineInstruction){
                                                         .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, result_slot),
                                                                      ...
                                                                      machine_ref_make(MACHINE_REF_STACK_SLOT, value_slot),
                                                                      machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address_register)},
                                                         .payload = 16u | ((u32)instruction->atomic_operation << 8),
                                                         .opcode = MACHINE_X64_ATOMIC_RMW16,
                                                     });
        }
    }
    else
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_return(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    if (instruction->operand_count)
    {
        ...
        if (selector->return_shape.f80_result)
        {
            selected = value_slot != UINT32_MAX;
            if (selected)
            {
                for (u32 part = selector->return_shape.f80_result; part > 0; part -= 1)
                {
                    machine_x64_select_row(selector, (MachineInstruction){
                        .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, value_slot)}, .payload = (part - 1u) * 16u,
                        .opcode = MACHINE_X64_F80_RESULT_LOAD});
                }
            }
        }
        else if (selector->return_shape.indirect)
        {
        ...
        else if (selector->return_shape.vector_part_bytes)
        {
            selected = value_slot != UINT32_MAX;
            for (u32 part = 0; selected && part < selector->return_shape.part_count; part += 1)
            {
                machine_x64_select_vector_frame_part(selector, true, MACHINE_X64_ZMM0 + part, value_slot,
                                                     part * selector->return_shape.vector_part_bytes, selector->return_shape.vector_part_bytes);
            }
        }
        ...
        else if (selector->return_shape.aggregate)
        {
            ...
            if (selected)
            {
                ...
                for (u32 populate_pass = 0; populate_pass < 2; populate_pass += 1)
                {
                    ...
                    {
                        if (selector->return_shape.x87_f64_result && part_index == 2)
                        {
                            continue;
                        }
                        ...
                        if (!part_float)
                        {
                            return_integer_values[return_integer_count] = machine_x64_select_frame_scalar_load(
                                selector, value_slot, selector->return_shape.part_offsets[part_index],
                                machine_x64_shape_part_size(&selector->return_shape, part_index));
                            return_integer_count += 1;
                            continue;
                        }
                        u32 bounce_register = machine_x64_select_frame_scalar_load(selector, value_slot,
                            selector->return_shape.part_offsets[part_index],
                            machine_x64_shape_part_size(&selector->return_shape, part_index));
                        machine_x64_select_row(selector, (MachineInstruction){
                                                             .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, bounce_register)},
                                                             .payload = return_float_index,
                ...
                for (u32 return_integer_index = return_integer_count; return_integer_index > 1; return_integer_index -= 1)
                {
                    u32 value_index = return_integer_index - 1;
                    machine_x64_select_row(selector, (MachineInstruction){
                                                         .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER,
                                                                                       machine_x64_return_integer_register(value_index)),
                                                                      machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER,
                                                                                       return_integer_values[value_index])},
                                                         .opcode = MACHINE_X64_MOV_RR,
                                                     });
                }
                if (return_integer_count)
                {
                    machine_x64_select_row(selector, (MachineInstruction){
                                                         .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, MACHINE_X64_RAX),
                                                                      machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, return_integer_values[0])},
                                                         .opcode = MACHINE_X64_MOV_RR,
                                                     });
                }
                if (selector->return_shape.x87_f64_result)
                {
                    machine_x64_select_row(selector, (MachineInstruction){
                        .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, value_slot)},
                        .payload = 24,
    ...
    if (selected)
    {
        machine_x64_select_row(selector, (MachineInstruction){
                                             ...
                                             .flags = (u16)(instruction->operand_count && (selector->return_shape.vector || selector->return_shape.vector_part_bytes >= 32)
                                                              ...
                                                              bool preserve_debug_values, MachineSelectionModule* module)
{
    ...
    MachineX64ValueShape* signature_parameter_shapes = arena_allocate(arena, MachineX64ValueShape, function_type->parameter_count);
    MachineX64ArgumentPlacement* signature_parameter_placements = arena_allocate(arena, MachineX64ArgumentPlacement, function_type->parameter_count);
    ...
    for (u32 parameter_index = 0; parameter_index < function_type->parameter_count; parameter_index += 1)
    {
        if (!machine_x64_value_shape(program, function_type->parameter_types[parameter_index], IR_ABI_USE_ARGUMENT, target,
            ...
            !machine_x64_argument_area_fits(signature_parameter_shapes + parameter_index, signature_stack_count))
        {
            return result;
        }
    ...
    selector.parameter_shapes = signature_parameter_shapes;
    ...
    for (u32 argument_index = 0; argument_index < selector.parameter_count; argument_index += 1)
    {
    ...
    for (u32 layout_index = 0; layout_index < function->block_count && selector.supported; layout_index += 1)
    {
        ...
        if (block_index == function->entry.value)
        {
            if (function_type->is_variadic && windows_abi)
            {
                // Home all four incoming GPRs before argument captures can
                // reuse them. The caller's homes adjoin the overflow tail.
                machine_x64_select_row(&selector, (MachineInstruction){
                    .operands = {machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, MACHINE_X64_RCX),
                                 machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, MACHINE_X64_RDX),
                                 machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, MACHINE_X64_R8),
                                 machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER, MACHINE_X64_R9)},
                    .opcode = MACHINE_X64_WIN_VA_SAVE,
                });
            }
            else if (selector.va_register_save_slot != UINT32_MAX)
            {
                machine_x64_select_row(&selector, (MachineInstruction){
            ...
            u32* normalize_registers = arena_allocate(arena, u32, function_type->parameter_count);
            u32* normalize_values = arena_allocate(arena, u32, function_type->parameter_count);
            u16* normalize_opcodes = arena_allocate(arena, u16, function_type->parameter_count);
            u32 normalize_count = 0;
            u32* indirect_bases = arena_allocate(arena, u32, function_type->parameter_count);
            u32 indirect_part_count = 0;
            for (u32 argument_index = 0; argument_index < function_type->parameter_count; argument_index += 1)
            {
                indirect_bases[argument_index] = indirect_part_count;
                if (selector.parameter_shapes[argument_index].indirect)
                {
                    indirect_part_count += selector.parameter_shapes[argument_index].part_count;
                }
            }
            u32* indirect_registers = arena_allocate(arena, u32, indirect_part_count ? indirect_part_count : 1);
            for (u32 capture_pass = 0; capture_pass < 2 && selector.supported; capture_pass += 1)
            {
                ...
                for (u32 argument_index = 0; argument_index < function_type->parameter_count; argument_index += 1)
                {
                    ...
                    if (shape->windows_scalar_lane_size)
                    {
                        ...
                        bool lane_float = shape->windows_scalar_float;
                        if (lane_float == float_pass)
                        {
                            for (u32 lane = 0; lane < parameter_placement->register_part_count; lane += 1)
                            {
                                MachineRef source;
                                if (lane_float)
                                {
                                    u32 lane_value = machine_x64_synthesize_register(&selector);
                                    machine_x64_select_row(&selector, (MachineInstruction){
                                        .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value)},
                                        .payload = (u32)parameter_placement->first_float + lane,
                                        .opcode = MACHINE_X64_MOVQ_FROM_XMM,
                                    });
                                    source = machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value);
                                }
                                else
                                {
                                    source = machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER,
                                                              machine_x64_argument_register(true,
                                                                  (u32)parameter_placement->first_integer + lane));
                                }
                                machine_x64_select_row(&selector, (MachineInstruction){
                                    .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, slot), source},
                                    .payload = lane * shape->windows_scalar_lane_size,
                                    .opcode = machine_x64_frame_store_opcode(shape->windows_scalar_lane_size),
                                });
                            }
                        }
                        if (float_pass)
                        {
                            for (u32 part = 0; part < parameter_placement->stack_part_count; part += 1)
                            {
                                u32 lane_value = machine_x64_synthesize_register(&selector);
                                machine_x64_select_row(&selector, (MachineInstruction){
                                    .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value)},
                                    .payload = incoming_stack_base + ((u32)parameter_placement->first_stack_part + part) * 8,
                                    .opcode = MACHINE_X64_LOAD_INCOMING,
                                });
                                u32 lane = (u32)parameter_placement->register_part_count + part;
                                machine_x64_select_row(&selector, (MachineInstruction){
                                    .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, slot),
                                                 machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, lane_value)},
                                    .payload = lane * shape->windows_scalar_lane_size,
                    ...
                    if (shape->indirect)
                    {
                        ...
                        u32 register_parts = shape->vector_part_bytes ? parameter_placement->register_part_count
                                                                     : parameter_placement->on_stack ? 0u : 1u;
                        u32 capture_count = float_pass ? parameter_placement->stack_part_count : register_parts;
                        for (u32 capture_index = 0; capture_index < capture_count; capture_index += 1)
                        {
                            u32 pointer = machine_x64_synthesize_register(&selector);
                            MachineInstruction capture = {
                                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, pointer)},
                                .payload = incoming_stack_base + ((u32)parameter_placement->first_stack_part + capture_index) * 8,
                                .opcode = MACHINE_X64_LOAD_INCOMING,
                            };
                            if (!float_pass)
                            {
                                capture.operands[1] = machine_ref_make(MACHINE_REF_PHYSICAL_REGISTER,
                                    machine_x64_argument_register(windows_abi, next_integer + capture_index));
                                capture.payload = 0;
                                capture.opcode = MACHINE_X64_MOV_RR;
                            }
                            machine_x64_select_row(&selector, capture);
                            indirect_registers[indirect_bases[argument_index] + (float_pass ? register_parts + capture_index : capture_index)] = pointer;
                        }
                        continue;
                    }
                    ...
                    if (shape->vector_part_bytes)
                    {
                        if (float_pass)
                        {
                            ...
                            for (u32 part = 0; part < shape->part_count; part += 1)
                            {
                                machine_x64_select_vector_frame_part(&selector, false, MACHINE_X64_ZMM0 + next_float + part, slot,
                                                                     part * shape->vector_part_bytes, shape->vector_part_bytes);
                            }
                        }
                        continue;
                    }
                    ...
                    if (!scalar_float && normalize_count < function_type->parameter_count)
                    {
            ...
            if (windows_abi)
            {
                for (u32 argument_index = 0; argument_index < function_type->parameter_count && selector.supported; argument_index += 1)
                {
                    ...
                    if (value != IR_ID_UNDERLYING_INVALID && shape->indirect)
                    {
                        ...
                        u32 vector_register = selector.value_virtual_registers[value];
                        if (shape->vector && vector_register != UINT32_MAX)
                        {
                            u32 row = machine_x64_select_row(&selector, (MachineInstruction){
                                .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, vector_register),
                                             machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER,
                                                              indirect_registers[indirect_bases[argument_index]])},
                                .opcode = MACHINE_X64_VLOAD_PTR,
                            });
                            machine_x64_define(&selector, vector_register, row);
                        }
                        ...
                        else
                        {
                            if (shape->vector_part_bytes)
                            {
                                for (u32 part = 0; part < shape->part_count; part += 1)
                                {
                                    u32 source = indirect_registers[indirect_bases[argument_index] + part];
                                    for (u32 copied = 0; copied < shape->vector_part_bytes; copied += 8)
                                    {
                                        u32 address = source;
                                        if (copied)
                                        {
                                        ...
                                        u32 chunk = machine_x64_synthesize_register(&selector);
                                        machine_x64_select_row(&selector, (MachineInstruction){
                                            .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, chunk),
                                                         machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, address)},
                                            .opcode = MACHINE_X64_LOAD_PTR64,
                                        });
                                        machine_x64_select_row(&selector, (MachineInstruction){
                                            .operands = {machine_ref_make(MACHINE_REF_STACK_SLOT, slot),
                                                         machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, chunk)},
                                            .payload = part * shape->vector_part_bytes + copied,
                                            .opcode = MACHINE_X64_STORE_FRAME64,
                                        });
                                    }
                                }
            ...
            *line_mark_cursor = (MachineLineMark){.row = selector.builder.instructions.total_count, .instruction = id.value};
            ...
            else
            {
                ...
                case IR_OPCODE_UNARY:
                    ...
                    else if (machine_x64_type_class(&selector, instruction->canonical_type).flags & MACHINE_TYPE_CLASS_INTEGER128)
                    {
                ...
                case IR_OPCODE_INLINE_ASSEMBLY:
                    instruction_selected = machine_x64_select_compiler_barrier(&selector, instruction) ||
                                           ...
                                           machine_x64_select_inline_assembly(&selector, instruction);
                    break;
...
BUSTER_GLOBAL_LOCAL BusterX86MetadataPhysicalOperand machine_x64_exact_ymm_operand(u32 reg)
{
    return (BusterX86MetadataPhysicalOperand){
        .kind = BUSTER_X86_METADATA_PHYSICAL_OPERAND_REGISTER, .width = 256,
        .reg = {.index = (u16)reg, .width = 256, .physical_class = BUSTER_X86_METADATA_PHYSICAL_CLASS_YMM},
    };
}
...
BUSTER_GLOBAL_LOCAL MachineX64ShapeMnemonic const machine_x64_shape_mnemonics[] = {
    {S8_INITIALIZER("JB"), 7},
    {S8_INITIALIZER("JZ"), 11},
    {S8_INITIALIZER("OR"), 14},
    {S8_INITIALIZER("ADC"), 79},
    {S8_INITIALIZER("SBB"), 80},
    {S8_INITIALIZER("ADD"), 1},
    {S8_INITIALIZER("AND"), 2},
    {S8_INITIALIZER("CDQ"), 3},
    {S8_INITIALIZER("CMP"), 4},
    {S8_INITIALIZER("CQO"), 5},
    {S8_INITIALIZER("DIV"), 6},
    {S8_INITIALIZER("JMP"), 8},
    {S8_INITIALIZER("JNS"), 9},
    {S8_INITIALIZER("JNZ"), 10},
    {S8_INITIALIZER("LEA"), 12},
    {S8_INITIALIZER("MOV"), 13},
    {S8_INITIALIZER("POP"), 15},
    {S8_INITIALIZER("RET"), 16},
    {S8_INITIALIZER("SHL"), 78},
    {S8_INITIALIZER("SHR"), 17},
    {S8_INITIALIZER("SUB"), 18},
    {S8_INITIALIZER("UD2"), 19},
    {S8_INITIALIZER("XOR"), 20},
    ...
    {S8_INITIALIZER("MOVQ"), 25},
    {S8_INITIALIZER("PUSH"), 26},
    {S8_INITIALIZER("TEST"), 33},
    {S8_INITIALIZER("KORQ"), 44},
    {S8_INITIALIZER("FSTP"), 57},
    {S8_INITIALIZER("FILD"), 58},
    {S8_INITIALIZER("FCHS"), 65},
    {S8_INITIALIZER("SETZ"), 68},
    {S8_INITIALIZER("SETB"), 70},
    {S8_INITIALIZER("SETP"), 74},
    {S8_INITIALIZER("ADDSD"), 27},
    {S8_INITIALIZER("ADDSS"), 28},
    {S8_INITIALIZER("MOVSD"), 29},
    {S8_INITIALIZER("MOVZX"), 30},
    {S8_INITIALIZER("SUBSD"), 31},
    {S8_INITIALIZER("SUBSS"), 32},
    {S8_INITIALIZER("KMOVB"), 45},
    {S8_INITIALIZER("KMOVW"), 46},
    {S8_INITIALIZER("KMOVD"), 47},
    {S8_INITIALIZER("KMOVQ"), 48},
    {S8_INITIALIZER("KANDQ"), 49},
    {S8_INITIALIZER("KXORQ"), 50},
    {S8_INITIALIZER("PAUSE"), 53},
    {S8_INITIALIZER("CPUID"), 54},
    {S8_INITIALIZER("FISTP"), 59},
    {S8_INITIALIZER("FADDP"), 60},
    {S8_INITIALIZER("FSUBP"), 61},
    {S8_INITIALIZER("FMULP"), 62},
    {S8_INITIALIZER("FDIVP"), 63},
    {S8_INITIALIZER("FLDCW"), 67},
    {S8_INITIALIZER("SETNZ"), 69},
    ...
    {S8_INITIALIZER("FUCOMIP"), 64},
    {S8_INITIALIZER("VMOVDQU"), 77},
    {S8_INITIALIZER("CVTSI2SD"), 37},
    {S8_INITIALIZER("CVTSI2SS"), 38},
    {S8_INITIALIZER("VMOVDQU8"), 39},
    {S8_INITIALIZER("CVTTSD2SI"), 40},
    {S8_INITIALIZER("CVTTSS2SI"), 41},
    {S8_INITIALIZER("CMPXCHG16B"), 42},
    {S8_INITIALIZER("VZEROUPPER"), 43},
};
...
BUSTER_GLOBAL_LOCAL BusterX86MetadataPhysicalOperand machine_x64_x87_operand(u32 index)
{
    return (BusterX86MetadataPhysicalOperand){.kind = BUSTER_X86_METADATA_PHYSICAL_OPERAND_REGISTER, .width = 80,
        .reg = {.index = (u16)index, .width = 80, .physical_class = BUSTER_X86_METADATA_PHYSICAL_CLASS_SPECIAL}};
}

BUSTER_GLOBAL_LOCAL void machine_x64_metadata_shape_cache_prepare_x87(void)
{
    BusterX86MetadataPhysicalAttributes attributes = {0};
    BusterX86MetadataFeatureInput features = {.names = machine_x64_sse2_features, .count = BUSTER_ARRAY_LENGTH(machine_x64_sse2_features)};
    String8 memory_names[] = {S8("FLD"), S8("FSTP"), S8("FILD"), S8("FISTP"), S8("FNSTCW"), S8("FLDCW")};
    for (u32 displacement = 0; displacement < 2; displacement += 1)
    {
        s64 offset = displacement ? -256 : -16;
        for (u32 name = 0; name < BUSTER_ARRAY_LENGTH(memory_names); name += 1)
        {
            u32 count = name < 2 ? 3u : 1u;
            for (u32 width = 0; width < count; width += 1)
            {
                u16 bits = name < 2 ? (width == 0 ? 32u : width == 1 ? 64u : 80u) : name < 4 ? 64u : 16u;
                BusterX86MetadataPhysicalOperand operand = machine_x64_exact_rbp_memory_operand(offset, bits, true);
                (void)machine_x64_metadata_shape_cache_add(memory_names[name], &operand, 1, features, attributes);
            }
        }
        for (u32 width = 16; width <= 32; width += 16)
        {
            BusterX86MetadataPhysicalOperand operands[] = {machine_x64_exact_rbp_memory_operand(offset, (u16)width, true),
                machine_x64_exact_immediate_operand(0, (u16)width)};
            (void)machine_x64_metadata_shape_cache_add(S8("MOV"), operands, 2, (BusterX86MetadataFeatureInput){0}, attributes);
        }
    }
    BusterX86MetadataPhysicalOperand pair[] = {machine_x64_x87_operand(1), machine_x64_x87_operand(0)};
    String8 binary_names[] = {S8("FADDP"), S8("FSUBP"), S8("FMULP"), S8("FDIVP")};
    for (u32 name = 0; name < BUSTER_ARRAY_LENGTH(binary_names); name += 1)
    {
        (void)machine_x64_metadata_shape_cache_add(binary_names[name], pair, 1, features, attributes);
    }
    ...
    BusterX86MetadataPhysicalOperand extend[] = {machine_x64_exact_gpr_operand(0, 64), byte};
    (void)machine_x64_metadata_shape_cache_add(S8("MOVZX"), extend, 2, (BusterX86MetadataFeatureInput){0}, attributes);
}
...
{
    BusterX86MetadataPhysicalAttributes attributes = {0};
    String8 binary_names[] = {S8("MOV"), S8("XOR"), S8("ADD"), S8("ADC"), S8("SUB"), S8("SBB"), S8("AND"), S8("OR")};
...
BUSTER_GLOBAL_LOCAL void machine_x64_metadata_shape_cache_prepare_immediates(void)
{
    ...
    String8 names[] = {S8("MOV"), S8("SUB"), S8("SHL"), S8("SHR"), S8("AND"), S8("CMP"), S8("ADD")};
    ...
    for (u32 name_index = 0; name_index < BUSTER_ARRAY_LENGTH(names); name_index += 1)
    {
        for (u32 width_index = 0; width_index < BUSTER_ARRAY_LENGTH(register_widths); width_index += 1)
        {
            ...
            if ((string_equal(names[name_index], S8("SHL")) || string_equal(names[name_index], S8("SHR"))) && register_width != 64) continue;
            ...
            for (u32 immediate_width_index = 0; immediate_width_index < BUSTER_ARRAY_LENGTH(immediate_widths); immediate_width_index += 1)
            {
                ...
                if ((string_equal(names[name_index], S8("SHL")) || string_equal(names[name_index], S8("SHR"))) && immediate_width != 8) continue;
                ...
                s64 representative_immediate = (string_equal(names[name_index], S8("SHL")) || string_equal(names[name_index], S8("SHR"))) && immediate_width == 8
                                                   ? 2
                                                   : (immediate_width == 8 ? 1 : 128);
...
BUSTER_GLOBAL_LOCAL void machine_x64_metadata_shape_cache_prepare_float_vector_atomic(void)
{
    ...
    for (u32 displacement_index = 0; displacement_index < 3; displacement_index += 1)
    {
        ...
        memory_operands[0] = ymm_operand;
        memory_operands[1] = memory_operand;
        (void)machine_x64_metadata_shape_cache_add(S8("VMOVDQU"), memory_operands, 2, avx, attributes);
        memory_operands[0] = memory_operand;
        memory_operands[1] = ymm_operand;
        (void)machine_x64_metadata_shape_cache_add(S8("VMOVDQU"), memory_operands, 2, avx, attributes);
...
BUSTER_GLOBAL_LOCAL bool machine_x64_emit_x87(MachineX64Encoder* encoder, String8 mnemonic,
                                                BusterX86MetadataPhysicalOperand const* operands, u32 count)
{
    return machine_x64_emit_metadata_instruction(encoder, mnemonic, operands, count,
        (BusterX86MetadataFeatureInput){.names = machine_x64_sse2_features, .count = BUSTER_ARRAY_LENGTH(machine_x64_sse2_features)},
        (BusterX86MetadataPhysicalAttributes){0}, 0);
}

BUSTER_GLOBAL_LOCAL void machine_x64_emit_x87_memory(MachineX64Encoder* encoder, String8 mnemonic, s32 displacement, u16 width)
{
    BusterX86MetadataPhysicalOperand operand = machine_x64_exact_rbp_memory_operand(displacement, width, true);
    (void)machine_x64_emit_x87(encoder, mnemonic, &operand, 1);
}

BUSTER_GLOBAL_LOCAL void machine_x64_emit_f80_padding(MachineX64Encoder* encoder, s32 displacement)
{
    (void)machine_x64_emit_metadata_memory_immediate(encoder, S8("MOV"), MACHINE_X64_RBP, displacement + 10, 0, 16, 16, 0);
    (void)machine_x64_emit_metadata_memory_immediate(encoder, S8("MOV"), MACHINE_X64_RBP, displacement + 12, 0, 32, 32, 0);
}

// Every arithmetic/conversion sequence balances its own x87 stack and keeps
// the caller's precision/rounding control. C integer casts temporarily select
// truncation using baseline FISTP; SSE3 FISTTP is not assumed available.
BUSTER_GLOBAL_LOCAL void machine_x64_emit_f80(MachineX64Encoder* encoder, MachineInstruction* instruction, MachineStackPlacement* placement)
{
    s32 offsets[3] = {0};
    bool compare = instruction->opcode == MACHINE_X64_F80_COMPARE;
    u32 count = instruction->opcode == MACHINE_X64_F80_BINARY || compare || instruction->opcode == MACHINE_X64_F80_CONVERT ? 3u :
                instruction->opcode == MACHINE_X64_F80_NEGATE ? 2u : 1u;
    for (u32 operand = compare ? 1u : 0u; operand < count; operand += 1)
    {
        offsets[operand] = -(s32)placement->stack_slot_offsets[machine_ref_payload(instruction->operands[operand])];
    }
    ...
    {
        machine_x64_emit_x87_memory(encoder, S8("FLD"), offsets[1], 80);
        (void)machine_x64_emit_x87(encoder, S8("FCHS"), 0, 0);
        machine_x64_emit_x87_memory(encoder, S8("FSTP"), offsets[0], 80);
        machine_x64_emit_f80_padding(encoder, offsets[0]);
    }
    else if (compare)
    {
        ...
        (void)machine_x64_emit_metadata_instruction(encoder, S8("MOVZX"), extend, 2, (BusterX86MetadataFeatureInput){0}, (BusterX86MetadataPhysicalAttributes){0}, 0);
    }
    else if (instruction->opcode == MACHINE_X64_F80_CONVERT)
    {
        u32 mode = instruction->payload;
        bool to_f80 = mode == 0 || mode == 1 || mode == 4;
        u16 source_width = (u16)(mode == 0 ? 32 : mode == 1 || mode == 4 ? 64 : 80);
        u16 result_width = (u16)(to_f80 ? 80 : mode == 2 ? 32 : 64);
        if (mode == 5)
        {
            machine_x64_emit_x87_memory(encoder, S8("FNSTCW"), offsets[2], 16);
            (void)machine_x64_emit_metadata_register_memory(encoder, S8("MOVZX"), MACHINE_X64_RAX, MACHINE_X64_RBP, offsets[2], 64, 16, 0);
            (void)machine_x64_emit_metadata_register_immediate(encoder, S8("OR"), MACHINE_X64_RAX, 0x0c00, 32, 32, 0);
            (void)machine_x64_emit_metadata_memory_register(encoder, S8("MOV"), MACHINE_X64_RBP, offsets[2] + 2, MACHINE_X64_RAX, 16, 16, 0);
            machine_x64_emit_x87_memory(encoder, S8("FLDCW"), offsets[2] + 2, 16);
        }
        machine_x64_emit_x87_memory(encoder, mode == 4 ? S8("FILD") : S8("FLD"), offsets[1], source_width);
        machine_x64_emit_x87_memory(encoder, mode == 5 ? S8("FISTP") : S8("FSTP"), offsets[0], result_width);
        if (mode == 5)
        {
            machine_x64_emit_x87_memory(encoder, S8("FLDCW"), offsets[2], 16);
        }
        if (to_f80)
        {
            machine_x64_emit_f80_padding(encoder, offsets[0]);
        ...
        {
            (void)machine_x64_emit_metadata_memory_immediate(encoder, S8("MOV"), MACHINE_X64_RBP, offsets[0] + 4, 0, 32, 32, 0);
        }
    }
    else
    {
            ...
            else
            {
                switch (instruction->opcode)
                {
                    ...
                    case MACHINE_X64_F80_RESULT_STORE:
                        machine_x64_emit_f80(&encoder, instruction, placement);
                        break;
                    ...
                    break; case MACHINE_X64_INLINE_ASSEMBLY:
                    {
                        ...
                        for (u32 operand_index = 0; operand_index < assembly->operand_count && !encoder.overflow; operand_index += 1)
                        {
                            ...
                            if (operand->flags & MACHINE_INLINE_ASSEMBLY_OPERAND_X87_TOP)
                            {
                                x87_top = side_index;
                            }
                            else if (operand->flags & MACHINE_INLINE_ASSEMBLY_OPERAND_X87_BELOW)
                            {
                                x87_below = side_index;
                            }
                            else if ((operand->flags & (MACHINE_INLINE_ASSEMBLY_OPERAND_INPUT | MACHINE_INLINE_ASSEMBLY_OPERAND_VECTOR)) ==
                                     (MACHINE_INLINE_ASSEMBLY_OPERAND_INPUT | MACHINE_INLINE_ASSEMBLY_OPERAND_VECTOR))
                            {
                                u32 frame_offset = placement->stack_slot_offsets[operand->stack_slot];
                                u32 xmm = operand->physical_register - MACHINE_X64_ZMM0;
                            ...
                            {
                                machine_x64_emit_exact_frame_chunk(&encoder, true, operand->physical_register,
                                                                   placement->stack_slot_offsets[operand->stack_slot], 8, 0);
                            }
                        }
                        u32 x87_depth = 0;
                        ...
                        for (u32 relocation_index = 0; relocation_index < assembly->relocation_count && !encoder.overflow; relocation_index += 1)
                        {
                    ...
                    case MACHINE_X64_STORE_YMM_FRAME256:
                    {
                        ...
                        s64 displacement = -(s64)(s32)placement->stack_slot_offsets[slot] + instruction->payload;
                        bool ymm = instruction->opcode == MACHINE_X64_LOAD_YMM_FRAME256 || instruction->opcode == MACHINE_X64_STORE_YMM_FRAME256;
                        bool store = instruction->opcode == MACHINE_X64_STORE_XMM_FRAME128 || instruction->opcode == MACHINE_X64_STORE_YMM_FRAME256;
                        u16 width = ymm ? 256 : 128;
                        BusterX86MetadataPhysicalOperand memory = machine_x64_exact_memory_operand(MACHINE_X64_RBP, width, displacement, displacement != 0);
                        BusterX86MetadataPhysicalOperand vector = ymm ? machine_x64_exact_ymm_operand(operand_registers[0] - MACHINE_X64_ZMM0)
                            : machine_x64_exact_xmm_operand(operand_registers[0] - MACHINE_X64_ZMM0, width);
                        BusterX86MetadataPhysicalOperand operands[] = {store ? memory : vector, store ? vector : memory};
                        BusterX86MetadataFeatureInput features = ymm
                            ? (BusterX86MetadataFeatureInput){.names = machine_x64_avx_features, .count = BUSTER_ARRAY_LENGTH(machine_x64_avx_features)}
                            : (BusterX86MetadataFeatureInput){0};
                        (void)machine_x64_emit_metadata_instruction(&encoder, ymm ? S8("VMOVDQU") : S8("MOVDQU"), operands, 2,
                            features, (BusterX86MetadataPhysicalAttributes){0}, 0);
                    }
                    ...
                    break; case MACHINE_X64_WIN_VA_SAVE:
                    {
                        ...
                        {
                            s64 displacement = 16 + (s64)placement->incoming_base + (s64)index * 8;
                            (void)machine_x64_emit_metadata_memory_register(&encoder, S8("MOV"), MACHINE_X64_RBP, displacement,
                                                                            operand_registers[index], 64, 64, 0);
                        }
                    }
                    break; case MACHINE_X64_LEA_INCOMING:
                    {
                        s64 displacement = 16 + (s64)placement->incoming_base + (s64)instruction->payload;
                        (void)machine_x64_emit_metadata_register_memory(&encoder, S8("LEA"), operand_registers[0], MACHINE_X64_RBP,
                                                                        displacement, 64, 64, 0);
                    }
                    ...
                    break; case MACHINE_X64_ATOMIC_STORE16:
                    {
                        u32 desired_slot = machine_ref_payload(instruction->operands[0]);
                        u32 address = operand_registers[3];
                        u32 desired_offset = placement->stack_slot_offsets[desired_slot];
                        u32 stored_size = instruction->payload;
                        // CMPXCHG16B takes the desired image in RBX:RCX and
                        // the expected image in RAX:RDX.  The desired pair is
                        // loaded once and remains unchanged across retries;
                        // only RAX:RDX are replaced by a failed comparison.
                        (void)machine_x64_emit_exact_frame_chunk(&encoder, true, MACHINE_X64_RBX, desired_offset, 8, 0);
                        (void)machine_x64_emit_exact_frame_chunk(&encoder, true, MACHINE_X64_RCX, desired_offset - 8, 8, 0);
                        if (stored_size > 8 && stored_size < 16)
                        {
                            u32 shift = 64u - (stored_size - 8u) * 8u;
                            (void)machine_x64_emit_metadata_register_immediate(&encoder, S8("SHL"), MACHINE_X64_RCX, shift, 64, 8, 0);
                            (void)machine_x64_emit_metadata_register_immediate(&encoder, S8("SHR"), MACHINE_X64_RCX, shift, 64, 8, 0);
                        }
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RAX, MACHINE_X64_RAX, 32, 0);
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RDX, MACHINE_X64_RDX, 32, 0);
                        u32 retry_offset = encoder.count;
                        (void)machine_x64_emit_metadata_atomic_memory(&encoder, S8("CMPXCHG16B"), address, 128, 0);
                        u32 retry_branch = encoder.count;
                        (void)machine_x64_emit_metadata_relative(&encoder, S8("JNE"), 0, 32, 0);
                    ...
                    break; case MACHINE_X64_ATOMIC_LOAD16:
                    {
                        u32 result_slot = machine_ref_payload(instruction->operands[0]);
                        u32 address = operand_registers[3];
                        u32 result_offset = placement->stack_slot_offsets[result_slot];
                        // The desired pair mirrors the current expected pair
                        // on every attempt.  CMPXCHG16B therefore leaves the
                        // observed image in RAX:RDX after either the initial
                        // zero comparison succeeds or a nonzero comparison
                        // fails and the retry imports the actual memory pair.
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RAX, MACHINE_X64_RAX, 32, 0);
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RDX, MACHINE_X64_RDX, 32, 0);
                        u32 retry_offset = encoder.count;
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("MOV"), MACHINE_X64_RBX, MACHINE_X64_RAX, 64, 0);
                    ...
                    break; case MACHINE_X64_ATOMIC_RMW16:
                    {
                        u32 result_slot = machine_ref_payload(instruction->operands[0]);
                        u32 value_slot = machine_ref_payload(instruction->operands[1]);
                        u32 address = operand_registers[3];
                        u32 result_offset = placement->stack_slot_offsets[result_slot];
                        u32 value_offset = placement->stack_slot_offsets[value_slot];
                        u32 atomic_operation = instruction->payload >> 8;
                        String8 low_operation = atomic_operation == IR_ATOMIC_ADD           ? S8("ADD")
                            ...
                            : atomic_operation == IR_ATOMIC_EXCHANGE    ? S8("MOV")
                                                                         : (String8){0};
                        String8 high_operation = atomic_operation == IR_ATOMIC_ADD        ? S8("ADC")
                            : atomic_operation == IR_ATOMIC_SUBTRACT ? S8("SBB")
                            : low_operation;
                        // R8:R9 retain the requested value across retries;
                        // RBX:RCX are rebuilt from the current expected pair
                        // before applying the operation on every attempt.
                        (void)machine_x64_emit_exact_frame_chunk(&encoder, true, MACHINE_X64_R8, value_offset, 8, 0);
                        (void)machine_x64_emit_exact_frame_chunk(&encoder, true, MACHINE_X64_R9, value_offset - 8, 8, 0);
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RAX, MACHINE_X64_RAX, 32, 0);
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("XOR"), MACHINE_X64_RDX, MACHINE_X64_RDX, 32, 0);
                        u32 retry_offset = encoder.count;
                        (void)machine_x64_emit_metadata_registers(&encoder, S8("MOV"), MACHINE_X64_RBX, MACHINE_X64_RAX, 64, 0);
