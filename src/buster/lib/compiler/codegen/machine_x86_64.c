...
struct MachineX64ValueShape
{
    ...
    // The regular scalar/aggregate part arrays remain two entries: vector
    // offsets are part_index * vector_part_bytes and need no extra storage.
...
BUSTER_CT_CHECK(sizeof(MachineX64ValueShape) == 40);
...
struct MachineX64Selector
{
    ...
    u32 last_native_vector_result;
    u32 last_native_vector_size;
    bool dynamic_stack;
...
{
    u64 bytes = shape->indirect ? (u64)shape->part_count * 8u : shape->byte_size;
                                                           ...
                                                           u32 result_register, u32 result_slot, bool aggregate)
{
    ...
    if (type && type->layout.resolved && type->layout.size && type->layout.size <= UINT32_MAX)
    {
        ...
        bool vector = selector->vector_registers_supported && machine_x64_type_is_vector_register(type);
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_va_start(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    if (result_slot != UINT32_MAX && windows && list_type && list_type->kind == IR_TYPE_VA_LIST && list_type->layout.size == 8)
    {
        ...
        {
            MachineX64ValueShape* shape = selector->parameter_shapes + argument;
            named_slots += shape->indirect && shape->vector_part_bytes ? shape->part_count : 1u;
        }
        u32 cursor = machine_x64_synthesize_register(selector);
        machine_x64_select_row(selector, (MachineInstruction){
                                                          ...
                                                          u32 right_register, u32 result_register)
{
        ...
        element->kind == IR_TYPE_INTEGER)
    {
        ...
        if (vector_opcode)
        {
            bool forwarded_left = selector->last_native_vector_result == instruction->operands[0].value &&
                                  selector->last_native_vector_size == operand_type->layout.size;
            bool forwarded_right = selector->last_native_vector_result == instruction->operands[1].value &&
                                   selector->last_native_vector_size == operand_type->layout.size &&
                                   (instruction->binary_operation == IR_BINARY_VECTOR_INTEGER_ADD ||
                                    instruction->binary_operation == IR_BINARY_VECTOR_INTEGER_BITWISE_AND ||
                                    instruction->binary_operation == IR_BINARY_VECTOR_INTEGER_BITWISE_OR ||
                                    instruction->binary_operation == IR_BINARY_VECTOR_INTEGER_BITWISE_XOR);
            u32 row = machine_x64_select_row(selector, (MachineInstruction){
                                                           .operands = {machine_ref_make(MACHINE_REF_VIRTUAL_REGISTER, result_register),
            ...
            machine_x64_define(selector, result_register, row);
            selector->native_vector_operation_count += 1;
            selector->forwarded_wide_vector_load_count += forwarded_left || forwarded_right;
            selector->last_native_vector_result = instruction->result.value;
            selector->last_native_vector_size = (u32)operand_type->layout.size;
            selected = true;
        }
    }
    return selected;
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_vector_lanes(MachineX64Selector* selector, IrInstruction* instruction, IrType* vector)
{
    ...
    if (selected && vector->layout.size > 16)
    {
        selector->split_vector_operation_count += 1;
    }
    return selected;
}
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_binary(MachineX64Selector* selector, IrInstruction* instruction, u32 result_register)
{
    ...
    else if (machine_x64_binary_is_i128(selector, instruction))
    {
        ...
        // register path below cannot see them at all. A refusal here is a
        // structured native code-generation failure.
...
BUSTER_GLOBAL_LOCAL bool machine_x64_select_array(MachineX64Selector* selector, IrInstruction* instruction)
{
    ...
    u32 slot = selector->value_stack_slots[instruction->result.value];
    u32 result_register = selector->value_virtual_registers[instruction->result.value];
    bool vector = selector->vector_registers_supported && machine_x64_type_is_vector_register(type) && result_register != UINT32_MAX;
    if (vector)
    {
                                                              ...
                                                              bool preserve_debug_values, MachineSelectionModule* module)
{
    ...
    MachineX64Selector selector = {
        ...
        .last_native_vector_result = IR_ID_UNDERLYING_INVALID,
        .failed_opcode = IR_OPCODE_COUNT,
    };
    ...
    for (u32 layout_index = 0; layout_index < function->block_count && selector.supported; layout_index += 1)
    {
        ...
        for (u32 row_offset = 0; row_offset < block_row_count && selector.supported; row_offset += 1)
        {
            ...
            if (selector.last_native_vector_result != IR_ID_UNDERLYING_INVALID)
            {
                bool uses_last_native = false;
                for (u32 operand_index = 0; operand_index < instruction->operand_count; operand_index += 1)
                {
                    uses_last_native |= instruction->operands[operand_index].value == selector.last_native_vector_result;
                }
                IrType* result_type = ir_type_from_id(&program->types, instruction->canonical_type);
                bool preserves_native = instruction->opcode == IR_OPCODE_FIELD ||
                    (instruction->opcode == IR_OPCODE_LOAD && result_type && result_type->kind == IR_TYPE_VECTOR &&
                     result_type->layout.resolved && result_type->layout.size <= 64);
                if (!uses_last_native && !preserves_native)
                {
                    selector.last_native_vector_result = IR_ID_UNDERLYING_INVALID;
                    selector.last_native_vector_size = 0;
                }
            }
    ...
    result.native_vector_operation_count = selector.native_vector_operation_count;
    result.split_vector_operation_count = selector.split_vector_operation_count;
    result.forwarded_wide_vector_load_count = selector.forwarded_wide_vector_load_count;
    return result;
}
...
MachineEncodeResult machine_x64_test_emit_movabs(u8* bytes, u32 capacity, u32 start, u32 reg, u64 value, bool reference)
{
    ...
    result.vzeroupper_count = encoder.vzeroupper_count;
...
BUSTER_GLOBAL_LOCAL void machine_x64_emit_f80(MachineX64Encoder* encoder, MachineInstruction* instruction, MachineStackPlacement* placement)
{
    ...
    else
    {
        ...
        offsets[0] += (s32)instruction->payload;
        machine_x64_emit_x87_memory(encoder, load ? S8("FLD") : S8("FSTP"), offsets[0], 80);
        if (!load)
        {
            machine_x64_emit_f80_padding(encoder, offsets[0]);
        }
...
MachineEncodeResult machine_encode_x86_64(Arena* arena, MachineFunction* function, MachineStackPlacement* placement)
{
    ...
    {
        // Reuse the shared bounded Win64 probe. Its R10/R11
