        ...
        for (u32 debug_index = 0; result && debug_index < ir_function->debug_local_count; debug_index += 1)
        {
            ...
            if (!local_capacity)
            {
                for (u32 block_index = 0; block_index < ir_function->block_count; block_index += 1)
                {
                    ...
                    local_capacity += value.value != IR_ID_UNDERLYING_INVALID && published->instruction_count;
                }
            }
            local_capacity = local_capacity ? local_capacity : 1;
            result = local_capacity <= UINT32_MAX - capacity64;
            capacity64 += result ? local_capacity : 0;
        }
        u32 capacity = result ? (u32)capacity64 : 0;
        MachineDebugValue* values = result ? arena_allocate(arena, MachineDebugValue, capacity ? capacity : 1) : 0;
        u32 value_count = 0;
        parameter_ordinal = 0;
        for (u32 debug_index = 0; result && debug_index < ir_function->debug_local_count; debug_index += 1)
        {
                ...
                if (instruction->opcode == MACHINE_X64_VA_ARG)
                {
                    ...
                    for (u32 part = 0; part < metadata->part_count && valid; part += 1)
                    {
                        valid = metadata->parts[part].size <= 8 ||
                                (metadata->parts[part].size == 16 && metadata->parts[part].is_float && metadata->result_is_frame);
                    }
                }
...
BUSTER_GLOBAL_LOCAL MachineStackPlacement machine_stack_placement_build_core(Arena* arena, MachineFunction* function)
{
    ...
    if (target)
    {
        ...
        // space and return address overwrite them. Refuse the placement rather
        // than publishing a miscompiled machine function.
