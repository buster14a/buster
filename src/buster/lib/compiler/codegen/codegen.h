...
typedef enum CodegenModuleRelocationKind
{
    ...
    CODEGEN_MODULE_RELOCATION_AARCH64_BRANCH26,
...
struct CodegenStatistics
{
    ...
    u64 code_bytes;
    // MIR selector/encoder decisions: native target-width vector rows,
    // wider vectors expanded into lanes, chained native results consumed
    // without a frame reload, and actual transition-hygiene instructions.
