...
// and AArch64. Every supported native mode routes through canonical IR,
// machine selection, placement, and metadata-backed emission. The historical
// NONE spelling is a MIR_STACK compatibility alias; a machine failure is a
// structured failed result, never a direct-native fallback.
//
...
// relocations), then per function runs machine selection, placement, and
// encoding. codegen_generate_canonical_module wraps it in a retry loop that
...
//                                                call layout
//   codegen_x64_emit_windows_stack_allocate and  frame setup and stack probes
//   codegen_a64_windows_large_stack_adjust      (shared machine helpers)
...
//                                                into the module
//   AArch64 constant/frame-memory helpers         shared encoding and ABI
...
// The vector registers an 'x' operand may be allocated. The MIR machine path
// keeps every value in its frame slot between instructions, so none of these
// is live across the template; the pool is capped at eight because the
// metadata bridge's vector operand is encoded from a register index in that
// range.
...
// The MIR inline-assembly path only gives ordinary scalar GPR operands. Keep
// the spelling table here, rather than teaching the
                                                                           ...
                                                                           MachineSelectionModule* machine_module, BootstrapTrace* bootstrap_trace)
{
    ...
    for (u32 function_index = 0; function_index < module->function_count; function_index += 1)
    {
        ...
        // Snapshot the committed prefix before this function contributes any
        // bytes, entries, line rows, descriptors, relocations, debug rows, or
        ...
        u32 function_line_start = result.line_entry_count;
        u32 function_debug_start = result.debug_location_count;
        u64 function_buffer_start = buffer.count;
        ...
        if (buffer.error != CODEGEN_ERROR_NONE)
        {
            ...
            fallback_reason = CODEGEN_FALLBACK_OUTPUT_CAPACITY;
            goto machine_transaction_failure;
        }
        result.entries[result.entry_count++] = (CodegenModuleEntry){
        ...
        descriptor = result.functions + result.function_count;
        ...
        *descriptor = (CodegenFunctionDescriptor){
        ...
        u32 machine_native_vector_operation_count = 0;
        u32 machine_split_vector_operation_count = 0;
        u32 machine_forwarded_wide_vector_load_count = 0;
        u32 machine_vzeroupper_count = 0;
        u32 machine_stack_frame_size = 0;
        u32* machine_block_offsets = 0;
        bool machine_function_emitted = false;
        // Selection is attempted before any machine side table is published.
        // A failed function rolls back its staged bytes and metadata and is
        // returned as a structured diagnostic; there is no direct emitter
        // recovery edge.
        goto machine_attempt;

    machine_attempt:
        // The descriptor is a shell until this path knows its unwind shape.
        // Every public allocator spelling reaches machine selection, stack
        // placement, and encoding. ELF/Mach-O frames retain their established
        // shape. PE AArch64 uses a compact chain/save area and bounded probe,
        // with every prologue instruction represented in its unwind
        // description.
        if (target.cpu_arch == CPU_ARCH_X86_64 || target.cpu_arch == CPU_ARCH_AARCH64)
        {
            ...
            bool selection_verification_failed = false;
            ...
            if (bootstrap_trace)
            {
                ...
                if (bootstrap_trace->invalid_mir)
                {
                    ...
                    fallback_reason = CODEGEN_FALLBACK_VERIFICATION;
                    selection_verification_failed = true;
                    selected.supported = false;
                }
            }
            machine_simd_operation_count = selected.simd_operation_count;
            machine_native_vector_operation_count = selected.native_vector_operation_count;
            machine_split_vector_operation_count = selected.split_vector_operation_count;
            machine_forwarded_wide_vector_load_count = selected.forwarded_wide_vector_load_count;
            if (!selected.supported && !selection_verification_failed)
            {
            ...
            if (selected.supported && verify_error == MACHINE_VERIFY_NONE)
            {
                ...
                if (!placement.valid)
                {
                    if (fallback_reason != CODEGEN_FALLBACK_VERIFICATION)
                    {
                ...
                if (placement.valid)
                {
                    ...
                    // the transaction below returns the structured failure.
                    ...
                    machine_vzeroupper_count = encoded.vzeroupper_count;
                    ...
                    // an abandoned function never enters the returned module.
                    ...
                    if (encoded_fits && target.cpu_arch == CPU_ARCH_AARCH64)
                    {
                        ...
                        if (machine_unwind_valid)
                        {
                            ...
                            if (machine_inline_relocations_valid)
                            {
                                ...
                                {
                                    // Label fixups run after the selector
                                    // scratch scope closes; retain this small
                                    // map in the attempt arena rather than
                                    // reading rewound scratch storage.
                                    machine_block_offsets = arena_allocate(arena, u32, function->block_count);
            ...
            if (machine_function_emitted)
            {
                ...
                result.statistics.native_vector_operation_count += machine_native_vector_operation_count;
                result.statistics.split_vector_operation_count += machine_split_vector_operation_count;
                result.statistics.forwarded_wide_vector_load_count += machine_forwarded_wide_vector_load_count;
                result.statistics.vzeroupper_count += machine_vzeroupper_count;
            }
            scratch_end(machine_scratch);
        }
        if (machine_function_emitted)
        {
            // Global-initializer label relocations precede this function's
            // relocation checkpoint. Validate every matching fixup first so
            // a later bad block or addend cannot leave an earlier relocation
            // partially rewritten on the failed result.
    ...
    machine_transaction_failure:
        // A machine function is a transaction. Selection, encoding, unwind,
        ...
        buffer.count = function_buffer_start;
        result.entry_count = function_entry_start;
        result.function_count = function_descriptor_start;
        result.statistics = function_statistics_start;
        BUSTER_CHECK(result.relocation_count == function_relocation_start && result.line_entry_count == function_line_start &&
                     result.debug_location_count == function_debug_start && result.entry_count == function_entry_start &&
                     result.function_count == function_descriptor_start && buffer.count == function_buffer_start);
        if (descriptor)
        {
            descriptor->unwind_action_count = 0;
            descriptor->epilog_count = 0;
            descriptor->code_size = 0;
            descriptor->prolog_size = 0;
        }
        if (buffer.error != CODEGEN_ERROR_NONE)
        {
            result.failure_reason = codegen_fallback_reason_string(fallback_reason);
            result.error = transaction_error != CODEGEN_ERROR_NONE ? transaction_error : buffer.error;
            return result;
        }
        ...
        if (fallback_opcode < IR_OPCODE_COUNT)
        {
            for (u32 instruction_index = 0; instruction_index < function->instruction_count; instruction_index += 1)
            {
                if (function->instructions[instruction_index].opcode == fallback_opcode)
                {
                    ...
                    if (fallback_opcode == IR_OPCODE_INLINE_ASSEMBLY)
                    {
                        IrInstructionExtra extra = ir_instruction_extra(function, result.failed_instruction);
                        if (string_first_sequence(extra.literal, S8("%l")) < extra.literal.length)
                        {
                            result.failure_reason = S8("inline assembly label references (%l) are unsupported in this template form");
                        }
                    }
                    break;
                }
        ...
        else if (function->instruction_count)
        {
            result.failed_instruction = (IrInstructionId){.value = 0};
            result.failed_opcode = function->instructions[0].opcode;
        }
        result.error = transaction_error != CODEGEN_ERROR_NONE ? transaction_error
                       : fallback_reason == CODEGEN_FALLBACK_SIGNATURE ? CODEGEN_ERROR_UNSUPPORTED_ABI
                                                                                  ...
                                                                                  bool* code_buffer_exhausted)
{
    ...
    MachineSelectionModule* machine_module = target.cpu_arch == CPU_ARCH_X86_64 ? machine_select_module_prepare(arena, program, target) : 0;
    return codegen_generate_canonical_module_attempt(arena, program, slot_costs, module, target, options, capacity_scale,
                                                      code_buffer_exhausted, 0, machine_module, 0);
}
#endif
...
CodegenModule codegen_generate_canonical_module_with_trace(Arena* arena, IrProgram* program, IrModule* module, Target target, CodegenModuleOptions options, BootstrapTrace* bootstrap_trace)
{
    ...
    // Preserve the historical spelling as a compatibility alias while making
    // the selected pipeline explicit: native production code is always
    // selected, placed, encoded, and described as MIR.
    ...
    // Module-level assembly still uses the shared x86 metadata encoder; it
    // allocates its recipe cache lazily inside the attempt scope. Native
    // functions never need the retired direct-emitter caches.
