...
{
    if (!text.length || string_first_sequence(text, S8("<register_page")) == BUSTER_STRING_NO_MATCH ||
...
BUSTER_GLOBAL_LOCAL ProcessResult arm_a64_sysreg_run_generator(Arena* arena, String8 source_directory, String8 output_directory, bool check_only)
{
    ...
    if (!check_only)
    {
        ...
        if (argument_count >= BUSTER_ARRAY_LENGTH(arguments)) return PROCESS_RESULT_FAILED;
...
{
    if (!arena || !options.source_directory.length || !options.output_directory.length) return PROCESS_RESULT_FAILED;
    TemporalArena scratch_scope = scratch_begin(&arena, 1);
    Arena* scratch = scratch_scope.arena;
    ...
    if (copied)
    {
        fprintf(stdout, "Arm A-profile SysReg checked deterministic artifacts: xml=%u relevant=%u accepted=%u output=%.*s\n", xml_count, relevant_count,
                ARM_A64_EXPECTED_ACCEPTED_MECHANISM_COUNT,
                (int)options.output_directory.length, options.output_directory.pointer);
    }
    scratch_end(scratch_scope);
    return copied ? PROCESS_RESULT_SUCCESS : PROCESS_RESULT_FAILED;
}
