...
{
    return opcode >= ASSEMBLY_OPCODE_X86_FLD && opcode <= ASSEMBLY_OPCODE_X86_FISTP;
}
...
BUSTER_GLOBAL_LOCAL bool assembly_x86_instruction_size(AssemblyInstruction* instruction)
{
    ...
    if (assembly_x86_opcode_is_legacy_packed(opcode))
    {
        ...
        instruction->size = (packed_reg.class == ASSEMBLY_REGISTER_XMM ? 1 : 0) + (rex ? 1 : 0) + 2 + address_size;
        return true;
    }
    ...
    if (opcode == ASSEMBLY_OPCODE_X86_SETCC)
    {
        ...
        if (first->kind == ASSEMBLY_OPERAND_MEMORY && (!first->memory.width || first->memory.width == 8))
        {
            ...
            instruction->width = 8;
            instruction->size = (assembly_x86_memory_rex_needed(8, (AssemblyRegister){0}, first->memory) ? 1 : 0) + 2 + address_size;
            return true;
        }
        return false;
    }
    if (opcode == ASSEMBLY_OPCODE_X86_CMOVCC)
    {
        ...
        instruction->size = (instruction->width == 16 ? 1 : 0) + (rex ? 1 : 0) + 2 + address_size;
        return true;
    }
    if (assembly_x86_opcode_is_sse2(opcode))
    {
        ...
        instruction->size = (mandatory_prefix ? 1 : 0) + (rex ? 1 : 0) + 2 + address_size;
        return true;
    }
