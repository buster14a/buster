...
BUSTER_GLOBAL_LOCAL String8 buster_x86_metadata_pool_span(u32 offset, u32 length)
{
    ...
    {
        result = (String8){.pointer = buster_x86_metadata_pool_bytes + offset, .length = length};
    }
                                                                          ...
                                                                          BusterX86MetadataPatternSemantics pattern)
{
    ...
    {
        result = BUSTER_X86_METADATA_BLOCKER_ADDRESSING_FIELDS;
    }
    else if (pattern.has_element_size_control && buster_x86_metadata_emit_element_size_bits(form) != pattern.element_size_bits)
    {
        result = BUSTER_X86_METADATA_BLOCKER_ADDRESSING_FIELDS;
    }
    else if (pattern.has_implicit_one && (pattern.immediate_count || !buster_x86_metadata_emit_form_has_implicit_immediate(form)))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PATTERN_SEMANTICS;
    }
    else if (pattern.has_apx_control && !(form.apx_flags & BUSTER_X86_METADATA_APX))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PREFIX_FIELDS;
    }
    else if (pattern.has_nd && !(form.apx_flags & BUSTER_X86_METADATA_APX_ND))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PREFIX_FIELDS;
    }
    else if (pattern.has_nf && pattern.nf_value && !(form.apx_flags & BUSTER_X86_METADATA_APX_NF))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PREFIX_FIELDS;
    }
    else if (pattern.has_scc && (!(form.apx_flags & BUSTER_X86_METADATA_APX_SCC) || pattern.scc_value >= 16))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PREFIX_FIELDS;
    }
    else if (pattern.no_scc && (form.apx_flags & BUSTER_X86_METADATA_APX_SCC))
    {
        result = BUSTER_X86_METADATA_BLOCKER_PREFIX_FIELDS;
    }
    else if (pattern.has_tile_control && (form.encoder_family != BUSTER_X86_METADATA_ENCODER_AMX || !form.amx_flags))
    {
        result = BUSTER_X86_METADATA_BLOCKER_ADDRESSING_FIELDS;
    }
    else if (pattern.has_amx_control && form.encoder_family == BUSTER_X86_METADATA_ENCODER_AMX && !form.amx_flags)
    {
        result = BUSTER_X86_METADATA_BLOCKER_ADDRESSING_FIELDS;
    }
    else
    {
        result = BUSTER_X86_METADATA_BLOCKER_NONE;
    }
    ...
    BusterX86MetadataPatternSemantics pattern, u32 actual_index)
{
    ...
    else if (physical.has_unsigned_value)
    {
        result = physical.unsigned_value == 1;
    }
    else
    {
        result = physical.has_value && physical.value == 1;
    }
...
BUSTER_GLOBAL_LOCAL bool buster_x86_metadata_emit_atom_contains(BusterX86MetadataString atom, String8 needle)
{
    if (needle.length && atom.length >= needle.length)
    {
...
BUSTER_GLOBAL_LOCAL u8 buster_x86_metadata_emit_effective_field_source(BusterX86MetadataOperand metadata)
{
    ...
    else if (buster_x86_metadata_emit_atom_contains(metadata.atom, S8("_B")))
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_RM;
    }
    else if (buster_x86_metadata_emit_atom_contains(metadata.atom, S8("_N")))
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_VVVV;
    }
    else if (metadata.physical_class == BUSTER_X86_METADATA_PHYSICAL_CLASS_MASK && buster_x86_metadata_emit_atom_contains(metadata.atom, S8("_R")))
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_REG;
    }
    else if (metadata.physical_class == BUSTER_X86_METADATA_PHYSICAL_CLASS_MASK)
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_MASK;
    }
    else if (metadata.kind == BUSTER_X86_METADATA_OPERAND_IMMEDIATE)
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_IMMEDIATE;
    }
    else if (metadata.kind == BUSTER_X86_METADATA_OPERAND_RELATIVE)
    {
        result = BUSTER_X86_METADATA_FIELD_SOURCE_RELATIVE;
    }
    else
    {
        result = metadata.field_source;
    }
    ...
    BusterX86MetadataForm form, BusterX86MetadataPatternSemantics pattern)
{
    ...
    {
        result = buster_x86_metadata_emit_canonical_segment_override(form, pattern);
    }
...
BUSTER_GLOBAL_LOCAL bool buster_x86_metadata_emit_string_has(BusterX86MetadataString string, String8 needle)
{
    if (needle.length && string.length >= needle.length)
    {
...
BUSTER_GLOBAL_LOCAL u16 buster_x86_metadata_emit_element_size_bits(BusterX86MetadataForm form)
{
    ...
    if (!buster_x86_metadata_emit_string_has(form.element_size, S8("8_BITS")))
    {
...
BUSTER_GLOBAL_LOCAL u16 buster_x86_metadata_emit_tuple_memory_width(BusterX86MetadataPatternSemantics pattern)
{
    ...
    u16 result;
    if (!divisor || pattern.vector_length % divisor)
    {
        result = 0;
    }
    else
    {
        result = (u16)(pattern.vector_length / divisor);
    }
...
#define pattern (*pattern_view)
    ...
    for (u32 index = 0; index < binding_count; index += 1)
    {
        ...
        // Folded memory width is a scalar legacy/REX/REX2 width authority. VEX,
...
#undef BUSTER_X86_METADATA_FAST_PUSH_BYTE
    ...
    result->required_relocation_count = 0;
    result->status = buster_x86_metadata_instruction_length_status(byte_count);
    if (result->status == BUSTER_X86_METADATA_ENCODE_SUCCESS)
    {
        if (output_capacity < byte_count)
        {
            result->status = BUSTER_X86_METADATA_ENCODE_OUTPUT_CAPACITY;
                                                        ...
                                                        BusterX86MetadataPhysicalQuery query)
{
    ...
    if (query.attributes.sae || query.attributes.rounding_mode != BUSTER_X86_METADATA_ROUNDING_NONE)
    {
        ...
        if (buster_x86_metadata_form_standalone_sae_capable_impl(form) && memory_count == 0 && query.operand_count == 2 && vector_register_count != 0)
        {
            for (u32 operand_index = 0; operand_index < query.operand_count; operand_index += 1)
            {
                BusterX86MetadataPhysicalOperand operand = query.operands[operand_index];
                                                                          ...
                                                                          BusterX86MetadataPhysicalQuery query)
{
    ...
    {
        result = pattern.opcode[0] == 0x0f && (pattern.opcode[1] == 0x6e || pattern.opcode[1] == 0x7e);
    }
...
BusterX86MetadataSelectResult buster_x86_metadata_select_form(BusterX86MetadataPhysicalQuery query)
{
    ...
    if (buster_x86_metadata_emit_physical_query_valid(query))
    {
...
BUSTER_GLOBAL_LOCAL bool buster_x86_metadata_emit_form_mnemonic_matches(String8 mnemonic, u32 form_id)
{
    ...
    BusterX86MetadataForm form = {0};
    bool result;
    if (buster_x86_metadata_form(form_id, &form) && buster_x86_metadata_string_input_equal(form.iclass.offset, mnemonic))
    {
        result = true;
    }
    else
    ...
    BusterX86MetadataPhysicalQuery query, BusterX86MetadataForm form)
{
    ...
    if (buster_x86_metadata_form_coverage_allowed(generated, policy_query) && buster_x86_metadata_form_execution_mode_matches(generated, policy_query))
    {
...
{
    // A REX site may still be the MOV-r64 load, so the relaxable vocabulary
    // contains the closed one rather than replacing it.
    ...
    if (missing)
    {
        ...
        u32 first = (missing & BUSTER_X86_GOT_PREPARE_LOAD) ? 0 : 1;
        u32 last = (missing & BUSTER_X86_GOT_PREPARE_RELAXABLE) ? (u32)BUSTER_ARRAY_LENGTH(buster_x86_metadata_got_classes) : 1;
        for (u32 class_index = first; valid && class_index < last; class_index += 1)
        {
            ...
            u32 instances = entry_class.shape == BUSTER_X86_GOT_SHAPE_SINGLE ? 1 : BUSTER_X86_GOT_REGISTERS;
            u16 width = entry_class.operand_width;
            u32 source_form = UINT32_MAX;
            u32 direct_form = UINT32_MAX;
            // Highest register first: that instance selects both forms for
            // the whole class, and the rows it leaves behind are the same set
            ...
            for (u32 instance = instances; valid && instance > 0; instance -= 1)
            {
                ...
                // The source is the GOT load itself: a rip-relative field
                // whose -4 puts the next instruction's address in the sum.
                ...
                // Same length, same field offset, and the field kind the
                // class promised the linker it would patch.
                ...
                if (valid)
                {
                    ...
                    // REX.X and REX.B name no operand in a RIP-relative
                    // form. Only a row whose first byte is that prefix -- a
                    // row one byte wider than opcode and ModRM -- may ignore
                    // them; for any other row the first byte is the opcode.
                    bool rex = prefix == BUSTER_X86_GOT_PREFIX_MAX && (source_bytes[0] & 0xf0u) == 0x40u;
                    ...
                    valid = buster_x86_metadata_got_counts[slot] < BUSTER_X86_GOT_CAPACITY &&
                            ...
                            (class_index || !buster_x86_metadata_got_load_count || buster_x86_metadata_got_load_slot == slot);
                    if (valid)
                    {
            ...
            // The closed family stands on its own, whichever tiers this call
            // asked for: a later class the module cannot derive must not take
            // the conversion that already worked down with it.
            buster_x86_metadata_got_valid |= valid && !class_index ? BUSTER_X86_GOT_PREPARE_LOAD : 0;
        }
        // Cache failure too. A partial vocabulary is never published: a shape
        // this module could not derive must fail the link by name instead of
        // being rewritten from bytes nothing checked.
                                                                 ...
                                                                 s64 addend)
{
    ...
    u8 requested = site == BUSTER_X86_METADATA_GOT_SITE_GOTPCREL ? BUSTER_X86_GOT_PREPARE_LOAD : BUSTER_X86_GOT_PREPARE_RELAXABLE;
    if (section && site < BUSTER_X86_METADATA_GOT_SITE_COUNT && field_offset <= section_size &&
        BUSTER_X86_GOT_FIELD_WIDTH <= section_size - field_offset && buster_x86_metadata_got_prepare(requested))
    {
        // The spelling fixes how much of this instruction precedes its field:
        // opcode and ModRM for GOTPCRELX, one REX prefix more for
        // REX_GOTPCRELX. Plain GOTPCREL reads the closed MOV-r64 rows, whose
        // own derivation says how wide they are.
        u32 prefix = site == BUSTER_X86_METADATA_GOT_SITE_GOTPCRELX       ? BUSTER_X86_GOT_PREFIX_MIN
                     : site == BUSTER_X86_METADATA_GOT_SITE_REX_GOTPCRELX ? BUSTER_X86_GOT_PREFIX_MAX
                                                                          : (u32)buster_x86_metadata_got_load_slot + BUSTER_X86_GOT_PREFIX_MIN;
        u32 slot = prefix - BUSTER_X86_GOT_PREFIX_MIN;
        u32 count = site == BUSTER_X86_METADATA_GOT_SITE_GOTPCREL ? buster_x86_metadata_got_load_count : buster_x86_metadata_got_counts[slot];
        if (field_offset >= prefix)
        {
            u8* sequence = section + field_offset - prefix;
            for (u32 index = 0; result == BUSTER_X86_METADATA_GOT_PATCH_NONE && index < count; index += 1)
            {
                BusterX86MetadataGotEntry const* entry = &buster_x86_metadata_got_entries[slot][index];
                bool matched = (u8)(sequence[0] & entry->mask) == entry->source[0] &&
                               memcmp(sequence + 1, entry->source + 1, prefix - 1) == 0;
                // An absolute replacement answers with the address itself, so
                // the -4 that aimed a rip-relative field at the slot means
                // nothing there. Refuse any other addend rather than fold one
                // into a value it was never about.
                if (matched && (entry->patch == BUSTER_X86_METADATA_GOT_PATCH_PC32 || addend == -(s64)BUSTER_X86_GOT_FIELD_WIDTH))
                {
                    // Nothing is written before the whole shape is decided,
                    // and the field's own four bytes stay untouched for the
                    // object-format relocation writer.
...
BUSTER_GLOBAL_LOCAL bool buster_x86_metadata_pool_string_has_token(u32 offset, String8 token)
{
    if (token.length)
    {
        ...
        {
            String8 span = buster_x86_metadata_pool_span(offset, length);
            if (span.length == length && span.length >= token.length)
            {
                u32 cursor = 0;
...
{
    u32 length = 0;
    if (needle.length && buster_x86_metadata_string_offset_terminated(offset, &length) && needle.length <= length)
    {
...
{
    if (buster_x86_metadata_string_offset_terminated(offset, 0))
    {
        for (u32 index = 0; index < input.count; index += 1)
        {
            String8 feature = input.names[index];
                                                                      ...
                                                                      BusterX86MetadataFeatureInput input)
{
    ...
    bool result;
    if (!buster_x86_metadata_string_offset_terminated(requirement, &requirement_length))
    {
        result = false;
    }
    else if (!requirement_length)
    {
        result = true;
    }
    else
    {
        result = buster_x86_metadata_canonical_feature_matches(requirement, input);
    }
                                                                         ...
                                                                         BusterX86MetadataOperandSignature signature)
{
    ...
    bool result;
    if (signature.has_visible && signature.visible != operand.visible)
    {
        result = false;
    }
    else
...
BUSTER_GLOBAL_LOCAL bool buster_x86_metadata_form_address_size_matches(BusterX86GeneratedForm form, u8 address_size)
{
    ...
    {
        result = (address_flags & BUSTER_X86_GENERATED_MODE_EA16) != 0;
    }
    else if (address_size == 32)
    {
        result = (address_flags & (BUSTER_X86_GENERATED_MODE_EA32 | BUSTER_X86_GENERATED_MODE_EANOT16)) != 0;
    }
    else
    {
        result = address_size == 64 && (address_flags & (BUSTER_X86_GENERATED_MODE_EA64 | BUSTER_X86_GENERATED_MODE_EANOT16)) != 0;
    }
...
{
    bool result;
    if (form.coverage_class == BUSTER_X86_GENERATED_COVERAGE_NORMALIZED)
    {
        result = true;
    }
    else if (form.coverage_class == BUSTER_X86_GENERATED_COVERAGE_PRIVILEGED && query.include_privileged)
    {
        result = true;
    }
    else if (form.coverage_class == BUSTER_X86_GENERATED_COVERAGE_NOT64 && query.include_not64)
    {
        result = true;
    }
    else
                                                           ...
                                                           u32 form_id_capacity)
{
    ...
    if ((!form_id_capacity || form_ids) && buster_x86_metadata_resolution_query_valid(query, 0))
    {
