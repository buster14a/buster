...
BUSTER_GLOBAL_LOCAL u8 buster_x86_metadata_emit_tuple_scale(BusterX86MetadataForm const* form,
                                                              BusterX86MetadataPatternSemantics const* pattern, bool broadcast)
{
    ...
    {
    // EVEX.b changes Full/Half tuple addressing to a single scalar element.
    // Compressing by the vector width here silently changes the address.
...
#define pattern (*pattern_view)
    ...
    if (uses_apx_egpr &&
        ...
        return BUSTER_X86_METADATA_ENCODE_FEATURE_MODE_PRIVILEGE;
    BusterX86MetadataAddressEncoding address = {0};
    if (has_memory && !moffs_form && !buster_x86_metadata_emit_address(memory, &form, &pattern, query.attributes.broadcast_elements != 0, force_disp32, &address))
        return BUSTER_X86_METADATA_ENCODE_ADDRESSING;
    ...
    BusterX86MetadataEmitResult* result, u8* output, u32 output_capacity, bool force_disp32)
{
    ...
    if (has_memory)
    {
        ...
        if (!address_ready)
            address_ready = buster_x86_metadata_emit_address(memory, form, plan->pattern, query.attributes.broadcast_elements != 0, force_disp32, &address);
    }
