...
{
    // `###` wins over `##`: it replaces the identity outright, so it is tested
    // first and the double form only applies when no triple form is present.
    String8 result = label;
    u64 triple_index = string_first_sequence(label, S8("###"));
    if (triple_index < label.length)
    {
        result = string_slice(label, triple_index + 3, label.length);
    }
    else
    {
        ...
        {
            result = string_slice(label, double_index + 2, label.length);
        }
    }
...
UI_Box* ui_separator(Axis2 axis)
{
    ...
    if (axis == AXIS2_X)
    {
        ...
        result = ui_widget_box(S8("separator_x"), S8(""), UI_BoxFlag_DrawSideBottom);
    }
    else
    {
        ui_set_next_pref_width(ui_pixels(1, 1.0f));
        ui_set_next_pref_height(ui_percentage(1.0f, 0.0f));
        result = ui_widget_box(S8("separator_y"), S8(""), UI_BoxFlag_DrawSideRight);
    }
...
{
    // The range check has to pass before the arithmetic below is meaningful, so
    // the capacity test is nested rather than joined onto the same condition.
    bool result = value && value->pointer && (replacement.length == 0 || replacement.pointer) && value->length <= capacity && first <= one_past_last &&
                  one_past_last <= value->length;
    if (result)
    {
        u64 removed_length = one_past_last - first;
        u64 retained_length = value->length - removed_length;
        result = replacement.length <= capacity && retained_length <= capacity - replacement.length;
        if (result)
        {
