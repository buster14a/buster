...
BUSTER_V_IMPL BuildTarget build_target_native = {
...
BUSTER_GLOBAL_LOCAL StringOs c_std_flags[] = {
    SOs("-std=gnu2x"),
};

BUSTER_GLOBAL_LOCAL StringOs cpp_std_flags[] = {
    SOs("-std=gnu++2c"),
};
...
ENUM(BuildFlag,
    BUILD_OPTION_FLAG_OPTIMIZE,
    BUILD_OPTION_FLAG_FUZZ,
    BUILD_OPTION_FLAG_HAS_DEBUG_INFORMATION,
    BUILD_OPTION_FLAG_UNITY_BUILD,
    BUILD_OPTION_FLAG_JUST_PREPROCESSOR,
    BUILD_OPTION_FLAG_SELF_HOSTED,
...
ENUM_T(BuildIntegerOptionFormat, u8,
    BUILD_OPTION_INTEGER_FORMAT_HEXADECIMAL,
    BUILD_OPTION_INTEGER_FORMAT_DECIMAL_POSITIVE,
    BUILD_OPTION_INTEGER_FORMAT_DECIMAL_NEGATIVE,
    BUILD_OPTION_INTEGER_FORMAT_OCTAL,
    BUILD_OPTION_INTEGER_FORMAT_BINARY);
...
{
    BuildIntegerOptionValue values[(u64)BuildIntegerOption::Count];
    BuildIntegerOptionSettings settings[(u64)BuildIntegerOption::Count];
    u8 reserved[7];
};

ENUM(BuildStringOption,
    BUILD_OPTION_STRING_XC_SDK_PATH,
    BUILD_OPTION_STRING_PRINT_FUNCTION_OPTIMIZATION);
...
{
    StringOs values[(u64)BuildStringOption::Count];
    BuildStringOptionSettings settings[(u64)BuildStringOption::Count];
    u8 reserved[6];
};
...
STRUCT(BuildProgramState)
{
    ...
    FLAG_ARRAY_U64(value_flags, BuildFlag);
    FLAG_ARRAY_U64(set_flags, BuildFlag);
...
BUSTER_V_IMPL ProgramState* program_state = &build_program_state.general_state;

BUSTER_GLOBAL_LOCAL bool build_flag_is_set(BuildFlag flag)
{
    return flag_get(build_program_state.set_flags, flag);
}

BUSTER_GLOBAL_LOCAL bool build_flag_get(BuildFlag flag)
{
    return flag_get(build_program_state.value_flags, flag);
}

BUSTER_GLOBAL_LOCAL void build_flag_set(BuildFlag flag, bool value)
{
    flag_set(build_program_state.value_flags, flag, value);
    flag_set(build_program_state.set_flags, flag, true);
}

BUSTER_GLOBAL_LOCAL u64 build_integer_option_get_unsigned(BuildIntegerOption option_id)
{
    let option_value = build_program_state.integer.values[(u64)option_id];
    return option_value.unsigned_value;
}

BUSTER_GLOBAL_LOCAL StringOs build_string_option_get(BuildStringOption option_id)
{
    let option_value = build_program_state.string.values[(u64)option_id];
    return option_value;
}
...
BUSTER_F_IMPL ProcessResult process_arguments()
{
...

    {
        ...
        if (command.pointer)
        {
            ...
            else
            {
                StringOs possible_commands[] = {
                    [(u64)BuildCommand::BUILD_COMMAND_BUILD] = SOs("build"),
                    [(u64)BuildCommand::BUILD_COMMAND_TEST] = SOs("test"),
                    [(u64)BuildCommand::BUILD_COMMAND_TEST_ALL] = SOs("test_all"),
                    [(u64)BuildCommand::BUILD_COMMAND_DEBUG] = SOs("debug"),
                };
                static_assert(BUSTER_ARRAY_LENGTH(possible_commands) == (u64)BuildCommand::Count);
        ...
        while ((argument = string_os_list_iterator_next(&arg_it)).pointer)
        {
            ...
            StringOs build_flag_strings[] = {
                [(u64)BuildFlag::BUILD_OPTION_FLAG_OPTIMIZE] = SOs("--optimize="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_FUZZ] = SOs("--fuzz="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_HAS_DEBUG_INFORMATION] = SOs("--has-debug-information="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_UNITY_BUILD] = SOs("--unity-build="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_JUST_PREPROCESSOR] = SOs("--just-preprocessor="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_SELF_HOSTED] = SOs("--self-hosted="),
                [(u64)BuildFlag::BUILD_OPTION_FLAG_SANITIZE] = SOs("--sanitize="),
            ...
            let boolean_result = boolean_argument_process(build_flag_strings, BUSTER_ARRAY_LENGTH(build_flag_strings), build_program_state.value_flags, (u64)BuildFlag::Count, argument);
            bool found = boolean_result.valid && boolean_result.index < (u64)BuildFlag::Count;

            if (found)
            {
                flag_set(build_program_state.set_flags, (BuildFlag)boolean_result.index, true);
            }

            if (!found)
            {
                StringOs integer_option_strings[] = {
                    [(u64)BuildIntegerOption::BUILD_OPTION_INTEGER_FUZZ_DURATION_SECONDS] = SOs("--fuzz-duration="),
                };
                ...
                for (EACH_ENUM_INT_FREE(BuildIntegerOption, build_integer_option))
                {
                    ...
                    if (string_os_starts_with_sequence(argument, integer_option_string))
                    {
                        ...
                        let format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_DECIMAL_POSITIVE;
                        ...
                        if (value_string.length > 2 && value_string.pointer[0] == '0')
                        {
                            ...
                            switch (second_ch)
                            {
                                break; case 'x': format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_HEXADECIMAL; prefix_character_count = 2; p = string_os_parse_u64_hexadecimal(value_string.pointer + 2);
                                break; case 'd': format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_DECIMAL_POSITIVE; prefix_character_count = 2; p = string_os_parse_u64_decimal(value_string.pointer + 2);
                                break; case 'o': format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_OCTAL; prefix_character_count = 2; p = string_os_parse_u64_octal(value_string.pointer + 2);
                                break; case 'b': format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_BINARY; prefix_character_count = 2; p = string_os_parse_u64_binary(value_string.pointer + 2);
                                break; default: {}
                            }
                        }
                        else if (value_string.length > 1 && value_string.pointer[0] == '-')
                        {
                            format = BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_DECIMAL_NEGATIVE;
                            prefix_character_count = 1;
                            p = string_os_parse_u64_decimal(value_string.pointer + 1);
                        }

                        if (format == BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_DECIMAL_POSITIVE && p.length == 0)
                        {
                        ...
                        {
                            if (format == BuildIntegerOptionFormat::BUILD_OPTION_INTEGER_FORMAT_DECIMAL_NEGATIVE)
                            {
                ...
                found = build_integer_option != (u64)BuildIntegerOption::Count;
            }

            if (!found)
            {
                StringOs string_option_strings[] = {
                    [(u64)BuildStringOption::BUILD_OPTION_STRING_XC_SDK_PATH] = SOs("--xc-sdk-path="),
                    [(u64)BuildStringOption::BUILD_OPTION_STRING_PRINT_FUNCTION_OPTIMIZATION] = SOs("--print-function-optimization="),
                };
                ...
                // for (build_string_option = 0; build_string_option < BUILD_OPTION_STRING_COUNT; build_string_option += 1)
                {
                    ...
                    if (string_os_starts_with_sequence(argument, string_option_string))
                    {
                        ...
                        string8_print(S8("Setting {SOs}{SOs}...\n"), string_option_string, value_string);

                        break;
                    }
                }

                found = build_string_option != (u64)BuildStringOption::Count;
            }

            if (!found)
            {
                let os_argument_process_result = buster_argument_process(argv, envp, argument_count, argument);
                if (os_argument_process_result != ProcessResult::Success)
                {
    ...
    if (!build_flag_is_set(BuildFlag::BUILD_OPTION_FLAG_HAS_DEBUG_INFORMATION))
    {
        build_flag_set(BuildFlag::BUILD_OPTION_FLAG_HAS_DEBUG_INFORMATION, true);
    }
    ...
    {
        flag_set(program_state->input.flags, ProgramFlag::Verbose, true);
    }

    return result;
}

BUSTER_F_IMPL ToolchainInformation toolchain_get_information(Arena* arena, LLVMVersion llvm_version)
{
    StringOs home_path;
#if defined(_WIN32)
    ...
    let install_path = string_os_join_arena(arena, (Slice<StringOs>)BUSTER_ARRAY_TO_SLICE(install_path_parts), true);
    ...
    let llvm_basename = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(llvm_basename_parts), true);
    ...
    let url = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(url_parts), true);
    ...
    let prefix_path = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(prefix_path_parts), true);
    ...
    let clang_path = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(clang_path_parts), true);
...
BUSTER_F_IMPL StringOsList build_compile_link_arguments(Arena* arena, const CompileLinkOptions * const restrict options)
{
    ...
    if (vulkan_sdk_path.pointer)
    {
        ...
        vulkan_include_flag = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(parts), true);
    }

    StringOs function_optimization_log_argument = {};
    if (options->function_optimization_log.pointer)
    {
        StringOs parts[] = {
            SOs("-filter-print-funcs="),
            options->function_optimization_log,
        };

        function_optimization_log_argument = string_os_join_arena(arena, (Slice<StringOs>) BUSTER_ARRAY_TO_SLICE(parts), true);
    }
    ...
    if (options->sanitize)
    {
        if (!(options->target->pointer->cpu_arch == CpuArch::CPU_ARCH_AARCH64 && options->target->pointer->os == OperatingSystem::OPERATING_SYSTEM_WINDOWS))
        {
    ...
    if (options->compile)
    {
        ...
        bool is_c = string_os_ends_with_sequence(options->source_paths[0], SOs(".c"));
        let std_flags = is_c ? (Slice<StringOs>)BUSTER_ARRAY_TO_SLICE(c_std_flags) : (Slice<StringOs>)BUSTER_ARRAY_TO_SLICE(cpp_std_flags);

        for (u64 i = 0; i < std_flags.length; i += 1)
        {
            let std_flag = std_flags.pointer[i];
        ...
        string_os_list_builder_append(builder, BUILD_BOOLEAN_DEFINE(options->fuzz, BUSTER_FUZZ));
        ...
        string_os_list_builder_append(builder, BUILD_BOOLEAN_DEFINE(options->optimize, BUSTER_OPTIMIZE));
        ...
        if (function_optimization_log_argument.pointer)
        {
            string_os_list_builder_append(builder, SOs("-S"));
            string_os_list_builder_append(builder, SOs("-emit-llvm"));

            string_os_list_builder_append(builder, SOs("-mllvm"));
            string_os_list_builder_append(builder, SOs("-print-before-all"));

            string_os_list_builder_append(builder, SOs("-mllvm"));
            string_os_list_builder_append(builder, SOs("-print-after-all"));

            string_os_list_builder_append(builder, SOs("-mllvm"));
            string_os_list_builder_append(builder, SOs("-print-machine-instrs"));

            string_os_list_builder_append(builder, SOs("-mllvm"));
            string_os_list_builder_append(builder, function_optimization_log_argument);

            // string_os_list_builder_append(builder, SOs("-mllvm"));
            // string_os_list_builder_append(builder, SOs("-print-pipeline-passes"));
        }
    }

    if (!options->just_preprocessor && options->link && !function_optimization_log_argument.pointer)
    {
        ...
        if (options->use_graphics)
        {
            ...
            {
                break; case OperatingSystem::OPERATING_SYSTEM_LINUX:
                {
        ...
        if (options->target->pointer->os == OperatingSystem::OPERATING_SYSTEM_WINDOWS)
        {
